...
const ThermoSudokuGame: React.FC<Props> = ({ difficulty, onComplete, onExit }) => {
    ...
    const generatePuzzle = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            const usedInThermo = Array(9).fill(false).map(() => Array(9).fill(false));
            let thermoCount = 0;

            // Try to generate 4 to 8 thermometers based on difficulty
            const targetThermos = difficulty === Difficulty.VERY_EASY ? 8 : difficulty === Difficulty.EASY ? 6 : difficulty === Difficulty.MEDIUM ? 5 : 4;
            ...
            for (let attempt = 0; attempt < 100 && thermoCount < targetThermos; attempt++) {
                ...
                while (path.length < targetLength) {
                    ...
                    const neighbors = [
                        ...
                        { r: last.r - 1, c: last.c - 1 }, { r: last.r + 1, c: last.c + 1 },
                        { r: last.r - 1, c: last.c + 1 }, { r: last.r + 1, c: last.c - 1 }
                    ].filter(n => n.r >= 0 && n.r < 9 && n.c >= 0 && n.c < 9
                        && !usedInThermo[n.r][n.c]
                        && !path.some(p => p.r === n.r && p.c === n.c)
                        && board[n.r][n.c] > currentVal); // Must strictly increase!
        ...
        setGrid(prev => {
            ...
            for (let i = 0; i < 9; i++) {
                for (let j = 0; j < 9; j++) {
                    ...
                    for (let k = 0; k < 9; k++) {
                        if (k !== j && next[i][k].value === val) { next[i][j].hasError = true; next[i][k].hasError = true; anyError = true; }
                        if (k !== i && next[k][j].value === val) { next[i][j].hasError = true; next[k][j].hasError = true; anyError = true; }
                    }
                    ...
                    for (let ii = 0; ii < 3; ii++) {
                        for (let jj = 0; jj < 3; jj++) {
                            if ((br + ii !== i || bc + jj !== j) && next[br + ii][bc + jj].value === val) {
                                next[i][j].hasError = true; next[br + ii][bc + jj].hasError = true; anyError = true;
                            }
                        }
                    }
                }
    ...
    const getStars = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 300;
            default: return 80;
        }
    };
    ...
    return (
        <div className="flex flex-col items-center max-w-4xl mx-auto w-full">
            <div className="flex justify-between items-center w-full mb-6 relative px-4">
                <button onClick={onExit} className="px-6 py-2 bg-white/10 text-white rounded-full font-bold hover:bg-white/20 transition-all flex items-center gap-2">
                    <span>⬅</span> Çıkış
                </button>
                <div className="absolute left-1/2 -translate-x-1/2 text-center w-max">
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-600 drop-shadow-lg uppercase tracking-wider">Thermo Sudoku</h2>
                </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-4 md:p-8 rounded-[40px] shadow-2xl border border-red-500/20 flex flex-col items-center relative overflow-hidden w-full max-w-2xl">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-md mb-8">
                            <li className="flex items-start gap-3">
                                <span className="text-xl">🌡️</span><span>Izgara üzerinde termometre şekilleri bulunmaktadır.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">1️⃣</span><span>Klasik Sudoku kuralları tüm tahtada geçerlidir.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">📈</span><span>Bir termometrenin <b>yuvarlak ampul kestiğinden</b> yukarı (ucuna) doğru gidildikçe rakamlar <b>KESİNLİKLE ARTMALIDIR</b>.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">💡</span><span>Örneğin: Ampulde 2 varsa, sonraki kare 3-4-5-6-7-8 veya 9 olabilir. Asla 2 veya 1 olamaz! Rakamlar atlayarak artabilir (Örn: 2, 5, 8 geçerlidir).</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="bg-gradient-to-r from-red-600 to-red-800 text-white font-black py-4 px-12 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all text-xl">
                            ANLADIM, BAŞLA!
                        </button>
                    </div>
                )}
                ...
                <div className="mb-4 flex justify-between items-center w-full opacity-90">
                    <span className="text-white font-bold bg-white/10 px-4 py-2 rounded-xl">Ödül: {getStars()} ⭐</span>
                    {isGenerating && <span className="text-red-400 font-bold animate-pulse">Sıcaklık Ölçülüyor...</span>}
                    <button onClick={generatePuzzle} disabled={isGenerating} className="text-white hover:text-red-400 font-bold transition-colors disabled:opacity-50">Yeniden Düşman Getir 🔄</button>
                </div>
                ...
                <div className="grid grid-cols-9 gap-[1px] p-[2px] bg-red-950/80 rounded-xl shadow-[0_0_30px_rgba(248,113,113,0.1)] select-none border-4 border-slate-950 relative">
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" style={{ padding: '2px' }}>
                        {thermos.map(t => (
                            <g key={t.id}>
                                {/* Bulb */}
                                <circle
                        ...
                        row.map((cell, c) => {
                            ...
                            const isRelated = selectedCell &&
                                (selectedCell.r === r || selectedCell.c === c ||
                                    (Math.floor(selectedCell.r / 3) === blkR && Math.floor(selectedCell.c / 3) === blkC)
                                );

                            const isSameNumber = selectedCell && grid[selectedCell.r][selectedCell.c].value === cell.value && cell.value !== null;
                ...
                <div className="mt-8 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-lg">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                        <button key={num} onClick={() => handleNumInput(num)} className="bg-white/10 hover:bg-white/20 text-white font-black text-xl py-3 rounded-xl transition-all shadow-md hover:scale-105 active:scale-95">
                            {num}
                        </button>
                    ))}
                    <button onClick={() => handleNumInput(null)} className="bg-red-500/20 hover:bg-red-500/40 text-red-100 font-bold text-sm rounded-xl py-3 transition-all flex items-center justify-center">SİL</button>
                </div>
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/70 backdrop-blur-sm rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-red-600 to-red-900 p-8 rounded-[32px] text-center shadow-[0_0_50px_rgba(248,113,113,0.5)] scale-110 border-4 border-white">
                            ...
                            <h3 className="text-4xl font-black text-white mb-2 drop-shadow-lg">Ateşin Yükseldi!</h3>
                            <p className="text-red-200 font-bold mb-4">Mükemmel çözüm!</p>
                            <button onClick={() => onComplete(getStars())} className="bg-white text-red-900 px-8 py-4 mt-2 rounded-full font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all w-full">
                                +{getStars()} ⭐ KAZAN
                            </button>
                        </div>
                    </div>
