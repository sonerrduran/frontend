...
const TrainTracksGame: React.FC<TrainTracksProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        const initial = Array(size).fill('').map(() => Array(size).fill(''));
    ...
    const checkWinCondition = (currentGrid: TrackType[][]) => {
        ...
        if (boardData.start.dir === 'top') { prevR = -1; prevC = currC; }
        if (boardData.start.dir === 'left') { prevR = currR; prevC = -1; }
        // Add more if needed
        ...
        while (true) {
            ...
            const accepts = nextConns.some(conn =>
                (nextCell!.r + conn.dr === currR) && (nextCell!.c + conn.dc === currC)
            );
    ...
    const renderTrackSVG = (type: TrackType) => {
        ...
        return (
            <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md p-1">
                {type === 'hz' && <line x1="0" y1="50" x2="100" y2="50" stroke={color} strokeWidth={w} strokeLinecap="round" />}
                {type === 'vt' && <line x1="50" y1="0" x2="50" y2="100" stroke={color} strokeWidth={w} strokeLinecap="round" />}
                {type === 'tr' && <path d="M 50 0 A 50 50 0 0 0 100 50" fill="transparent" stroke={color} strokeWidth={w} strokeLinecap="round" />}
                {type === 'tl' && <path d="M 50 0 A 50 50 0 0 1 0 50" fill="transparent" stroke={color} strokeWidth={w} strokeLinecap="round" />}
                {type === 'br' && <path d="M 50 100 A 50 50 0 0 1 100 50" fill="transparent" stroke={color} strokeWidth={w} strokeLinecap="round" />}
                {type === 'bl' && <path d="M 50 100 A 50 50 0 0 0 0 50" fill="transparent" stroke={color} strokeWidth={w} strokeLinecap="round" />}
            </svg>
        );
    };
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                <span>🛤️</span>
                                <div>Düz veya kavisli ray parçaları döşeyerek Başlangıç ve Bitiş noktalarını birleştirmelisin.</div>
                            </li>
                            <li className="flex gap-3 text-emerald-400 font-bold">
                                <span>🔢</span>
                                <div>Kenarlardaki sayılar, o satır veya sütunda **kaç adet ray parçası** olması gerektiğini söyler!</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-slate-400">🖱️</span>
                                <span>Hücreye sol tıklayarak rayları çevir. Kesinlikle ray olmayan yere sağ tıklayarak **X** işareti koyabilirsin.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-500/30 active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-500 tracking-wider">Tracks</h2>
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex ml-12 mb-2">
                            {boardData.colClues.map((clue, i) => (
                                <div key={`tc-${i}`} className={`w-12 sm:w-16 text-center font-black text-lg
                        ...
                        <div className="flex">
                            ...
                            <div className="flex flex-col mr-2">
                                {boardData.rowClues.map((clue, i) => (
                                    <div key={`lc-${i}`} className={`h-12 sm:h-16 flex items-center justify-end pr-2 font-black text-lg w-10
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Parça seçmek için <span className="text-white font-bold cursor-pointer">Sol Tık</span> art arda bas, X içi <span className="text-white font-bold cursor-pointer">Sağ Tık</span> yap.</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <div className="flex flex-col gap-3">
                            <button onClick={() => onComplete(score)} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white shadow-lg active:scale-95 text-transparent bg-clip-text text-white">Ödülü Al ⭐️</button>
