...
const WORD_PAIRS = [
    { a: 'kalem', b: 'kalem' }, { a: 'çıta', b: 'çıta' }, { a: 'mağaza', b: 'mağaza' }, { a: 'sefalet', b: 'sefareti' },
    { a: 'tekrar', b: 'tekrar' }, { a: 'nefes', b: 'nefes' }, { a: 'uçurtma', b: 'uçuşma' }, { a: 'sıkıcı', b: 'sıkıcı' },
    { a: 'maden', b: 'maden' }, { a: 'plan', b: 'plan' }, { a: 'balkan', b: 'balkan' }, { a: 'yamuk', b: 'yamak' },
    { a: 'kader', b: 'kader' }, { a: 'sincap', b: 'sincap' }, { a: 'kazan', b: 'kızan' }, { a: 'resim', b: 'resim' },
    { a: 'armatör', b: 'armatör' }, { a: 'hareket', b: 'hareket' }, { a: 'yanık', b: 'yanık' }, { a: 'anket', b: 'anket' }
];
    ...
    const renderOverlay = () => (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-100/60 backdrop-blur-[2px] z-50 rounded-[32px]">
            <div className="bg-white p-10 rounded-3xl shadow-2xl text-slate-800 text-center max-w-lg border border-slate-200">
                <h3 className="text-2xl font-black mb-4 uppercase tracking-tight text-slate-600">BENZER KELİMELER</h3>
                <p className="text-slate-500 text-sm mb-8 leading-relaxed font-medium">
                    Bu egzersiz konsantrasyonunuzu ve beyin algılama hızınızı geliştirmeye yöneliktir.
                    Kutucuk içerisinde iki kelime gösterilmektedir, bazı kutucuklarda bulunan kelimeler farklıdır.
                    Farklı olan kelimeleri bulun!
                </p>
                <div className="flex gap-4 justify-center">
                    <button onClick={() => { setIsPlaying(true); setTimer(60); setScore(0); }} className="bg-sky-600 hover:bg-sky-700 text-white px-8 py-3 rounded-lg font-black uppercase text-sm transition-all shadow-lg">Egzersizi Başlat</button>
                    <button className="bg-slate-400 hover:bg-slate-500 text-white px-8 py-3 rounded-lg font-black uppercase text-sm transition-all">Ayarlar</button>
                </div>
            </div>
        </div>
    );
        ...
        <div className="w-full h-screen bg-slate-100 flex flex-col font-sans overflow-hidden">
            <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-6">
                    <button onClick={onExit} className="text-slate-400 hover:text-slate-600 font-black">⬅</button>
                    <h1 className="text-xl font-black text-slate-700 uppercase tracking-tight">BENZER KELİMELER</h1>
                </div>

                <div className="flex items-center gap-4">
                    <select value="1:00" className="bg-white border border-slate-300 rounded p-1 text-sm font-bold text-slate-600">
                        <option>1:00</option>
                    </select>
                    <select value="5 Kelime" className="bg-white border border-slate-300 rounded p-1 text-sm font-bold text-slate-600">
                        <option>5 Kelime</option>
                    </select>
                </div>

                <div className="flex items-center gap-4">
                    <div className="bg-slate-700 text-[#7bb] px-4 py-1 rounded font-bold text-sm uppercase">Kalan: {mismatches.length}</div>
