...
const DominosaGame: React.FC<DominosaProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getConfig = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 5;
            default: return 4;
        }
    };
    ...
    const [placedDominoes, setPlacedDominoes] = useState<{ r1: number, c1: number, r2: number, c2: number }[]>([]);
    ...
    const checkWinCondition = useCallback((
        ...
        const usedCells = Array(boardData.rows).fill(false).map(() => Array(boardData.cols).fill(false));
        ...
        if (uniqueDoms.size === formedDominoes.length && formedDominoes.length === generateAllDominoes().length) {
            handleGameOver(true);
        } else {
            addMistake();
        }
    ...
    const initGame = useCallback(() => {
        setPlacedDominoes([]);
        setHConns(Array(boardData.rows - 1).fill(0).map(() => Array(boardData.cols).fill(0)));
        setVConns(Array(boardData.rows).fill(0).map(() => Array(boardData.cols - 1).fill(0)));
    ...
    const isCellUsed = (r: number, c: number, checkPlaced: { r1: number, c1: number, r2: number, c2: number }[]) => {
    ...
    const toggleConn = (type: 'h' | 'v', r: number, c: number, buttonObj: number) => {
        ...
        if (buttonObj === 0) { // Left Click (Join)
            if (currentVal === 1) {
                // Remove join
                if (type === 'h') newH[r][c] = 0; else newV[r][c] = 0;
                curPlaced = curPlaced.filter(p => !(p.r1 === r1 && p.c1 === c1 && p.r2 === r2 && p.c2 === c2));
            } else {
                // Ensure neither cell is already used
                if (isCellUsed(r1, c1, curPlaced) || isCellUsed(r2, c2, curPlaced)) return;

                // Add join
                if (type === 'h') newH[r][c] = 1; else newV[r][c] = 1;
                // If there were X's, overwrite
                curPlaced.push({ r1, c1, r2, c2 });
            }
        } else if (buttonObj === 2) { // Right Click (X)
            if (currentVal === 2) {
                // Remove X
                if (type === 'h') newH[r][c] = 0; else newV[r][c] = 0;
            } else {
                // Add X
                if (type === 'h') newH[r][c] = 2; else newV[r][c] = 2;
        ...
        >
            ...
            <div className="flex flex-col lg:flex-row gap-8 justify-center w-full">
                <div className="relative select-none touch-none mx-auto bg-slate-100 p-6 sm:p-8 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border-4 border-slate-300">
                    <div className="flex flex-col gap-2">{boardData.numbers.map((row, r) => (
                                <React.Fragment key={`row-${r}`}>
                                    <div className="flex gap-2">
                                        {row.map((cellObj, c) => {
                                            ...
                                            let joinedRight = placedDominoes.some(p => p.r1 === r && p.c1 === c && p.r2 === r && p.c2 === c + 1);
                                            let joinedLeft = placedDominoes.some(p => p.r1 === r && p.c1 === c - 1 && p.r2 === r && p.c2 === c);
                                            let joinedBottom = placedDominoes.some(p => p.r1 === r && p.c1 === c && p.r2 === r + 1 && p.c2 === c);
                                            let joinedTop = placedDominoes.some(p => p.r1 === r - 1 && p.c1 === c && p.r2 === r && p.c2 === c);

                                            const baseColor = isUsed ? 'bg-slate-800 text-white' : 'bg-white text-slate-800';
                                            ...
                                            return (
                                                <React.Fragment key={`cell-${r}-${c}`}>
                                                    <div className={`w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center font-black text-2xl sm:text-3xl transition-colors duration-200 border-2 border-slate-300/50 shadow-sm z-10 ${baseColor} ${borderRadius}`}>
                                                        {cellObj}
                                                    </div>
                                                    ...
                                                    {c < boardData.cols - 1 && (
                                                        <div
                                                            ...
                                                            onClick={(e) => { e.preventDefault(); toggleConn('v', r, c, 0); }}
                                                            onContextMenu={(e) => { e.preventDefault(); toggleConn('v', r, c, 2); }}
                                                        >
                                                            <div className={`w-10 h-6 sm:w-14 sm:h-8 rounded-full flex items-center justify-center transition-colors
                                                ...
                                                return (
                                                    <React.Fragment key={`hjoin-${r}-${c}`}>
                                                        <div
                                                            ...
                                                            onClick={(e) => { e.preventDefault(); toggleConn('h', r, c, 0); }}
                                                            onContextMenu={(e) => { e.preventDefault(); toggleConn('h', r, c, 2); }}
                                                        >
                                                            <div className={`w-6 h-10 sm:w-8 sm:h-14 rounded-full flex items-center justify-center transition-colors
                ...
                <div className="bg-slate-800/80 p-6 rounded-3xl border border-white/10 shadow-xl flex-1 max-w-sm">
                    <h3 className="text-center text-slate-400 font-bold mb-4 text-sm uppercase tracking-wider flex items-center justify-center gap-2">
                        ...
                        <span className="bg-slate-900 px-2 py-0.5 rounded-full text-white">{placedDominoes.length} / {allDominoes.length}</span>
                    </h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {allDominoes.map(domino => {
                            ...
                            return (
                                <div key={domino} className={`flex rounded-xl overflow-hidden border-2 transition-all duration-300
                                    ${isPlaced ? 'border-slate-700 opacity-30 grayscale' : 'border-slate-600 bg-white shadow-md shadow-white/5'}
                                `}>
                                    <div className={`w-1/2 flex items-center justify-center py-2 font-black text-lg ${isPlaced ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-800'}`}>{d1}</div>
                                    <div className="w-[2px] bg-slate-400/30"></div>
                                    <div className={`w-1/2 flex items-center justify-center py-2 font-black text-lg ${isPlaced ? 'bg-slate-800 text-slate-500' : 'bg-white text-slate-800'}`}>{d2}</div>
                                </div>
                            );
                        })}
                    </div>
