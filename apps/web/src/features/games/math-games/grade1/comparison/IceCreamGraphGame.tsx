...
const IceCreamGraphGame: React.FC<Props> = ({ onComplete, onExit }) => {
  ...
  const generateLevel = () => {
    ...
    const selected = [...flavorData].sort(() => Math.random() - 0.5).slice(0, numFlavors)
    ...
    if (!isNaN(Number(answer))) {
      ...
      const wrongOptions = selected.filter(f => f.name !== answer).map(f => f.name).slice(0, 2);
      setOptions([answer, ...wrongOptions].sort(() => Math.random() - 0.5));
    }
        ...
        <div className="bg-white/10 rounded-3xl p-8 mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
            {flavors.map((flavor, idx) => (
              <div key={idx} className="bg-gradient-to-br from-pink-500 to-rose-600 rounded-2xl p-6 text-center">
                ...
                <div className="bg-white/20 rounded-xl p-3 mb-3">
                  <div className="flex flex-col-reverse gap-1">
                    {Array.from({ length: flavor.count }).map((_, i) => (
                      <div key={i} className="bg-gradient-to-r from-yellow-400 to-orange-400 h-6 rounded flex items-center justify-center">
