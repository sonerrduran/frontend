...
const ShikakuGame: React.FC<ShikakuGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const isCellInRect = (r: number, c: number, rect: { r1: number, c1: number, r2: number, c2: number }) => {
    ...
    const getCellAppearance = (r: number, c: number) => {
        ...
        if (committedRect) {
            const isError = committedRect.targetNum === null || committedRect.area !== committedRect.targetNum;
            let borders = '';
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8" onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
                    ...
                    <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-orange-600 rounded-2xl p-8 md:p-12 mb-8">
                                ...
                                {Array.from({ length: size }).map((_, r) =>
                                    Array.from({ length: size }).map((_, c) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {clue !== null && (
                                                    <span className={`z-0 ${appearance.isError ? 'text-red-700' : 'text-slate-800'}`}>
                                                        {clue}
                                                    </span>
                                                )}
                                            </div>
                        ...
                        <button onClick={() => setShowRules(true)} className="w-full bg-orange-500 hover:bg-orange-400 text-white text-xl font-black py-4 rounded-2xl transition-all">
                            📖 NASIL OYNANIR?
                        </button>
                    </div>
                </div>
                ...
                {showRules && (
                    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center rounded-3xl animate-in fade-in zoom-in">
                        <div className="bg-slate-800/80 p-6 md:p-8 rounded-3xl border border-orange-500/30 max-w-md w-full">
                            ...
                            <h3 className="text-2xl md:text-3xl font-black text-orange-400 mb-4 uppercase">Nasıl Oynanır?</h3>
                            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base font-medium">
                                <li className="flex gap-2"><span className="text-orange-400">1.</span> Amacın, tüm oyun alanını dikdörtgen (veya kare) alanlara bölmektir.</li>
                                <li className="flex gap-2"><span className="text-orange-400">2.</span> Çizdiğin her dikdörtgenin içinde tam olarak bir tane sayı olmalıdır.</li>
                                <li className="flex gap-2"><span className="text-orange-400">3.</span> Dikdörtgenin içindeki küçük karelerin sayısı (alanı), içindeki sayıya eşit olmalıdır.</li>
                                <li className="flex gap-2"><span className="text-orange-400">4.</span> Bir dikdörtgen oluşturmak için hücreyi basılı tutup sürükle. Sürükleyip bıraktığında dikdörtgen boyanacaktır. Yanlışsa üzerine tıkla/dokun, silinir!</li>
                                <li className="flex gap-2"><span className="text-orange-400">5.</span> Hiç boşluk kalmadığında ve tüm sayılar doğru kapsandığında kazanırsın!</li>
                            </ul>
                            <button
                ...
                {isGameOver && (
                    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-3xl">
                        <h3 className={`text-5xl md:text-7xl font-black mb-4 ${timeLeft === 0 ? 'text-red-500' : 'text-orange-400'}`}>
                            {timeLeft === 0 ? 'SÜRE BİTTİ' : 'ALANLAR BÖLÜNDÜ!'}
                        </h3>
                        <p className="text-xl md:text-2xl text-white font-bold mb-8">Kazanılan Yıldız: <span className="text-orange-400">⭐ {score}</span></p>
