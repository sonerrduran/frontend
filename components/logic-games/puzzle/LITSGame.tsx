...
const LITSGame: React.FC<LITSGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const identifyTetromino = (cells: { r: number, c: number }[]) => {
        ...
        const local = Array(h).fill(0).map(() => Array(w).fill(false));
        cells.forEach(c => {
        ...
        // L / J
        const L_shapes = [
            '10-10-11', '01-01-11', '11-10-10', '11-01-01', // 2x3
            '111-100', '111-001', '100-111', '001-111'      // 3x2
        ];
        ...
        if (!hasError) {
            for (let r = 0; r < size - 1; r++) {
                for (let c = 0; c < size - 1; c++) {
                    if (currentGrid[r][c] === 1 && currentGrid[r + 1][c] === 1 &&
                        currentGrid[r][c + 1] === 1 && currentGrid[r + 1][c + 1] === 1) {
                        hasError = true;
                    }
                }
            }
    ...
    const getBorderClasses = (r: number, c: number) => {
        ...
        // Bottom
        if (r === size - 1 || getRegionId(r + 1, c) !== reg) classes += " border-b-[4px] border-b-slate-800";
        ...
        // Right
        if (c === size - 1 || getRegionId(r, c + 1) !== reg) classes += " border-r-[4px] border-r-slate-800";
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Her kalın bölgenin içine **L, I, T veya S** şekillerinden (Tetris blokları - 4 hücreli) tam 1 tane çizmelisin. O şekli (kutu) çizmek yasak.</div>
                            </li>
                            ...
                            <li className="flex gap-3 text-rose-400 font-bold">
                                <span>🚫</span>
                                <div>Siyah hücreler hiçbir yerde **2x2 bir kare oluşturamaz** ve **Aynı iki harf/şekil birbirine değemez!**</div>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-orange-600 hover:bg-orange-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-3xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-500 tracking-wider">LITS</h2>
                    ...
                    <div className="text-right flex flex-col items-end gap-2 text-white">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-orange-300'}`}>
                                ...
                                <div key={r} className="flex">
                                    {row.map((cell, c) => (
                                        <button
                                            ...
                                            `}
                                        >
                                            {cell === 2 && <span className="text-slate-400 font-black text-2xl select-none">×</span>}
                                        </button>
                                    ))}
                                </div>
                            ))}
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Blok koymak için <span className="text-white font-bold">Sol Tık</span>, boşluk işareti için <span className="text-white font-bold">Sağ Tık (X)</span>.</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Kural İhlali!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika Şekiller!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Aynı harfler temas etti, 2x2 oluştu veya ağ koptu.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tüm kurallara uyarak Tetromino ağını kurdun! Kazanılan Yıldız: ${score}`}
                        </p>

                        <div className="flex flex-col gap-3">
                            {mistakes < maxMistakes && timeLeft > 0 && (
                                <button onClick={() => onComplete(score)} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-400 hover:to-amber-500 text-white shadow-lg active:scale-95 text-transparent bg-clip-text text-white">Ödülü Al ⭐️</button>
                            )}
