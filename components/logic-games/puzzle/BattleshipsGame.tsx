...
const BattleshipsGame: React.FC<BattleshipsGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
                ...
                if (currentBoard[r][c] === 'ship' && !visited[r][c]) {
                    ...
                    while (stack.length > 0) {
                        ...
                        for (const [dr, dc] of orthogonals) {
                            ...
                            if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited[nr][nc] && currentBoard[nr][nc] === 'ship') {
                                visited[nr][nc] = true;
                                stack.push([nr, nc]);
                            }
                        }
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Kenarlardaki sayılar, o satır veya sütunda toplam kaç <b>gemi parçası</b> (Ship) olduğunu gösterir.</div>
                            </li>
                            <li className="flex gap-3">
                            ...
                            <li className="flex gap-3 text-red-400 font-bold">
                                <span>🚫</span>
                                <span>Gemiler birbirlerine (kendi parçaları hariç) asla temas edemezler, <b>çaprazdan bile!</b></span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-blue-600 hover:bg-blue-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Battleships</h2>
                    ...
                    <div className="relative inline-block mx-auto lg:mx-0">
                        ...
                        <div className="flex ml-8 mb-2 w-fit">
                            {boardData.clues.col.map((c, i) => {
                                ...
                                return (
                                    <div key={`c-${i}`} className={`w-10 h-8 md:w-14 flex items-end justify-center font-black text-xl 
                                        ${isOver ? 'text-rose-500' : isMatched ? 'text-slate-500' : 'text-white'}`}>{c}</div>
                                )
                            })}
                        </div>
                        ...
                        <div className="flex">
                            ...
                            <div className="flex flex-col mr-2">
                                {boardData.clues.row.map((c, i) => {
                                    ...
                                    const isOver = currentCount > c;
                                    return (
                                        <div key={`r-${i}`} className={`w-6 h-10 md:h-14 flex items-center justify-end font-black text-xl pr-2 
                                            ${isOver ? 'text-rose-500' : isMatched ? 'text-slate-500' : 'text-white'}`}>{c}</div>
                                    )
                                })}
                            </div>
                                    ...
                                    row.map((cellState, c) => (
                                        <button
                                            ...
                                            className={`w-10 h-10 md:w-14 md:h-14 flex items-center justify-center text-3xl transition-all duration-200
                                                ${cellState === 'ship' ? 'bg-slate-700 shadow-[inset_0_3px_10px_rgba(0,0,0,0.8)] border border-slate-600' :
                                                    cellState === 'water' ? 'bg-cyan-800/80 text-cyan-300' :
                        ...
                        <div className="flex flex-col gap-3">
                            {boardData.fleet.map((shipSize, idx) => (
                                <div key={idx} className="flex gap-1 justify-center bg-slate-900/50 p-2 rounded-xl">
                                    {Array(shipSize).fill(0).map((_, i) => (
                                        <div key={i} className="w-6 h-6 bg-slate-600 rounded-sm flex items-center justify-center text-xs">🚢</div>
                                    ))}
                                </div>
                            ))}
                        </div>
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-2xl mx-auto">
                    <p className="text-slate-400 text-sm font-bold">1 Tık: <span className="text-cyan-300 bg-cyan-800 px-2 rounded-sm mx-1">Su (〰)</span> | 2 Tık: <span className="bg-slate-700 px-2 rounded-sm mx-1">Gemi (🚢)</span> | 3 Tık: Boş</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Gemiler çaprazdan bile birbirine değemez.' : timeLeft <= 0 ? 'Zamanı iyi yönetemedin.' : `Tüm filoyu başarıyla deşifre ettin! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-blue-600 hover:bg-blue-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
