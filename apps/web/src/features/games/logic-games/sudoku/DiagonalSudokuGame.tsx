...
const DiagonalSudokuGame: React.FC<Props> = ({ difficulty, grade, onComplete, onExit }) => {
    ...
    useEffect(() => {
        ...
        demoGrid[0][0] = 5; demoGrid[4][4] = 9; demoGrid[8][8] = 2; // Main diag
        demoGrid[0][8] = 7; demoGrid[8][0] = 3; // Anti diag
        setGrid(demoGrid);

        const initFlags = Array(9).fill(null).map(() => Array(9).fill(false));
        initFlags[0][0] = true; initFlags[4][4] = true; initFlags[8][8] = true;
        initFlags[0][8] = true; initFlags[8][0] = true;
            ...
            <div className="bg-slate-900/80 p-6 rounded-3xl border border-white/10 shadow-2xl">
                        ...
                        row.map((val, c) => {
                          ...
                          `}
                                >
                                    ...
                                    {inDiag && (
                                        <div className={`absolute pointer-events-none w-full h-full opacity-20 ${r === c ? 'bg-gradient-to-br' : 'bg-gradient-to-bl'} from-fuchsia-500 to-transparent`}></div>
                                    )}
                ...
                <div className="grid grid-cols-5 md:grid-cols-9 gap-2">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                        <button key={num} onClick={() => handleNumberClick(num)} disabled={!selectedCell} className="bg-fuchsia-500/20 hover:bg-fuchsia-500/40 disabled:opacity-50 text-fuchsia-100 font-bold py-3 md:py-4 rounded-xl text-xl shadow-lg transition-all border border-fuchsia-500/30">
                            {num}
                        </button>
                    ))}
                </div>
