...
const HashiGame: React.FC<HashiGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const doBridgesCross = (b1: Bridge, b2: Bridge) => {
        ...
        return (
            horiz.rMin > vert.rMin && horiz.rMin < vert.rMax &&
            vert.cMin > horiz.cMin && vert.cMin < horiz.cMax
        );
    };
    ...
    const handleIslandClick = (islandId: string) => {
        ...
        if (selectedIslandId === null) {
            ...
            const tempBridge = {
                id: bridgeId, island1: i1.id, island2: i2.id, count: 1, orientation, rMin, rMax, cMin, cMax
            } as Bridge;
            ...
            if (nextCount > 0 && (i1Current + nextCount > i1.targetValue || i2Current + nextCount > i2.targetValue)) {
                setSelectedIslandId(null);
                return;
            }
                ...
                <div className="flex flex-col items-center w-full">
                    <div className="relative bg-slate-800 rounded-3xl p-4 md:p-8 shadow-[inset_0_4px_10px_rgba(0,0,0,0.5)] border border-slate-700 w-full max-w-[400px] aspect-square select-none">
                        {(Object.values(bridges) as Bridge[]).map((bridge) => {
                            ...
                            const length = (bridge.orientation === 'horizontal' ? bridge.cMax - bridge.cMin : bridge.rMax - bridge.rMin) * cSize;

                            return (
                                <div key={bridge.id} className="absolute z-10 pointer-events-none" style={{
                                    ...
                                    <div className={`${bridge.orientation === 'vertical' ? 'w-1.5 h-full' : 'h-1.5 w-full'} bg-cyan-400 rounded-full shadow-[0_0_8px_rgba(34,211,238,0.8)]`}></div>
                                    {bridge.count === 2 && (
                                        <div className={`${bridge.orientation === 'vertical' ? 'w-1.5 h-full' : 'h-1.5 w-full'} bg-cyan-400 rounded-full shadow-[0_0_8px_rgba(34,211,238,0.8)]`}></div>
                                    )}
                                </div>
                    ...
                    <p className="mt-8 text-white/70 text-sm font-medium text-center max-w-sm">
                        Bir adaya (daireye) tıkla, sonra aynı hizada (yatay/dikey) bulunan diğer bir adaya tıklayarak aralarında köprü kur. Çift köprü için aynı işlemi tekrarla!
                    </p>
                </div>
            </LogicGameWrapper>
