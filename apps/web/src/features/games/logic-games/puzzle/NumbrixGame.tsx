        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white font-sans selection:bg-rose-500/30">
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>1'den {size * size}'a kadar tüm sayıları <b>ardı ardına (zincirleme)</b> birleştirmen gerekiyor.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-amber-400">🧭</span>
                                <span>Sayılar yalnızca <b>Yatay ve Dikey</b> (yukarı, aşağı, sağ, sol) olarak birbirleriyle bağlantılı olabilir. Çapraz geçiş yasaktır.</span>
                            </li>
                            <li className="flex gap-3 text-emerald-400 font-bold">
                                <span>🎯</span>
                                <span>Siyah numaralar sabittir. Tablodan eksik sayıları bulup zinciri tamamla!</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-rose-600 hover:bg-rose-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12 flex flex-col items-center lg:flex-row lg:items-start lg:justify-center gap-8">
                <div className="flex-1 w-full max-w-md">
                    ...
                    <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-orange-500">Numbrix</h2>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-12 h-12 md:w-16 md:h-16 text-xl md:text-2xl font-black rounded-lg transition-all duration-200 flex items-center justify-center
                                                ${fixed ? 'bg-slate-800 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] cursor-not-allowed' :
                                                    selected ? 'bg-orange-400 text-white shadow-[0_0_15px_rgba(251,146,60,0.8)] scale-105 z-10' :
                                                        cell !== 0 ? 'bg-rose-100 text-rose-900 shadow-md hover:bg-rose-200' :
                ...
                <div className="w-full max-w-xs bg-slate-800 p-6 rounded-3xl shadow-2xl border border-white/10 mt-4 lg:mt-0">
                    ...
                    <p className="text-xs text-slate-400 mt-4 text-center leading-relaxed">Art arda basarak iki basamaklı sayı yazabilirsin (Maks: {size * size}).</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Zincir Koptu!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Sayı zinciri yatay ve dikey kesintisiz olmalıydı.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tüm sayıları eksiksiz birleştirdin! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-orange-600 hover:bg-orange-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
