...
const ZebraPuzzle: React.FC<ZebraPuzzleProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getTimeForDifficulty = () => {
        switch (difficulty) {
            case Difficulty.VERY_EASY:
            case Difficulty.EASY: return 600; // 10m
            case Difficulty.MEDIUM: return 900; // 15m
            case Difficulty.HARD:
            case Difficulty.VERY_HARD: return 1200; // 20m
            default: return 900;
        }
    };
    ...
    const initGame = useCallback(() => {
        const createEmptyGrid = () => Array(n).fill(null).map(() => Array(n).fill(0));
        setGrids({
            AB: createEmptyGrid(),
            AC: createEmptyGrid(),
            CB: createEmptyGrid(),
                    ...
                    for (let c2 = 0; c2 < n; c2++) {
                        if (b[r2][c2] === 1) {
                            for (let idx = 0; idx < n; idx++) {
                                if (idx !== c2 && b[r2][idx] === 0) { b[r2][idx] = 2; changed = true; }
                                if (idx !== r2 && b[idx][c2] === 0) { b[idx][c2] = 2; changed = true; }
                            }
                        }
                    }
                }
    ...
    const checkWinCondition = (currentGrids: typeof grids) => {
        ...
        let invalid = false;
        for (const [a, b, c] of puzzle.solution) {
            if (currentGrids.AB[a][b] === 2 || currentGrids.AC[a][c] === 2 || currentGrids.CB[c][b] === 2) {
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Aşağıdaki ipuçlarını okuyarak kimin neyle eşleştiğini bul. Klasik bir eleme (Einstein) bulmacasıdır.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-yellow-400">🖱️</span>
                                <span>Kutuya 1. tıkta kırmızı çarpı <b>(X)</b> - "Olamaz" işareti koyar. 2. tıkta yeşil onay <b>(O)</b> - "Kesinlikle O" işareti koyar.</span>
                            </li>
                            <li className="flex gap-3 text-emerald-400 font-bold">
                                <span>✨</span>
                                <span>Yeşil <b>(O)</b> koyduğunda o satır ve sütundaki diğer ihtimaller sistem tarafından otomatik olarak çarpı <b>(X)</b> yapılır.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg active:scale-95">ANLADIM, DETEKTİF!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12 flex flex-col items-center">
                ...
                <div className="flex justify-between items-center w-full mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        <button onClick={onExit} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all relative z-50">⬅ Geri Dön</button>
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Zebra Puzzle</h2>
                            <div className="text-xs md:text-sm font-bold text-slate-400 bg-slate-900/50 px-3 py-1 rounded-full mt-1 inline-block">SEVİYE: {n} Kategorili</div>
                        </div>
                    </div>
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            <span className="text-lg">⏱️</span>
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-cyan-300'}`}>
                                {Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}
                            </span>
                        </div>
                        <div className="flex gap-1">
                            {[...Array(maxMistakes)].map((_, i) => (
                                <div key={i} className={`w-3 h-3 md:w-4 md:h-4 rounded-full ${i < mistakes ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.8)]' : 'bg-slate-700'}`} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start w-full max-w-5xl justify-center">
                    ...
                    <div className="bg-slate-800 p-2 md:p-4 rounded-2xl shadow-2xl border border-slate-700 overflow-x-auto w-full lg:w-auto">
                        ...
                        <div className="flex flex-col w-max">
                            ...
                            <div className="flex">
                                ...
                                <div className="flex border-b-2 border-slate-500">
                                    {puzzle.categories[1].map((cat1, idx) => (
                                        <div key={`th-b-${idx}`} className={`${cellSize} h-24 flex items-end justify-center pb-2 border-l border-slate-700`}>
                                            <span className="origin-bottom-left -rotate-90 whitespace-nowrap translate-x-3">{cat1}</span>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex border-b-2 border-slate-500">
                                    {puzzle.categories[2].map((cat2, idx) => (
                                        <div key={`th-c-${idx}`} className={`${cellSize} h-24 flex items-end justify-center pb-2 border-l border-slate-700`}>
                                            <span className="origin-bottom-left -rotate-90 whitespace-nowrap translate-x-3 text-amber-200">{cat2}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex border-b-2 border-slate-500">
                                ...
                                <div className="flex flex-col border-r-2 border-slate-500">
                                    {puzzle.categories[0].map((cat0, idx) => (
                                        <div key={`lh-a-${idx}`} className={`${headerWidth} ${cellSize} flex items-center justify-end pr-2 text-cyan-200 border-b border-slate-700`}>
                                            {cat0}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-col border-r-2 border-slate-500 bg-slate-900/50">
                                    {grids.AB.map((row, r) => (
                                        <div key={`ab-r${r}`} className="flex">
                                            {row.map((cell, c) => (
                                                <button key={`ab-c${c}`} onClick={() => onCellClick('AB', r, c)}
                                ...
                                <div className="flex flex-col bg-slate-900/50">
                                    {grids.AC.map((row, r) => (
                                        <div key={`ac-r${r}`} className="flex">
                                            {row.map((cell, c) => (
                                                <button key={`ac-c${c}`} onClick={() => onCellClick('AC', r, c)}
                                                    className={`${cellSize} border-b border-l border-slate-700 hover:bg-slate-700/50 transition-colors`}>
                                                    {renderCell(cell)}
                                                </button>
                                            ))}
                            ...
                            <div className="flex">
                                ...
                                <div className="flex flex-col border-r-2 border-slate-500">
                                    {puzzle.categories[2].map((cat2, idx) => (
                                        <div key={`lh-c-${idx}`} className={`${headerWidth} ${cellSize} flex items-center justify-end pr-2 text-amber-200 border-b border-slate-700`}>
                                            {cat2}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-col bg-slate-900/50">
                                    {grids.CB.map((row, r) => (
                                        <div key={`cb-r${r}`} className="flex">
                                            {row.map((cell, c) => (
                                                <button key={`cb-c${c}`} onClick={() => onCellClick('CB', r, c)}
                                                    className={`${cellSize} border-b border-l border-slate-700 hover:bg-slate-700/50 transition-colors`}>
                                                    {renderCell(cell)}
                                                </button>
                                            ))}
                    ...
                    <div className="w-full lg:w-96 bg-slate-800 p-6 rounded-2xl shadow-2xl border border-slate-700">
                        <h3 className="text-xl font-black text-cyan-400 mb-4 flex items-center gap-2"><span className="text-2xl">🕵️</span> İpuçları</h3>
                        <div className="space-y-3">
                            {puzzle.clues.map((clue, idx) => (
                                <div key={idx} className="p-3 bg-slate-900/50 border border-white/5 rounded-xl text-slate-300 text-sm md:text-base leading-relaxed hover:bg-slate-700/50 transition-colors cursor-pointer"
                                    onClick={(e) => { e.currentTarget.classList.toggle('line-through'); e.currentTarget.classList.toggle('opacity-50'); }}>
                                    {clue}
                                </div>
                            ))}
                        </div>
                        <p className="mt-4 text-xs text-slate-500 text-center italic">Not: İpuçlarının üstünü çizmek için tıklayabilirsiniz.</p>
                    </div>
                </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Mantık Hatası!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Tebrikler Detektif!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Tabloda tamamen dolu ve çözülemez bir çelişki oluştu.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tüm ipuçlarını başarıyla analiz ettin! Kazanılan Yıldız: ${score}`}
                        </p>

                        <div className="flex flex-col gap-3">
                            {mistakes < maxMistakes && timeLeft > 0 && (
                                <button onClick={() => onComplete(score)} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 text-white shadow-lg shadow-green-500/25 active:scale-95">Ödülü Al ⭐️</button>
                            )}
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
                            <button onClick={onExit} className="w-full py-4 rounded-2xl font-black text-lg transition-all bg-slate-700 hover:bg-slate-600 text-white shadow-lg active:scale-95">Ana Üsse Dön</button>
                        </div>
                    </div>
                </div>
            )}
