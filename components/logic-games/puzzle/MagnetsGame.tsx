...
const MagnetsGame: React.FC<MagnetsGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const handleDominoClick = (domino: Domino) => {
        ...
        let newV1 = 0, newV2 = 0;
        if (cell1 === 0) { newV1 = 1; newV2 = -1; }
        else if (cell1 === 1) { newV1 = -1; newV2 = 1; }
        else if (cell1 === -1) { newV1 = 2; newV2 = 2; }
        else if (cell1 === 2) { newV1 = 0; newV2 = 0; }
        ...
        >
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="flex mb-2 ml-[88px]">
                            {boardData.topPlus.map((val, c) => (
                                <div key={`tcp-${c}`} className="flex flex-col items-center justify-end font-black" style={{ width: `${cellSize}px` }}>
                                    <span className="text-red-400 font-black text-xs md:text-sm">+{val}</span>
                                    <span className="text-blue-400 font-black text-xs md:text-sm">-{boardData.topMinus[c]}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex">
                            <div className="flex flex-col mr-2 justify-between py-2">
                                {boardData.leftPlus.map((val, r) => (
                                    <div key={`lcp-${r}`} className="flex items-center justify-end font-black gap-2 pr-2" style={{ height: `${cellSize}px` }}>
                                        <span className="text-red-400 font-black text-xs md:text-sm">+{val}</span>
                                        <span className="text-blue-400 font-black text-xs md:text-sm">-{boardData.leftMinus[r]}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            >
                                <div className="absolute inset-2 grid pointer-events-none opacity-20" style={{ gridTemplateColumns: `repeat(${C}, 1fr)` }}>
                                    {Array(R * C).fill(0).map((_, i) => (
                                ...
                                <div className="absolute inset-2">
                                    {boardData.dominoes.map((domino) => {
                                        ...
                                        const renderCellContent = (val: number) => {
                                            if (val === 1) return <span className="text-3xl text-red-500 drop-shadow-sm leading-none">+</span>;
                                            if (val === -1) return <span className="text-3xl text-blue-500 drop-shadow-sm leading-none">-</span>;
                                            if (val === 2) return <span className="text-xl text-slate-500 drop-shadow-sm opacity-50">X</span>;
                                            return null;
                                        };
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`absolute bg-slate-300 pointer-events-none
                                                    ${isHorizontal ? 'w-[2px] h-full left-1/2 -translate-x-1/2' : 'h-[2px] w-full top-1/2 -translate-y-1/2'}
                                                `} />

                                                <div className={`flex items-center justify-center font-black transition-colors duration-300
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Bloklara (Sol Tık) basarak kutupları değiştirin. Sağ tıklayarak hızla <span className="font-black text-slate-300">X</span> (boş) bırakabilirsiniz.</p>
                </div>
            </div>
        </LogicGameWrapper>
    );
