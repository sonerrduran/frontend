      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/10 rounded-3xl p-8 mb-8">
          <div className={`grid gap-4 mb-8 ${grid.length <= 20 ? 'grid-cols-5' : 'grid-cols-6'}`}>
            {grid.map((emoji, idx) => (
              <div key={idx} className="bg-gradient-to-br from-white/20 to-white/10 rounded-xl p-4 flex items-center justify-center h-20">
          ...
          {feedback && <div className={`mt-8 text-center text-3xl font-black ${feedback.includes('Mükemmel') || feedback.includes('Doğru') ? 'text-green-400' : 'text-red-400'}`}>{feedback}</div>}
        </div>
      </div>
    </div>
  );
