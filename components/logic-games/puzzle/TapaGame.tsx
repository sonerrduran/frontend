    ...
    const has2x2Black = (currentBoard: number[][]) => {
        for (let r = 0; r < size - 1; r++) {
            for (let c = 0; c < size - 1; c++) {
                if (currentBoard[r][c] === 1 && currentBoard[r + 1][c] === 1 &&
                    currentBoard[r][c + 1] === 1 && currentBoard[r + 1][c + 1] === 1) {
                    return true;
                }
            }
        }
    ...
    const isWallConnected = (currentBoard: number[][]) => {
        ...
        while (queue.length > 0) {
            ...
            for (const [dr, dc] of dirs) {
                const nr = curR + dr;
                const nc = curC + dc;
                if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited[nr][nc] && currentBoard[nr][nc] === 1) {
                    visited[nr][nc] = true;
                    queue.push([nr, nc]);
                }
            }
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>İpucu bulunan hücreler dışındaki hücreleri boya ve bitişik tek bir <b>duvar</b> oluştur.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-indigo-400">🎯</span>
                                <span>İpucu hücresindeki sayılar, o hücrenin etrafındaki (8 komşu) siyah blokların uzunluklarını verir. (Örn: 1-3 varsa, etrafta 1 hücrelik bir blok ve 3 hücrelik başka bir blok vardır, aralarında ise en az bir beyaz boşluk olmalıdır).</span>
                            </li>
                            <li className="flex gap-3 text-red-400 font-bold">
                                <span>🚫</span>
                                <span>Siyah duvar hiçbir yerde <b>2x2'lik bir kare</b> (blok) oluşturamaz.</span>
                            </li>
                        </ul>
            ...
            <div className="w-full max-w-2xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">Tapa</h2>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center font-black transition-all duration-150
                                            ${hint ? 'bg-slate-800 text-white cursor-not-allowed text-xs md:text-sm shadow-[inset_0_2px_5px_rgba(0,0,0,0.5)]' :
                                                cell === 1 ? 'bg-slate-900 shadow-[inset_0_3px_5px_rgba(0,0,0,0.8)]' :
                                                    cell === 2 ? 'bg-slate-300' :
                                    ...
                                    >
                                        {hint ? (
                                            <div className="flex flex-col items-center justify-center leading-tight">
                                                {hint.vals.map((v, i) => <span key={i}>{v}</span>)}
                                            </div>
                                        ) : (
                                            cell === 2 ? <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-slate-500"></div> : null
                                        )}
                                    </button>
                                );
                            })
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-md mx-auto">
                    <p className="text-slate-400 text-sm font-bold">1 Tık: <span className="bg-slate-900 px-2 text-white border border-slate-700">Duvar (Siyah)</span> | 2 Tık: <span className="bg-slate-300 px-2 text-slate-800 rounded-sm">Nokta (.)</span> | 3 Tık: Boş</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Yanlışlıkla 2x2 boyutunda bir siyah blok oluşturdun.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tapa duvarını mükemmel ördün! Kazanılan Yıldız: ${score}`}
                        </p>
