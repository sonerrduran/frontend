...
const ChessGame: React.FC<Props> = ({ onExit }) => {
    ...
    const updateCapturedPieces = () => {
        ...
        history.forEach(move => {
            if (move.captured) {
                const piece = move.color === 'w' ? move.captured.toLowerCase() : move.captured.toUpperCase();
                if (move.color === 'w') {
    ...
    const onDrop = (data: { sourceSquare: string, targetSquare: string | null, piece: { pieceType: string } }) => {
            ...
            <div className="relative max-w-7xl mx-auto">
                ...
                <div className="grid lg:grid-cols-[1fr_600px_1fr] gap-6 items-start">
                    ...
                    <div className="space-y-4">
                        <div className={`bg-gradient-to-br from-slate-800 to-slate-900 backdrop-blur-xl p-6 rounded-3xl border-2 transition-all ${
                            game.turn() === 'b' ? 'border-purple-500 shadow-[0_0_30px_rgba(168,85,247,0.4)] scale-105' : 'border-slate-700/50'
                        }`}>
                            ...
                            <div className="bg-slate-900/50 rounded-xl p-3">
                                ...
                                <div className="flex flex-wrap gap-1 min-h-[32px]">
                                    {capturedPieces.white.map((piece, i) => (
                                        <span key={i} className="text-2xl opacity-70">{piece}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-20">
                                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-3xl border-2 border-purple-500 shadow-[0_0_50px_rgba(168,85,247,0.6)] text-center max-w-[90%] transform animate-in zoom-in-95 duration-500">
                                            <div className="text-7xl mb-4 animate-bounce">
                                                {outcome.includes('Mat') ? '🏆' : outcome.includes('Berabere') ? '🤝' : '⚠️'}
                                            </div>
                    ...
                    <div className="space-y-4">
                        <div className={`bg-gradient-to-br from-slate-800 to-slate-900 backdrop-blur-xl p-6 rounded-3xl border-2 transition-all ${
                            game.turn() === 'w' ? 'border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.4)] scale-105' : 'border-slate-700/50'
                        }`}>
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-12 h-12 bg-slate-700 rounded-full flex items-center justify-center text-2xl">
                                    ♔
