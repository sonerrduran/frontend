...
const HidatoGame: React.FC<HidatoGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const handleNumberInput = (num: number) => {
        ...
        let maxVal = 0;
        boardData.initialBoard.forEach(row => row.forEach(cell => {
            if (cell !== -1) maxVal++;
        }));
                            ...
                            <div className="bg-amber-900/50 p-3 rounded-3xl shadow-2xl border-4 border-amber-800 backdrop-blur-sm">
                                <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))` }}>
                                    {board.map((row, r) =>
                                        row.map((cell, c) => {
                                            if (cell === -1) {
                                                return <div key={`${r}-${c}`} className="w-12 h-12 md:w-16 md:h-16 opacity-0" />;
                                            }
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-12 h-12 md:w-16 md:h-16 text-xl md:text-2xl font-black rounded-xl transition-all duration-200 flex items-center justify-center border-b-4 relative overflow-hidden
                                                        ${fixed ? 'bg-amber-100 text-amber-900 border-amber-300 shadow-[inset_0_4px_8px_rgba(0,0,0,0.1)] cursor-not-allowed' :
                                                            selected ? 'bg-rose-400 text-white border-rose-600 shadow-[0_0_20px_rgba(251,113,133,0.8)] scale-110 z-10' :
                                                                cell !== 0 ? 'bg-amber-500 text-white border-amber-700 shadow-md hover:bg-amber-400 hover:border-amber-600 hover:-translate-y-1' :
                                                ...
                                                >
                                                    ...
                                                    {!fixed && !selected && cell === 0 && (
                                                        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 50% 50%, white 2px, transparent 2px)', backgroundSize: '10px 10px' }} />
                                                    )}
                                                </button>
                                            );
                                        })
