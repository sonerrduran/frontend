...
const MosaicPuzzleGame: React.FC<MosaicPuzzleGameProps> = ({ onBack }) => {
  ...
  const generatePuzzle = () => {
    const size = 10;
    const newSolution: ('filled' | 'empty')[][] = Array(size).fill(0).map(() => Array(size).fill('empty'));
    ...
    const newClues: number[][] = Array(size).fill(0).map(() => Array(size).fill(0));
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        let count = 0;
    ...
    const newGrid: ('filled' | 'empty' | '')[][] = Array(size).fill(0).map(() => Array(size).fill(''));
