...
const SamuraiSudokuGame: React.FC<Props> = ({ difficulty, onComplete, onExit }) => {
    ...
    const getStars = () => {
        switch (difficulty) {
            ...
            case Difficulty.HARD: return 250;
            case Difficulty.VERY_HARD: return 400;
            default: return 120;
        }
    };
    ...
    return (
        <div className="flex flex-col items-center w-full min-h-screen">
            <div className="flex justify-between items-center w-full max-w-5xl mb-4 px-4 relative">
                <button onClick={onExit} className="px-6 py-2 bg-white/10 text-white rounded-full font-bold hover:bg-white/20 transition-all">
                    ⬅ Çıkış
                </button>
                <h2 className="text-xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-700 uppercase tracking-wider">Samurai Sudoku</h2>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-4 md:p-6 rounded-[40px] shadow-2xl border border-white/10 flex flex-col items-center w-full max-w-5xl overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-lg mb-8">
                            <li className="flex items-start gap-3">
                                ...
                                <span>Samurai Sudoku, kesişen tam 5 ayrı Sudoku (9x9) tahtasından oluşur. (Köşelerde 4 ve merkezde 1 adet)</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">1️⃣</span>
                                <span>Tüm 5 tahta da aynı anda normal Sudoku kurallarına (Satır, Sütun ve 3x3 kutularda 1-9 birer kez) uymak zorundadır.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">🧩</span>
                                <span>Orta alanın köşelerindeki 3x3'lük kutular her iki kesişen tahtaya da aittir ve her iki tarafın kurallarına birlikte uymalıdır.</span>
                            </li>
                        </ul>
                ...
                <div className="mb-4 flex justify-between items-center w-full opacity-90">
                    ...
                    {isGenerating && <span className="text-yellow-400 font-bold animate-pulse">Savaş Alanı Kuruluyor...</span>}
                ...
                <div className="w-full flex-1 overflow-auto custom-scrollbar p-2 flex justify-center items-center">
                    <div className="grid grid-cols-21 gap-[1px] bg-slate-800 p-[2px] rounded-lg min-w-[600px] md:min-w-[800px] select-none"
                        style={{ gridTemplateColumns: 'repeat(21, minmax(0, 1fr))' }}>
                        {!isGenerating && board.map((row, r) => (
                            row.map((cell, c) => {
                                if (!cell.isActive) {
                                    return <div key={`${r}-${c}`} className="w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 opacity-0"></div>;
                                }
                                ...
                                if (selectedCell) {
                                    ...
                                    if (sharedGrids.length > 0) {
                                        ...
                                        const bR2 = Math.floor(selectedCell.r / 3); const bC2 = Math.floor(selectedCell.c / 3);
                      ...
                      ${borders}
                      ${cell.hasError ? 'bg-red-500 text-white animate-pulse' :
                                                isSelected ? 'bg-red-600 text-white z-10 scale-110 shadow-lg' :
                                                    isSameNumber ? 'bg-red-400 text-slate-900' :
                                                        isRelated ? 'bg-white/20' :
                                                            (cell.gridId.length > 1 ? 'bg-white/10' : 'bg-slate-700')}
                ...
                <div className="mt-8 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-lg">
                    ...
                    <button onClick={() => handleNumberInput(null)} className="bg-red-500/20 hover:bg-red-500/40 text-red-200 font-bold text-sm rounded-xl py-3 md:py-4 transition-all flex items-center justify-center">
                        SİL
                    </button>
                </div>
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/70 backdrop-blur-sm rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-red-600 to-red-900 p-8 rounded-[32px] text-center shadow-[0_0_50px_rgba(239,68,68,0.5)] scale-110 border-4 border-white">
                            ...
                            <h3 className="text-4xl font-black text-white mb-2 drop-shadow-lg">Gerçek Bir Samuray!</h3>
