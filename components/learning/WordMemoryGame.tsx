...
const WORD_LIST = ["Gezegen", "Yıldız", "Galaksi", "Teleskop", "Astronot", "Roket", "Uydu", "Krater", "Kuyruklu Yıldız", "Işık Yılı", "Mars", "Jüpiter", "Kara Delik", "Nebula", "Meteor"];
...
const WordMemoryGame: React.FC = () => {
    ...
    const startLevel = () => {
        ...
        const distractors = WORD_LIST.filter(w => !selected.includes(w)).sort(() => 0.5 - Math.random()).slice(0, 5);
            ...
            <div className="w-full max-w-2xl bg-slate-900/80 backdrop-blur-3xl rounded-[40px] p-8 md:p-12 border border-white/10 shadow-2xl relative overflow-hidden text-center">
                ...
                {step === 'intro' && (
                    <div className="animate-in zoom-in duration-500">
                        ...
                        <p className="text-white/60 mb-10">Ekranda sırayla belirecek kelimeleri aklında tut ve sonra aynı sırayla seç!</p>
                        <button
                            onClick={startLevel}
                ...
                {step === 'memorizing' && (
                    <div className="animate-in fade-in duration-300">
                        <p className="text-xs font-black opacity-40 mb-10 uppercase tracking-widest">KELİMELERİ AKLINDA TUT</p>
                        <div className="flex flex-wrap justify-center gap-4">
                            {currentWords.map((word, i) => (
                                <div key={i} className="bg-indigo-600 px-8 py-4 rounded-2xl font-black text-2xl shadow-xl animate-bounce" style={{ animationDelay: `${i * 0.2}s` }}>
                                    {word}
                                </div>
                            ))}
                        </div>
                ...
                {step === 'guessing' && (
                    <div className="animate-in slide-in-from-bottom-10 duration-500">
                        <p className="text-xs font-black opacity-40 mb-6 uppercase tracking-widest">SIRAYLA SEÇ ({userSelection.length} / {currentWords.length})</p>

                        <div className="bg-white/5 p-6 rounded-2xl mb-8 flex flex-wrap justify-center gap-2 min-h-[60px]">
                            {userSelection.map((word, i) => (
                                <span key={i} className="bg-cyan-500/20 text-cyan-400 px-4 py-2 rounded-lg font-bold text-sm">
                                    {word}
                                </span>
                            ))}
                        </div>
                ...
                {step === 'result' && (
                    <div className="animate-in zoom-in duration-500">
                        ...
                        <h2 className="text-4xl font-black mb-2 uppercase">{isCorrect ? 'MÜKEMMEL!' : 'NEREDEYSE...'}</h2>
                        <p className="text-white/60 mb-10">
                            {isCorrect ? `Seviye ${level - 1} tamamlandı! Zorluk artıyor.` : 'Sıralamada bir hata yaptın. Tekrar deneyelim mi?'}
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            ...
                            <button
                                onClick={() => { setLevel(1); setStep('intro'); }}
