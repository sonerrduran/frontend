...
const SAMPLE_NOTES: Note[] = [
    { id: 1, title: '📅 Bu Hafta Sınav', body: '5 Mart Çarşamba günü Matematik sınavı var. Kesirler ve ondalık sayılar konularını tekrar edin.', color: 'pink', pinned: true, createdAt: '5 Mar' },
    { id: 2, title: '🏃 Spor Günü', body: '7 Mart Cuma günü okul spor günü. Spor kıyafeti getirilmesi zorunludur.', color: 'blue', pinned: false, createdAt: '5 Mar' },
    { id: 3, title: '📚 Ödev Hatırlatması', body: 'Haftalık okuma ödevi Cuma gününe kadar teslim edilecek.', color: 'yellow', pinned: false, createdAt: '4 Mar' },
];
                ...
                <div className="bg-slate-900/80 backdrop-blur-xl p-6 md:p-8 rounded-[40px] shadow-2xl border border-white/10">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-white/40 text-sm">{notes.length} duyuru · {notes.filter(n => n.pinned).length} sabit</p>
                        </div>
                    ...
                    {showForm && (
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
                            ...
                            <textarea value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))}
                                placeholder="Duyuru içeriği..." rows={3}
                            ...
                            <button onClick={addNote} className="w-full py-3 bg-amber-500 hover:bg-amber-400 text-white rounded-xl font-black text-sm transition-all">
                                📌 Duyuruyu Ekle
                            </button>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {sorted.map(note => {
                            ...
                            return (
                                <div key={note.id} className={`${style.bg} ${style.border} border rounded-2xl p-5 group relative transition-all hover:scale-[1.02]`}>
                                    {note.pinned && <div className="absolute -top-2 -right-2 text-lg rotate-12">📌</div>}
                                    <div className="flex items-start justify-between mb-2">
                                        <h3 className={`font-black text-sm ${style.title} flex-1 pr-2`}>{note.title}</h3>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                            ...
                                            <button onClick={() => deleteNote(note.id)}
                                                className="w-7 h-7 rounded-lg bg-white/5 text-white/40 hover:text-red-400 text-xs flex items-center justify-center transition-all">✕</button>
                                        </div>
                                    </div>
