...
const BOARDS: Record<number, BoardData> = {
    4: {
        size: 4,
        clues: [
            { r: 0, c: 0, val: 3 }, { r: 0, c: 3, val: 2 },
            { r: 1, c: 1, val: 1 },
            { r: 2, c: 2, val: 2 },
            { r: 3, c: 0, val: 4 }, { r: 3, c: 3, val: 4 }
        ]
    },
    5: {
        size: 5,
        clues: [
            { r: 0, c: 1, val: 2 }, { r: 0, c: 3, val: 4 },
            { r: 1, c: 0, val: 2 }, { r: 1, c: 4, val: 4 },
            { r: 3, c: 0, val: 3 }, { r: 3, c: 4, val: 1 },
            { r: 4, c: 1, val: 3 }, { r: 4, c: 3, val: 5 }
        ]
    },
    6: {
        size: 6,
        clues: [
            { r: 0, c: 0, val: 5 }, { r: 0, c: 5, val: 3 },
            { r: 1, c: 2, val: 2 }, { r: 1, c: 3, val: 4 },
            { r: 4, c: 2, val: 4 }, { r: 4, c: 3, val: 1 },
            { r: 5, c: 0, val: 6 }, { r: 5, c: 5, val: 4 }
        ]
    }
};
...
const FillominoGame: React.FC<FillominoGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            case Difficulty.VERY_EASY: return 4;
                ...
                if (!visited[r][c] && currentBoard[r][c] !== 0) {
                    ...
                    while (stack.length > 0) {
                        ...
                        for (const [dr, dc] of directions) {
                            ...
                            if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited[nr][nc] && currentBoard[nr][nc] === val) {
                                visited[nr][nc] = true;
                                stack.push([nr, nc]);
                            }
                        }
    ...
    const checkBoardState = (currentBoard: number[][], changedR: number, changedC: number) => {
        ...
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
                const region1 = regions.find(reg => reg.cells.some(cell => cell.r === r && cell.c === c));

                const directions = [[1, 0], [0, 1]]; // Only check right/down to avoid double count
                for (const [dr, dc] of directions) {
                    const nr = r + dr, nc = c + dc;
                    if (nr >= 0 && nr < size && nc >= 0 && nc < size) {
                        const region2 = regions.find(reg => reg.cells.some(cell => cell.r === nr && cell.c === nc));
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Tüm ızgarayı sayılarla doldurarak <b>bölgeler</b> (polyominos) oluştur.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-cyan-400">📏</span>
                                <span>Hücrenin içindeki sayı, o hücrenin dâhil olduğu bölgede toplam kaç tane hücre olduğunu gösterir (Örn: 4 yazan bir hücre, tam 4 hücrelik bir bölgenin içindedir).</span>
                            </li>
                            <li className="flex gap-3 text-rose-400 font-bold">
                                <span>🚫</span>
                                <span>Aynı büyüklüğe sahip iki farklı bölge, birbirine yanlardan temas edemez (çapraz temas serbesttir).</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-2xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-500">Fillomino</h2>
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-emerald-300'}`}>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center text-2xl md:text-3xl font-black transition-all transform active:scale-90
                                            ${borders}
                                            ${isError ? 'bg-rose-500 text-white animate-pulse shadow-[inset_0_0_15px_rgba(0,0,0,0.5)]' :
                                                isClue ? 'bg-slate-700 text-emerald-300 cursor-not-allowed shadow-[inset_0_2px_5px_rgba(0,0,0,0.5)] border-slate-500/30' :
                                                    isSelected ? 'bg-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.8)] z-10 scale-105 border-emerald-300' :
                                                        cell !== 0 ? 'bg-slate-700/50 hover:bg-slate-600 text-white shadow-sm' :
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Bölge Sınırı Aşıldı!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Alan taşması veya aynı boyuttaki komşu bölgeler çakıştı.' : timeLeft <= 0 ? 'Zamanı iyi yönetemedin.' : `Tüm alanları kurallara uygun tasarladın! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
