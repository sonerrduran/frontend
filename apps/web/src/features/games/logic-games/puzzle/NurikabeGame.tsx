    ...
    const isSeaConnected = (currentBoard: CellState[][]) => {
        ...
        while (stack.length > 0) {
            ...
            for (const [dr, dc] of directions) {
                ...
                if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited[nr][nc] && currentBoard[nr][nc] === 'sea') {
                    visited[nr][nc] = true;
                    connectedCount++;
                    stack.push([nr, nc]);
                }
    ...
    const has2x2Sea = (currentBoard: CellState[][]) => {
        for (let r = 0; r < size - 1; r++) {
            for (let c = 0; c < size - 1; c++) {
                if (currentBoard[r][c] === 'sea' && currentBoard[r + 1][c] === 'sea' &&
                    currentBoard[r][c + 1] === 'sea' && currentBoard[r + 1][c + 1] === 'sea') {
                    return true;
                }
            }
        }
                ...
                <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
                    ...
                    <div className="bg-gradient-to-br from-cyan-500 via-teal-500 to-cyan-600 rounded-2xl p-8 md:p-12 mb-8">
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded-xl font-black text-2xl md:text-3xl transition-all duration-300 transform active:scale-90
                                                    ${clue ? 'bg-white text-slate-900 cursor-not-allowed border border-slate-200' :
                                                        cellState === 'sea' ? 'bg-slate-900 border-2 border-slate-800' :
                                                            cellState === 'dot' ? 'bg-white/10 hover:bg-white/20 border-2 border-dashed border-white/20 text-white' :
                        ...
                        <div className="text-center bg-white/10 p-4 rounded-2xl mb-4">
                            <p className="text-white text-sm font-bold">1 Tık: <span className="text-slate-900 bg-slate-400 px-2 rounded-sm mx-1">Deniz</span> | 2 Tık: <span className="text-white bg-slate-600 px-2 py-0.5 rounded-sm mx-1">Ada Noktası</span> | 3 Tık: Boş</p>
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
                    <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-cyan-500/30 max-w-md w-full">
                        ...
                        <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
                            <li className="flex gap-2"><span className="text-cyan-400 font-bold">1.</span> Sayılı hücreler adaları temsil eder. Her ada o sayı kadar hücre içermelidir.</li>
                            <li className="flex gap-2"><span className="text-cyan-400 font-bold">2.</span> Deniz (siyah hücreler) tüm tahtayı bağlı bir şekilde kaplamalıdır.</li>
                            <li className="flex gap-2"><span className="text-cyan-400 font-bold">3.</span> 2x2 deniz havuzu oluşturulamaz!</li>
                            <li className="flex gap-2"><span className="text-cyan-400 font-bold">4.</span> Her adada sadece bir sayı olmalıdır.</li>
                        </ul>
                        <button
