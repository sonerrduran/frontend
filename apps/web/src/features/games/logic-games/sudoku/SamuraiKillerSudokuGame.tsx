...
const SamuraiKillerSudokuGame: React.FC<Props> = ({ difficulty, onComplete, onExit }) => {
    ...
    const cageColors = [
        'bg-red-900/40', 'bg-blue-900/40', 'bg-green-900/40',
        'bg-yellow-900/40', 'bg-purple-900/40', 'bg-pink-900/40',
        'bg-orange-900/40', 'bg-teal-900/40', 'bg-emerald-900/40'
    ];
    ...
    const generatePuzzle = useCallback(() => {
        ...
        setTimeout(() => {
            let rawBoard: number[][] = Array(21).fill(0).map(() => Array(21).fill(0));
            let success = false;

            while (!success) {
                rawBoard = Array(21).fill(0).map(() => Array(21).fill(0));
                ...
                success = solveGrid(6, 6) && solveGrid(0, 0) && solveGrid(0, 12) && solveGrid(12, 0) && solveGrid(12, 12);
            }

            // Group cells into Cages
            const cageMap: number[][] = Array(21).fill(0).map(() => Array(21).fill(-1));
            ...
            const isActiveCell = (r: number, c: number) => {
                ...
                return grids.some(g => r >= g.rOffset && r < g.rOffset + 9 && c >= g.cOffset && c < g.cOffset + 9);
            };
            ...
            const newBoard: Cell[][] = Array(21).fill(0).map((_, r) =>
                Array(21).fill(0).map((_, c) => {
                    let isActive = false;
        ...
        grids.forEach(g => {
            ...
            for (let r = 0; r < 9; r++) {
                ...
                for (let c = 0; c < 9; c++) {
                    ...
                    if (cell.value === null) { allFilled = false; continue; }
                    if (rowMap[cell.value]) { cell.hasError = true; rowMap[cell.value].hasError = true; anyError = true; }
            ...
            for (let c = 0; c < 9; c++) {
                ...
                for (let r = 0; r < 9; r++) {
                    ...
                    if (colMap[cell.value]) { cell.hasError = true; colMap[cell.value].hasError = true; anyError = true; }
                ...
                for (let bc = 0; bc < 3; bc++) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        for (let j = 0; j < 3; j++) {
                            ...
                            if (cell.value === null) continue;
                            if (boxMap[cell.value]) { cell.hasError = true; boxMap[cell.value].hasError = true; anyError = true; }
        ...
        Object.values(cageGroups).forEach(cells => {
            ...
            cells.forEach(cell => {
                ...
                if (cell.value === null) { isFull = false; }
                    ...
                    sum += cell.value;
                    if (vals[cell.value]) { cell.hasError = true; vals[cell.value].hasError = true; anyError = true; }
    ...
    const getBorders = (r: number, c: number) => {
        ...
        if (u && u.isActive && u.cageId !== cell.cageId) borders += 'border-t-2 border-dashed border-t-white/50 ';
        if (d && d.isActive && d.cageId !== cell.cageId) borders += 'border-b-2 border-dashed border-b-white/50 ';
        if (l && l.isActive && l.cageId !== cell.cageId) borders += 'border-l-2 border-dashed border-l-white/50 ';
        if (rgt && rgt.isActive && rgt.cageId !== cell.cageId) borders += 'border-r-2 border-dashed border-r-white/50 ';
    ...
    const getStars = () => {
        switch (difficulty) {
            ...
            case Difficulty.MEDIUM: return 500;
            case Difficulty.HARD: return 800;
            case Difficulty.VERY_HARD: return 1200;
            default: return 400;
        }
    };
    ...
    return (
        <div className="flex flex-col items-center w-full min-h-screen pb-10">
            <div className="flex justify-between items-center w-full max-w-6xl mb-4 px-4 relative mt-2">
                <button onClick={onExit} className="px-5 py-2 bg-white/10 text-white rounded-full font-bold hover:bg-white/20 transition-all flex items-center gap-2">
                    <span>⬅</span> Çıkış
                </button>
                <div className="absolute left-1/2 -translate-x-1/2 text-center w-max">
                    <h2 className="text-xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-600 drop-shadow-lg uppercase tracking-wider">Samurai Killer</h2>
                </div>
                <button onClick={() => setShowRules(true)} className="p-3 bg-red-500/20 text-red-300 rounded-full hover:bg-red-500/30 transition-all">
                    ℹ️ Kurallar
                </button>
            </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-2 md:p-6 rounded-[20px] md:rounded-[40px] shadow-2xl border border-rose-500/20 flex flex-col items-center w-full max-w-6xl overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-lg mb-8 text-sm md:text-base">
                            <li className="flex items-start gap-3">
                                ...
                                <span>Bu bir <b>Samuray Sudoku</b> (Kesişen 5 tahta) ile <b>Killer Sudoku</b>'nun birleşimidir! Olası en zor mantık oyunlarından biridir.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl md:text-2xl">🧩</span>
                                <span>Izgara üzerinde kesikli çizgilerle (ve renklerle) ayrılmış bloklara "Kafes" (Cage) denir.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl md:text-2xl">1️⃣</span>
                                <span>Bir kafesin içerisindeki rakamların <b>TOPLAMI</b>, kafesin sol üst köş köşesinde yazan ufak sayıyı vermelidir!</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl md:text-2xl">❌</span>
                                <span>Bir kafes içerisinde aynı rakam hiçbir zaman tekrar edemez! Normal 9x9 Sudoku satır/sütun/kutu kuralları da tüm tahtalarda aynen geçerlidir.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="bg-gradient-to-r from-red-600 to-rose-800 text-white font-black py-4 px-12 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all text-xl">
                            BU ZORLUĞA HAZIRIM!
                        </button>
                    </div>
                )}
                ...
                <div className="mb-2 flex justify-between items-center w-full opacity-90 px-4">
                    <span className="text-white font-bold bg-white/10 px-4 py-2 rounded-xl text-xs md:text-base">Ödül: {getStars()} ⭐</span>
                    {isGenerating && <span className="text-rose-400 font-bold animate-pulse text-xs md:text-base">Kafesler Örülüyor...</span>}
                    <button onClick={generatePuzzle} disabled={isGenerating} className="text-white hover:text-rose-400 font-bold transition-colors disabled:opacity-50 text-xs md:text-base">Yeniden Kur 🔄</button>
                </div>

                <div className="w-full flex-1 overflow-auto custom-scrollbar p-2 flex justify-center items-center">
                    <div className="grid grid-cols-21 gap-[1px] bg-slate-800 p-[2px] rounded-lg min-w-[750px] md:min-w-[900px] select-none"
                        style={{ gridTemplateColumns: 'repeat(21, minmax(0, 1fr))' }}>
                        {!isGenerating && board.map((row, r) => (
                            row.map((cell, c) => {
                                if (!cell.isActive) {
                                    return <div key={`${r}-${c}`} className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 opacity-0"></div>;
                                }

                                const isSelected = selectedCell?.r === r && selectedCell?.c === c;
                                const isSameNumber = selectedCell && board[selectedCell.r][selectedCell.c].value === cell.value && cell.value !== null;
                                const isSameCage = selectedCell && board[selectedCell.r][selectedCell.c].cageId === cell.cageId;
                                ...
                                return (
                                  ...
                                  ${borders}
                                  ${cageBg}
                                  ${cell.hasError ? 'bg-red-600/80 text-white animate-pulse' :
                                                isSelected ? 'bg-rose-500 text-white z-20 scale-110 shadow-lg ring-2 ring-white rounded-md' :
                                                    isSameCage ? 'brightness-150 saturate-150' :
                                                        isSameNumber ? 'bg-rose-400/80 text-white' : ''}
                ...
                <div className="mt-6 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-2xl px-2">
                    ...
                    <button onClick={() => handleNumberInput(null)} className="bg-red-500/20 hover:bg-red-500/40 text-red-100 font-bold text-sm rounded-xl py-3 transition-all flex items-center justify-center">
                        SİL
                    </button>
                </div>
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/80 backdrop-blur-md rounded-[20px] md:rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-rose-600 to-red-900 p-8 rounded-[32px] text-center shadow-[0_0_80px_rgba(225,29,72,0.6)] scale-110 border-4 border-white">
                            ...
                            <p className="text-white/80 font-bold text-lg mb-6 max-w-xs mx-auto">Samurai Killer Sudoku'yu çözdün. Sen bir mantık ustasısın!</p>
                            <button
