...
const ConnectFourGame: React.FC<Props> = ({ onExit }) => {
    ...
    const [board, setBoard] = useState<Player[][]>(Array(COLS).fill([]).map(() => []));
    ...
    const checkWinner = (currentBoard: Player[][], lastCol: number, lastRow: number, player: Player) => {
        if (!player) return;
        ...
        for (const axis of directions) {
            ...
            if (count >= 4) {
                ...
                setScores(prev => ({ ...prev, [player as string]: prev[player as keyof typeof prev] + 1 }));
                return;
            }
        }
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4 md:p-6 bounce-in">
            <div className="flex items-center justify-between mb-8">
                ...
                <h2 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-yellow-400 italic drop-shadow-lg">Hedef 4</h2>
            </div>
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[40px] shadow-2xl flex flex-col items-center">
                ...
                <div className="flex gap-8 mb-6 w-full justify-center">
                    <div className={`flex flex-col items-center p-4 rounded-2xl transition-all min-w-[120px] ${redIsNext && !winner ? 'bg-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.3)] scale-110' : 'bg-white/5 opacity-60'}`}>
                    ...
                    <div className={`flex flex-col items-center p-4 rounded-2xl transition-all min-w-[120px] ${!redIsNext && !winner ? 'bg-yellow-500/20 shadow-[0_0_20px_rgba(234,179,8,0.3)] scale-110' : 'bg-white/5 opacity-60'}`}>
                ...
                <div className="h-10 mb-4 flex items-center justify-center">
                    {winner ? (
                        <div className={`text-2xl md:text-3xl font-black animate-bounce ${winner === 'RED' ? 'text-red-400' : winner === 'YELLOW' ? 'text-yellow-400' : 'text-gray-400'}`}>
                            {winner === 'RED' ? '🔴 KIRMIZI KAZANDI!' : winner === 'YELLOW' ? '🟡 SARI KAZANDI!' : '🤝 BERABERE!'}
                        </div>
                    ) : (
                        <div className="text-lg text-white/80 font-bold flex items-center gap-2">
                            Sıra: <div className={`w-6 h-6 rounded-full shadow-inner ${redIsNext ? 'bg-red-500 border border-red-300' : 'bg-yellow-400 border border-yellow-200'}`}></div>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-blue-600 p-2 md:p-4 rounded-2xl shadow-[0_10px_30px_rgba(37,99,235,0.5)] border-4 border-blue-500 relative">
                    ...
                    {!winner && (
                        <div className="flex gap-1 md:gap-2 mb-2 px-1">
                            {Array.from({ length: COLS }).map((_, colIndex) => (
                                <div key={`hover-${colIndex}`} className="w-10 h-10 md:w-16 md:h-16 flex justify-center items-end">
                                    {hoverCol === colIndex && board[colIndex].length < ROWS && (
                                        <div className={`w-8 h-8 md:w-14 md:h-14 rounded-full shadow-lg ${redIsNext ? 'bg-red-500/80 saturate-150' : 'bg-yellow-400/80 saturate-150'} translate-y-2 animate-pulse`}></div>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex gap-1 md:gap-2 bg-blue-700/50 p-1 md:p-2 rounded-xl">
                        {board.map((col, colIndex) => (
                            ...
                            >
                                {Array.from({ length: ROWS }).map((_, rowIndex) => {
                                    ...
                                    const isWinningCell = winningCells.some(w => w.c === colIndex && w.r === rowIndex);
                      ...
                      `}
                                        >
                                            {/* Slot Content */}
                                            <div className={`w-[85%] h-[85%] rounded-full transition-transform duration-500 ease-bounce 
