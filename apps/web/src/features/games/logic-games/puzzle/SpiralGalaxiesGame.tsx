...
const SpiralGalaxiesGame: React.FC<SpiralGalaxiesProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        const initialH = Array(size + 1).fill(false).map(() => Array(size).fill(false));
        const initialV = Array(size).fill(false).map(() => Array(size + 1).fill(false));
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            ...
                            <li className="flex gap-3 text-purple-400 font-bold">
                                <span>📏</span>
                                <div>Izgarayı öyle bloklara bölmelisin ki; her galaksinin şekli, kendi çekirdeğine göre **180 derece dönüş simetrisine (noktasal simetriye)** sahip olsun.</div>
                            </li>
                            <li className="flex gap-3">
                                <span>🖱️</span>
                                <div>Hücre aralarındaki kesik çizgilere tıklayarak duvar ör/kaldır. Her kapalı bölgenin içinde tam 1 çekirdek olmalı.</div>
                            </li>
                        </ul>
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-fuchsia-500 tracking-wider">Galaxies</h2>
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Hücre aralarına tıklayarak simetrik galaksilerin duvarlarını örün.</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Evren Çöktü!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Kozmik Uyum!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Çok fazla hata yaptın.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tüm galaksileri simetrik olarak böldün! Kazanılan Yıldız: ${score}`}
                        </p>
