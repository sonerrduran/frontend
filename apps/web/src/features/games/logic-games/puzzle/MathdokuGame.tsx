...
const MathdokuGame: React.FC<MathdokuGameProps> = ({ onBack }) => {
  ...
  const [cages, setCages] = useState<{ cells: [number, number][]; target: number; operation: string }[]>([]);
  ...
  const generatePuzzle = () => {
    ...
    const newCages = [
      { cells: [[0, 0], [0, 1]], target: 11, operation: '+' },
      { cells: [[0, 2], [1, 2]], target: 2, operation: '÷' },
      { cells: [[0, 3], [0, 4], [0, 5]], target: 20, operation: '×' },
      { cells: [[1, 0], [2, 0]], target: 2, operation: '-' },
      { cells: [[1, 1], [2, 1]], target: 3, operation: '÷' },
      { cells: [[1, 3], [1, 4]], target: 3, operation: '-' },
      { cells: [[1, 5], [2, 5]], target: 3, operation: '÷' },
      { cells: [[2, 2], [2, 3], [2, 4]], target: 6, operation: '+' },
      { cells: [[3, 0], [4, 0]], target: 3, operation: '-' },
      { cells: [[3, 1], [3, 2]], target: 2, operation: '÷' },
      { cells: [[3, 3], [4, 3]], target: 1, operation: '-' },
      { cells: [[3, 4], [3, 5]], target: 11, operation: '+' },
      { cells: [[4, 1], [4, 2]], target: 5, operation: '+' },
      { cells: [[4, 4], [4, 5]], target: 4, operation: '×' },
      { cells: [[5, 0], [5, 1]], target: 10, operation: '+' },
      { cells: [[5, 2], [5, 3]], target: 4, operation: '×' },
      { cells: [[5, 4], [5, 5]], target: 7, operation: '+' }
    ];
  ...
  const getCageColor = (cageIndex: number) => {
    const colors = [
      'bg-red-200/50', 'bg-blue-200/50', 'bg-green-200/50', 'bg-yellow-200/50',
      'bg-purple-200/50', 'bg-pink-200/50', 'bg-indigo-200/50', 'bg-orange-200/50',
      'bg-teal-200/50', 'bg-cyan-200/50', 'bg-lime-200/50', 'bg-amber-200/50',
      'bg-rose-200/50', 'bg-violet-200/50', 'bg-fuchsia-200/50', 'bg-emerald-200/50',
      'bg-sky-200/50'
    ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-red-900 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <button onClick={onBack} className="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl font-bold transition-all">
            ⬅ GERİ DÖN
          </button>
