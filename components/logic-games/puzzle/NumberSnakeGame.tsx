...
const NumberSnakeGame: React.FC<Props> = ({ difficulty, grade, onComplete, onExit }) => {
    ...
    const [grid, setGrid] = useState<(number | null)[][]>(Array(5).fill(null).map(() => Array(5).fill(null)));
    const [initialGrid, setInitialGrid] = useState<boolean[][]>(Array(5).fill(null).map(() => Array(5).fill(false)));
    const [selectedCell, setSelectedCell] = useState<[number, number] | null>(null);

    useEffect(() => {
        // Generate a basic 5x5 snake base
        const demoGrid = Array(5).fill(null).map(() => Array(5).fill(null));
        demoGrid[0][0] = 1; demoGrid[4][4] = 25; demoGrid[2][2] = 13;
        setGrid(demoGrid);

        const initFlags = Array(5).fill(null).map(() => Array(5).fill(false));
        initFlags[0][0] = true; initFlags[4][4] = true; initFlags[2][2] = true;
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-4 flex flex-col items-center">
            ...
            <div className="bg-slate-900/80 p-6 rounded-3xl border border-white/10 shadow-2xl flex flex-col items-center">
                <p className="text-sm text-white/60 mb-6 text-center max-w-md">1'den 25'e kadar (yatay veya dikey olarak bitişik hücreleri) takip eden bir yol oluşturun.</p>
                ...
                <button onClick={clearCell} disabled={!selectedCell} className="px-6 py-2 bg-red-500/20 text-red-400 border border-red-500/50 rounded-lg mb-4 hover:bg-red-500/40 transition-colors">Seçili Hücreyi Sil</button>
