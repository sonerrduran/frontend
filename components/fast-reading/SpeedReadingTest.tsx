...
const READING_TEXTS: ReadingText[] = [
    {
        id: 1,
        title: 'Örnek Metin 1',
        content: 'Bu hızlı okuma testi için hazırlanmış örnek bir metindir. Metni okuyun ve sonunda soruları cevaplayın. Okuma hızınız ve anlama seviyeniz ölçülecektir.',
        difficulty: 'easy'
    },
    {
        id: 2,
        title: 'Örnek Metin 2',
        content: 'İkinci örnek metin burada yer almaktadır. Daha uzun ve detaylı bir içerik sunmaktadır. Dikkatli okuyun.',
        difficulty: 'medium'
    }
];
        ...
        return (
            <div className="w-full max-w-5xl mx-auto px-4 py-8 animate-in fade-in">
                <div className="flex items-center gap-4 mb-8">
                    <button onClick={() => setMode(GameMode.FAST_READING_MENU)} className="p-3 bg-white/10 rounded-xl hover:bg-white/20 transition-colors">
                        ⬅
                    </button>
                    <h2 className="text-3xl md:text-5xl font-black text-white italic">Okuma Hızı Testi</h2>
                </div>

                <p className="text-white/80 mb-8 text-lg">Bir metin seç ve okuma hızını (WPM - Dakikadaki Kelime Sayısı) ölçmeye başla!</p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {READING_TEXTS.map(text => (
                        <div key={text.id} className="bg-slate-800/80 backdrop-blur-md border border-white/10 p-6 rounded-3xl hover:border-indigo-500 hover:shadow-[0_0_20px_rgba(99,102,241,0.3)] transition-all cursor-pointer group" onClick={() => handleSelectText(text)}>
                            <div className="flex justify-between items-start mb-4">
                                <span className="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-xs font-bold">{text.category}</span>
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${text.level === 'Kolay' ? 'bg-green-500/20 text-green-300' : text.level === 'Orta' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-red-500/20 text-red-300'}`}>
                                    {text.level}
                                </span>
                            </div>
                            <h3 className="text-2xl font-black text-white mb-2 group-hover:text-indigo-400 transition-colors">{text.title}</h3>
                            <p className="text-slate-400 text-sm mb-6">{text.wordCount} Kelime • {text.questions.length} Soru</p>
    ...
    if (phase === 'PREPARATION') {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in zoom-in duration-300">
                <h2 className="text-2xl text-white/80 font-bold mb-8">Metin hazırlanıyor... Hadi başlayalım!</h2>
        ...
        return (
            <div className="w-full max-w-4xl mx-auto px-4 py-8 animate-in fade-in">
                <div className="bg-slate-800/90 backdrop-blur-xl p-8 md:p-12 rounded-[40px] shadow-2xl border border-white/10 relative">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black text-white mb-8 text-center">{selectedText.title}</h2>
        ...
        return (
            <div className="w-full max-w-3xl mx-auto px-4 py-8 animate-in slide-in-from-right">
                <div className="text-center mb-8">
                    ...
                    <p className="text-indigo-300 font-bold">Soru {currentQuestionIdx + 1} / {selectedText.questions.length}</p>
                </div>
    ...
    if (phase === 'RESULTS' && selectedText) {
        ...
        return (
            <div className="w-full max-w-4xl mx-auto px-4 py-8 animate-in zoom-in">
                <h2 className="text-4xl md:text-6xl font-black text-center text-white italic mb-12">TEST SONUCU</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                    ...
                    <div className="bg-gradient-to-br from-indigo-600 to-purple-800 p-8 rounded-[40px] text-center shadow-xl border border-white/20">
                        <p className="text-indigo-200 font-bold uppercase tracking-widest text-sm mb-4">Okuma Hızı</p>
                    ...
                    {/* Comprehension Card */}
                    <div className={`p-8 rounded-[40px] text-center shadow-xl border border-white/20 bg-gradient-to-br ${accuracy >= 70 ? 'from-emerald-600 to-teal-800' : 'from-orange-600 to-red-800'}`}>
                        <p className="text-white/80 font-bold uppercase tracking-widest text-sm mb-4">Anlama Oranı</p>
                        <div className="text-7xl font-black text-white mb-2 drop-shadow-lg">%{accuracy}</div>
                        <p className="text-white/80 font-bold text-lg">{score} / {selectedText.questions.length} Doğru</p>
                    </div>
                </div>

                <div className="bg-slate-800/80 p-6 rounded-3xl mb-12 text-center border border-white/10">
                    <p className="text-slate-300 text-lg">
                        Toplam <strong>{selectedText.wordCount}</strong> kelimeyi <strong>{readingTimeSecs.toFixed(1)}</strong> saniyede okudun.
                    </p>
                    {accuracy < 60 && (
                        <p className="text-orange-400 mt-2 font-bold">Anlama oranın düşük. Okuduğunu anlamak, hızlı okumak kadar önemlidir. Yavaşlayarak tekrar dene!</p>
                    )}
                </div>

                <div className="flex gap-4 justify-center">
                    <button onClick={() => setPhase('SELECTION')} className="px-8 py-4 bg-white/10 hover:bg-white/20 rounded-2xl text-white font-bold transition-colors">
                        🔄 TEKRAR TEST ET
                    </button>
                    <button onClick={() => setMode(GameMode.FAST_READING_MENU)} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 rounded-2xl text-white font-black shadow-lg transition-transform hover:scale-105 active:scale-95">
                        MENÜYE DÖN
                    </button>
                </div>
            </div>
