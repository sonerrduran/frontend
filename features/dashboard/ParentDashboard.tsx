                ...
                {selectedChild && analytics ? (
                    <>
                        ...
                        <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-2xl p-6 mb-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-white/40">{selectedChild.gradeLevel}. Sınıf • Seviye {selectedChild.level || 1}</p>
                                </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            {[
                                { icon: '⭐', label: 'Yıldız', value: selectedChild.stars || 0, color: 'from-yellow-500/20 to-amber-500/10' },
                                { icon: '🎯', label: 'Doğru Oranı', value: `%${analytics.summary.correctRate}`, color: 'from-green-500/20 to-emerald-500/10' },
                                { icon: '🔥', label: 'Seri', value: `${selectedChild.streakDays || 0} gün`, color: 'from-red-500/20 to-orange-500/10' },
                                { icon: '📊', label: 'Toplam XP', value: selectedChild.xp || 0, color: 'from-blue-500/20 to-cyan-500/10' },
                            ].map((stat, i) => (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-white font-semibold">{analytics.summary.weeklySessions}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-white/60 text-sm">Kazanılan XP</span>
                                        <span className="text-cyan-300 font-semibold">{analytics.summary.weeklyXp}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-white/60 text-sm">Doğru Cevap</span>
                                        <span className="text-green-300 font-semibold">{analytics.summary.totalCorrect}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-white/60 font-semibold">{Math.round((analytics.summary.avgDuration || 0) / 60)} dk</span>
                                    </div>
                                </div>
                                {/* Simple Bar Chart */}
                                <div className="mt-4 pt-4 border-t border-white/5">
                                    ...
                                    <div className="flex gap-1 h-6 rounded-full overflow-hidden bg-white/5">
                                        <div className="bg-green-500/60 rounded-l-full transition-all"
                                            style={{ width: `${analytics.summary.correctRate}%` }} />
                                        <div className="bg-red-500/40 rounded-r-full flex-1" />
                                    </div>
                                    <div className="flex justify-between mt-1">
                                        <span className="text-green-400 text-xs">✅ {analytics.summary.totalCorrect}</span>
                                        <span className="text-red-400 text-xs">❌ {analytics.summary.totalIncorrect}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                                ...
                                {Object.keys(analytics.gameDistribution || {}).length > 0 ? (
                                    <div className="space-y-3">
                                        {Object.entries(analytics.gameDistribution).map(([mode, data]: [string, any]) => (
                                            <div key={mode}>
                                                ...
                                                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                                                    <div className="h-full bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full transition-all"
                                                        style={{ width: `${Math.min(100, (data.count / Math.max(1, analytics.summary.totalSessions)) * 100)}%` }} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-6">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {analytics.badges.map((badge: any, i: number) => (
                                        <div key={i} className="bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 text-center">
                        ...
                        {aiReport && (
                            <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-xl p-6 mb-6">
                                ...
                                <pre className="text-white/80 text-sm whitespace-pre-line font-sans leading-relaxed">{aiReport}</pre>
                            </div>
                        )}
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-white/40 text-xs">
                                                        {new Date(s.completedAt).toLocaleDateString('tr-TR')} • {Math.round(s.durationSec / 60)} dk
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-white text-sm font-semibold">{s.score} puan</div>
                                                <div className="text-green-400 text-xs">{s.correctAnswers}/{s.totalQuestions} doğru</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
