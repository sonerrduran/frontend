...
const SlitherlinkGame: React.FC<SlitherlinkGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        ...
        const initialHLines = Array(size + 1).fill(null).map(() => Array(size).fill('empty'));
        setHLines(initialHLines);

        const initialVLines = Array(size).fill(null).map(() => Array(size + 1).fill('empty'));
                ...
                <div className="flex flex-col">
                        ...
                        {c < size && (
                            ...
                            >
                                <div className={`w-full h-full rounded-full transition-all 
                                    ${hLines[r] && hLines[r][c] === 'drawn' ? 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.8)]' :
                                        hLines[r] && hLines[r][c] === 'crossed' ? 'bg-transparent text-red-500 font-bold text-xs flex items-center justify-center' :
                    ...
                    {r < size && (
                        <div className="flex">
                            ...
                            >
                                <div className={`w-full h-full rounded-full transition-all 
                                    ${vLines[r] && vLines[r][c] === 'drawn' ? 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.8)]' :
                                        vLines[r] && vLines[r][c] === 'crossed' ? 'bg-transparent text-red-500 font-bold text-xs flex items-center justify-center leading-none' :
                                            'bg-slate-700/50 group-hover:bg-slate-600'}`
                                }>
                            ...
                            {c < size && (
                                <div className={`${cellSize} flex items-center justify-center text-2xl md:text-4xl font-black text-white`}>
                            ...
                            {c === size - 1 && (
                                ...
                                >
                                    <div className={`w-full h-full rounded-full transition-all 
                                        ${vLines[r] && vLines[r][c + 1] === 'drawn' ? 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.8)]' :
                                            vLines[r] && vLines[r][c + 1] === 'crossed' ? 'bg-transparent text-red-500 font-bold text-xs flex items-center justify-center leading-none' :
                                                'bg-slate-700/50 group-hover:bg-slate-600'}`
                                    }>
