...
const MinesweeperGame: React.FC<MinesweeperGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getMinesForDifficulty = (boardSize: number) => {
        switch (difficulty) {
            case Difficulty.VERY_EASY:
            case Difficulty.EASY: return Math.floor((boardSize * boardSize) * 0.12);
            case Difficulty.MEDIUM: return Math.floor((boardSize * boardSize) * 0.16);
            case Difficulty.HARD:
            case Difficulty.VERY_HARD: return Math.floor((boardSize * boardSize) * 0.20);
            default: return Math.floor((boardSize * boardSize) * 0.16);
        }
    };
        ...
        <>
                ...
                infoCard={
                    <div className="flex gap-4">
                        <div className="text-center">
                            <div className="text-red-400 font-bold text-lg">💣 {Math.max(0, totalMines - flagsCount).toString().padStart(2, '0')}</div>
                        </div>
                        <div className={`text-center font-bold text-lg ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}>
            ...
            >
                ...
                <div className="flex justify-center">
                    <div className="bg-slate-300 p-2 md:p-3 rounded-xl border-4 border-slate-700 grid gap-1 w-full max-w-[360px]"
