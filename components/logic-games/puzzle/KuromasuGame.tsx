...
const BOARDS: Record<number, BoardData> = {
    ...
    7: {
        size: 7,
        clues: [
            { r: 0, c: 0, val: 2 }, { r: 0, c: 6, val: 2 },
            { r: 1, c: 3, val: 7 },
            { r: 3, c: 1, val: 7 }, { r: 3, c: 5, val: 7 },
            { r: 5, c: 3, val: 7 },
            { r: 6, c: 0, val: 2 }, { r: 6, c: 6, val: 2 }
        ]
    }
};
...
const KuromasuGame: React.FC<KuromasuGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            ...
            default: return 7; // Medium/Hard sizes
        }
    };
    ...
    const initGame = useCallback(() => {
        const initialBoard = Array(size).fill(null).map(() => Array(size).fill('empty' as CellState));
    ...
    const isWhiteConnected = (currentBoard: CellState[][]) => {
        ...
        while (stack.length > 0) {
            ...
            for (const [dr, dc] of directions) {
                ...
                if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited[nr][nc] && currentBoard[nr][nc] !== 'black') {
                    visited[nr][nc] = true;
                    connectedCount++;
                    stack.push([nr, nc]);
                }
    ...
    const checkWinCondition = (currentBoard: CellState[][]) => {
        ...
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
                if (currentBoard[r][c] === 'empty' && !boardData.clues.some(cl => cl.r === r && cl.c === c)) {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white font-sans selection:bg-slate-500/30">
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>İçinde sayı olan hücreler, o hücreden bakıldığında sağa, sola, yukarı ve aşağı <b>siyah kutulara</b> veya <b>duvara</b> çarpana kadar görülebilen toplam <b>beyaz</b> hücre sayısını (kendi dâhil) gösterir.</div>
                            </li>
                            <li className="flex gap-3 text-rose-400 font-bold">
                                <span>🚫</span>
                                <span><b>Siyah kutular</b> birbirine sağdan, soldan, üstten veya alttan temas edemez (çapraz değebilir).</span>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-cyan-400">🔗</span>
                                <span>Tüm <b>beyaz hücreler</b> (üzerinde sayı olmayanlar da dâhil) birbirine bir şekilde ağ gibi <b>bağlı</b> olmalıdır (aralara siyah girip bir beyazı hapsedemez).</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-slate-200 hover:bg-white text-slate-900 shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-2xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-slate-200'}`}>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center font-black text-2xl md:text-3xl transition-all duration-300 relative
                                            ${clue ? 'bg-slate-100 text-slate-900 cursor-not-allowed' :
                                                cellState === 'black' ? 'bg-slate-900 scale-[0.85] rounded-md shadow-[inset_0_3px_10px_rgba(0,0,0,0.8)]' :
                                                    cellState === 'white' ? 'bg-slate-200 text-slate-400 hover:bg-white' :
                                    ...
                                    >
                                        {clue ? (
                                            <>
                                                <span>{clue.val}</span>
                                                {isFulfilled && <div className="absolute top-1 right-1 w-2.5 h-2.5 bg-green-500 rounded-full shadow-[0_0_8px_rgba(34,197,94,1)] animate-pulse" />}
                                            </>
                                        ) : cellState === 'white' ? '·' : ''}
                                    </button>
                                );
                ...
                <div className="text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5">
                    <p className="text-slate-400 text-sm font-bold">1 Tık: <span className="text-white bg-slate-900 px-2 rounded-sm mx-1">Siyah</span> | 2 Tık: <span className="border border-slate-400/50 text-slate-800 bg-slate-200 px-2 rounded-sm mx-1">Beyaz Nokta</span> | 3 Tık: Boş</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Mantık Hatası!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Siyahlar temas edemez!' : timeLeft <= 0 ? 'Zamanı iyi yönetemedin.' : `Işınları mükemmel hesapladın! Kazanılan Yıldız: ${score}`}
                        </p>
