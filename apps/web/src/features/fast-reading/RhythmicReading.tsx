        ...
        <div className="w-full h-screen bg-slate-50 flex flex-col font-sans overflow-hidden">
            <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between z-30 shadow-sm">
                <div className="flex items-center gap-6">
                    <button onClick={onExit} className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition-all font-black">⬅</button>
                    <div>
                        <h1 className="text-xl font-black uppercase tracking-tighter leading-none mb-1 text-emerald-700">Blok Okuma</h1>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">RİTMİK GÖZ Sıçratma</p>
                    </div>
                </div>

                <div className="flex items-center gap-6">
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-400 uppercase mb-1">Okuma Hızı (WPM)</span>
                        <div className="flex items-center gap-3">
                            <input type="range" min="100" max="800" step="50" value={wpm} onChange={e => setWpm(Number(e.target.value))} className="w-32 accent-emerald-500" />
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                            {[1, 2, 3, 4].map(s => (
                                <button key={s} onClick={() => setBlockSize(s)} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${blockSize === s ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{s} Kelime</button>
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <button
                    onClick={() => { setIsPlaying(!isPlaying); if (!isPlaying) setCurrentIndex(0); }}
                ...
                <div className="max-w-4xl w-full bg-white rounded-[40px] p-16 shadow-2xl border border-slate-200/50 relative">
                    ...
                    {!isPlaying && currentIndex === -1 && (
                        <div className="absolute inset-0 bg-white/60 backdrop-blur-sm rounded-[40px] flex items-center justify-center border-4 border-dashed border-emerald-500/20 m-4">
                            <p className="text-emerald-800/40 font-black text-xl uppercase tracking-widest">METİN ALANI HAZIR</p>
                        </div>
                    )}
                </div>
            </div>
