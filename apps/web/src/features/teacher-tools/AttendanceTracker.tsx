...
const AttendanceTracker: React.FC<Props> = ({ onExit }) => {
    ...
    const [date] = useState(new Date().toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' }));
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-6 md:p-8 rounded-[40px] shadow-2xl border border-white/10 w-full max-w-3xl">
                ...
                {!started ? (
                    <div className="flex flex-col gap-4">
                        <p className="text-white/60 text-sm">Öğrenci isimlerini her satıra bir tane gelecek şekilde yazın:</p>
                        <textarea
                            value={namesText}
                    ...
                    <>
                        ...
                        <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1 custom-scrollbar">
                            {students.map((student, i) => (
                                <div key={i} className={`flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 transition-all ${student.status !== 'unset' ? 'ring-2 ' + statusConfig[student.status].ring : ''}`}>
                                    ...
                                    <div className="flex gap-1.5">
                                        {(['present', 'late', 'absent'] as const).map(status => (
                                            <button key={status} onClick={() => setStatus(i, status)}
