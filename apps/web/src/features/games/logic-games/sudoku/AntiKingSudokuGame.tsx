        ...
        <div className="flex flex-col items-center max-w-4xl mx-auto w-full">
            <div className="flex justify-between items-center w-full mb-6 relative px-4">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 text-center w-max">
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-yellow-600 drop-shadow-lg uppercase tracking-wider">Anti-King Sudoku</h2>
                </div>
                <button onClick={() => setShowRules(true)} className="p-3 bg-yellow-500/20 text-yellow-300 rounded-full hover:bg-yellow-500/30 transition-all">
                    ℹ️ Kurallar
                </button>
            </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-4 md:p-8 rounded-[40px] shadow-2xl border border-yellow-500/20 flex flex-col items-center relative overflow-hidden w-full max-w-2xl">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-md mb-8">
                            <li className="flex items-start gap-3">
                                <span className="text-xl">♚</span><span>Klasik Sudoku kurallarına ek olarak <b>Satranç Şahı'nın Hareketi</b> kısıtlaması vardır.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">❌</span><span>Aynı rakam, bir satranç şahının gidebileceği <b>komşu 8 hücreden (çaprazlar dahil)</b> hiçbirine koyulamaz.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">💡</span><span>Satır ve sütunlar zaten Sudoku kuralı gereği yasak olduğu için, asıl dikkat etmen gereken <b>4 çapraz komşuda aynı sayı olmaması!</b></span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="bg-gradient-to-r from-yellow-500 to-yellow-700 text-white font-black py-4 px-12 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all text-xl">
                            ANLADIM, BAŞLA!
                        </button>
                    </div>
                )}
                ...
                <div className="mb-4 flex justify-between items-center w-full opacity-90">
                    ...
                    {isGenerating && <span className="text-yellow-400 font-bold animate-pulse">Krallık Kuruluyor...</span>}
                    <button onClick={generatePuzzle} disabled={isGenerating} className="text-white hover:text-yellow-400 font-bold transition-colors disabled:opacity-50">Yeniden Düşman Getir 🔄</button>
                </div>
                ...
                <div className="mt-8 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-lg">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                        <button key={num} onClick={() => handleNumInput(num)} className="bg-white/10 hover:bg-yellow-500/80 hover:text-slate-900 text-white font-black text-xl py-3 rounded-xl transition-all shadow-md hover:scale-105 active:scale-95">
                            {num}
                        </button>
                    ))}
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/70 backdrop-blur-sm rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-yellow-600 to-amber-900 p-8 rounded-[32px] text-center shadow-[0_0_50px_rgba(250,204,21,0.5)] scale-110 border-4 border-white">
                            ...
                            <button onClick={() => onComplete(getStars())} className="bg-white text-yellow-800 px-8 py-4 mt-2 rounded-full font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all w-full">
                                +{getStars()} ⭐ KAZAN
                            </button>
                        </div>
                    </div>
