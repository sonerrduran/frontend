            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-white/10 rounded-[40px] p-6 md:p-12 shadow-2xl">
                ...
                {activeTab === 'LEADERBOARD' && (
                    <div className="animate-in slide-in-from-bottom flex flex-col gap-4">
                        ...
                        {MOCK_CLASS.map((student, idx) => (
                            <div key={idx} className={`flex items-center justify-between p-6 rounded-3xl border border-white/5 transition-all hover:bg-white/5 ${idx < 3 ? 'bg-gradient-to-r from-orange-500/10 to-transparent border-l-4 border-l-orange-500' : 'bg-black/20'}`}>
                                <div className="flex items-center gap-4">
                                    <span className={`text-2xl font-black ${idx === 0 ? 'text-yellow-400' : idx === 1 ? 'text-slate-300' : idx === 2 ? 'text-amber-600' : 'text-slate-600'}`}>#{idx + 1}</span>
                                ...
                                <div className="flex flex-col items-end gap-1">
                                    ...
                                    <span className="text-xs md:text-sm font-bold opacity-80 currentcolor" style={{ color: student.wpm >= 500 ? '#f43f5e' : student.wpm >= 300 ? '#f97316' : student.wpm >= 200 ? '#10b981' : '#06b6d4' }}>
                                        {student.badge}
                                    </span>
                                </div>
                            </div>
                ...
                {activeTab === 'ASSIGN' && (
                    <div className="animate-in zoom-in h-40 flex items-center justify-center bg-black/20 border border-white/10 rounded-3xl p-8 text-center text-slate-400 font-medium">
                        Öğretmenler bu bölümden öğrencilere özel metinler ekleyebilir ve süre kısıtlamalı testler atayabilir. (Backend bağlantısı ile aktifleşecektir.)
                    </div>
                )}
            </div>
        </div>
