...
const QuickPoll: React.FC<Props> = ({ onExit }) => {
    ...
    const [optionsText, setOptionsText] = useState('Çok güzeldi 🌟\nİyiydi 👍\nOrtalama 😐\nAnlamadım 😟');
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-6 md:p-10 rounded-[40px] shadow-2xl border border-white/10 w-full max-w-2xl">
                ...
                {!started ? (
                    <div className="flex flex-col gap-4">
                        ...
                        <div>
                            <label className="block text-white/60 text-sm font-bold mb-2">Seçenekler (her satıra bir tane):</label>
                            <textarea
                                value={optionsText}
                                ...
                                <div key={i} className="relative">
                                    ...
                                    >
                                        ...
                                        {showResults && totalVotes > 0 && (
                                            <div className={`absolute inset-0 ${opt.color} opacity-20 transition-all duration-700`}
                                                style={{ width: `${(opt.votes / totalVotes) * 100}%` }} />
                                        )}
                                        <div className={`relative z-10 w-10 h-10 rounded-xl ${opt.color} flex items-center justify-center text-white font-black text-lg shadow-lg`}>
                                            {showResults ? opt.votes : String.fromCharCode(65 + i)}
                                        </div>
                                        <span className="relative z-10 text-white font-semibold flex-1">{opt.text}</span>
                                        {showResults && totalVotes > 0 && (
