...
const ITEMS: Item[] = [
  { name: 'Kalem', icon: '✏️', price: 2.50 },
  { name: 'Defter', icon: '📓', price: 5.75 },
  { name: 'Silgi', icon: '🧹', price: 1.25 },
  { name: 'Kalemtraş', icon: '✂️', price: 3.80 },
  { name: 'Cetvel', icon: '📏', price: 4.50 },
  { name: 'Boya Kalemi', icon: '🖍️', price: 12.90 },
  { name: 'Yapıştırıcı', icon: '📎', price: 6.35 },
  { name: 'Makas', icon: '✂️', price: 8.60 }
];
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-600 via-cyan-600 to-teal-600 p-4 md:p-8 flex items-center justify-center">
        <div className="bg-white/30 backdrop-blur-md rounded-3xl p-12 text-center shadow-2xl max-w-2xl">
          ...
          <div className="flex gap-4 justify-center">
            <button onClick={resetGame} className="px-8 py-4 bg-green-500 hover:bg-green-600 rounded-xl text-white font-bold text-xl transition-all">
              Tekrar Oyna
            </button>
            <button onClick={onBack} className="px-8 py-4 bg-white/20 hover:bg-white/30 rounded-xl text-white font-bold text-xl transition-all">
              Menüye Dön
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 via-cyan-600 to-teal-600 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <button onClick={onBack} className="px-6 py-3 bg-white/20 hover:bg-white/30 rounded-xl text-white font-bold transition-all backdrop-blur-sm">
            ← Geri
          </button>
          <h1 className="text-3xl md:text-5xl font-black text-white drop-shadow-lg">🛒 Ondalık Market</h1>
        ...
        <div className="bg-white/30 backdrop-blur-md rounded-3xl p-6 mb-6 text-center shadow-2xl">
          <h3 className="text-2xl font-black text-white">Sepetteki Ürünlerin Toplam Fiyatını Hesapla!</h3>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {basket.map((item, index) => (
              <div key={index} className="bg-white/40 rounded-2xl p-6 text-center shadow-lg">
                ...
                <div className="text-3xl font-black text-yellow-300">{item.price.toFixed(2)} TL</div>
              </div>
            ))}
          </div>
