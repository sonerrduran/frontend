        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-800 via-gray-900 to-slate-900 text-white p-4">
            <div className="w-full max-w-6xl bg-slate-900/90 backdrop-blur-xl rounded-[40px] p-6 md:p-8 border border-slate-500/30 shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                    <button onClick={onExit} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl border border-white/10 transition-all font-bold">
                        ⬅ GERİ
                    </button>
                ...
                {/* Player 2 Hand */}
                <div className="mb-6">
                    <div className="text-center mb-2 text-sm font-bold text-red-400">Oyuncu 2 ({player2Hand.length} taş)</div>
                    <div className="flex justify-center gap-2 flex-wrap">
                        {currentPlayer === 2 ? (
                            ...
                            player2Hand.map((_, idx) => (
                                <div key={idx} className="bg-slate-700 w-[60px] h-[80px] rounded-lg border-2 border-slate-600"></div>
                            ))
                        )}
                    </div>
                </div>
                ...
                <div className="bg-emerald-800/30 p-4 rounded-2xl border-2 border-emerald-700/50 mb-6 min-h-[120px]">
                    <div className="flex gap-2 overflow-x-auto pb-2 justify-center items-center flex-wrap">
                        {board.map((domino, idx) => (
                            <div key={idx}>
                                {renderDomino(domino, undefined, true)}
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Player 1 Hand */}
                <div>
                    <div className="text-center mb-2 text-sm font-bold text-blue-400">Oyuncu 1 ({player1Hand.length} taş)</div>
                    <div className="flex justify-center gap-2 flex-wrap">
                        {currentPlayer === 1 ? (
