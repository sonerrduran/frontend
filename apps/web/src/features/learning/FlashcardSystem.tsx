...
const INITIAL_CARDS = [
    ...
    { id: 3, front: "Sovereignty", back: "Egemenlik, bir devletin kendi toprakları üzerindeki tam kontrolü." },
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[500px] p-4 text-white">
            <div className="w-full max-w-xl bg-slate-900/80 backdrop-blur-3xl rounded-[40px] p-8 md:p-12 border border-white/10 shadow-2xl relative overflow-hidden text-center">
                ...
                <h3 className="font-black tracking-widest text-[10px] uppercase mb-10 opacity-40">SRS EZBER SİSTEMİ (Aralıklı Tekrar)</h3>

                {!isComplete ? (
                    <div className="animate-in fade-in duration-500">
                        {/* Progress */}
                        <div className="text-xs font-black mb-6 opacity-30">KART {currentIndex + 1} / {cards.length}</div>
                        ...
                        >
                            ...
                            <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[32px] flex items-center justify-center p-8 [backface-visibility:hidden] border border-white/20 shadow-xl">
                                ...
                                <div className="absolute bottom-6 text-[10px] font-black opacity-40 uppercase tracking-widest">ÇEVİRMEK İÇİN TIKLA</div>
                            </div>
                        ...
                        <div className="mt-12 flex flex-col gap-4">
                            {isFlipped && (
                                <div className="flex gap-2 animate-in slide-in-from-bottom-4">
                                    <button onClick={nextCard} className="flex-1 bg-rose-500/20 hover:bg-rose-500/30 text-rose-400 py-4 rounded-2xl font-black text-xs uppercase border border-rose-500/20 uppercase">ZOR (TEKRAR ET)</button>
                                    <button onClick={nextCard} className="flex-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 py-4 rounded-2xl font-black text-xs uppercase border border-cyan-500/20 uppercase">KOLAY (ÖĞRENDİM)</button>
                                </div>
                            )}
                            {!isFlipped && (
                                <button onClick={handleFlip} className="w-full bg-white text-slate-900 py-5 rounded-2xl font-black text-xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl">
                                    CEVABI GÖR 👀
                                </button>
                            )}
                        </div>
                    ...
                    <div className="animate-in zoom-in duration-500">
                        ...
                        <p className="text-white/60 mb-8 font-medium">Bu oturumdaki tüm kartları tamamladın. Yarın tekrar ederek kalıcı hafızanı güçlendirebilirsin.</p>
                        <button
