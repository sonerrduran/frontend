...
import { classroomAPI, questionAPI, sessionAPI, assignmentAPI, messageAPI, examAPI } from '../../services/api';
import { API_URL } from '../../config/api';

// ─── Araç Bileşenlerini Lazy Yükle ───────────────────────
const Whiteboard = lazy(() => import('../../components/teacher-tools/Whiteboard'));
const RandomStudentPicker = lazy(() => import('../../components/teacher-tools/RandomStudentPicker'));
const ClassTimer = lazy(() => import('../../components/teacher-tools/ClassTimer'));
const GroupMaker = lazy(() => import('../../components/teacher-tools/GroupMaker'));
const DiceRoller = lazy(() => import('../../components/teacher-tools/DiceRoller'));
const Scoreboard = lazy(() => import('../../components/teacher-tools/Scoreboard'));
const NoiseMeter = lazy(() => import('../../components/teacher-tools/NoiseMeter'));
const StickyNotes = lazy(() => import('../../components/teacher-tools/StickyNotes'));
const AttendanceTracker = lazy(() => import('../../components/teacher-tools/AttendanceTracker'));
const SeatingChart = lazy(() => import('../../components/teacher-tools/SeatingChart'));
const QuickPoll = lazy(() => import('../../components/teacher-tools/QuickPoll'));
const WordCloud = lazy(() => import('../../components/teacher-tools/WordCloud'));
const BirthdayCalendar = lazy(() => import('../../components/teacher-tools/BirthdayCalendar'));
const ClassGoals = lazy(() => import('../../components/teacher-tools/ClassGoals'));
const NoticeBulletin = lazy(() => import('../../components/teacher-tools/NoticeBulletin'));
const SpinWheel = lazy(() => import('../../components/teacher-tools/SpinWheel'));

// ─── Araç Tanımları ───────────────────────────────────────
type ToolKey = 'whiteboard' | 'random' | 'timer' | 'group' | 'dice' | 'scoreboard' | 'noise' | 'sticky' | 'attendance' | 'seating' | 'poll' | 'wordcloud' | 'birthday' | 'goals' | 'notice' | 'spinwheel';
type ActiveView = ToolKey | 'sinif' | 'odev' | 'soruhavuzu' | 'mesaj' | 'sinav' | null;

interface Tool { key: ToolKey; icon: string; label: string; desc: string; color: string; }

const TOOLS: Tool[] = [
    { key: 'whiteboard', icon: '✏️', label: 'Beyaz Tahta', desc: 'Çiz, tasarla, karala!', color: 'from-stone-500 to-neutral-700' },
    { key: 'random', icon: '🎡', label: 'Rastgele Öğrenci', desc: 'Çarkı çevir, isim belirle!', color: 'from-indigo-500 to-purple-700' },
    { key: 'timer', icon: '⏱️', label: 'Zamanlayıcı', desc: 'Geri sayım ve kronometre', color: 'from-rose-500 to-red-700' },
    { key: 'group', icon: '👥', label: 'Grup Oluşturucu', desc: 'Sınıfı adil takımlara ayır', color: 'from-emerald-500 to-teal-700' },
    { key: 'dice', icon: '🎲', label: 'Zar & Sayı', desc: 'Zarları at, sayı çek', color: 'from-amber-500 to-orange-700' },
    { key: 'scoreboard', icon: '📊', label: 'Skor Tablosu', desc: 'Takım puan tahtası', color: 'from-cyan-500 to-blue-700' },
    { key: 'noise', icon: '🤫', label: 'Gürültü Ölçer', desc: 'Sınıf ses seviyesi', color: 'from-fuchsia-500 to-pink-700' },
    { key: 'sticky', icon: '📝', label: 'Mantar Pano', desc: 'Notlar ve hatırlatmalar', color: 'from-yellow-400 to-yellow-600' },
    { key: 'attendance', icon: '📋', label: 'Yoklama', desc: 'Hızlı yoklama takibi', color: 'from-emerald-600 to-green-800' },
    { key: 'seating', icon: '🪑', label: 'Oturma Düzeni', desc: 'Sınıf oturma planı', color: 'from-blue-500 to-indigo-700' },
    { key: 'poll', icon: '📊', label: 'Hızlı Anket', desc: 'Sınıf içi oylama', color: 'from-purple-500 to-pink-700' },
    { key: 'wordcloud', icon: '☁️', label: 'Kelime Bulutu', desc: 'Fikir ve kelime toplama', color: 'from-cyan-400 to-teal-600' },
    { key: 'birthday', icon: '🎂', label: 'Doğum Günü Takvimi', desc: 'Sınıf doğum günleri', color: 'from-pink-500 to-rose-700' },
    { key: 'goals', icon: '🎯', label: 'Hedef Takip', desc: 'Sınıf hedefleri ve ilerleme', color: 'from-emerald-400 to-green-600' },
    { key: 'notice', icon: '📮', label: 'Duyuru Panosu', desc: 'Duyurular ve notlar', color: 'from-amber-400 to-orange-600' },
    { key: 'spinwheel', icon: '🎪', label: 'Çarkıfelek', desc: 'Animasyonlu seçim çarkı', color: 'from-violet-500 to-purple-700' },
];

// ─── Araç Bileşeni Seçici ──────────────────────────────────
function ToolComponent({ toolKey, onExit }: { toolKey: ToolKey; onExit: () => void }) {
    switch (toolKey) {
        case 'whiteboard': return <Whiteboard onExit={onExit} />;
        case 'random': return <RandomStudentPicker onExit={onExit} />;
        case 'timer': return <ClassTimer onExit={onExit} />;
        case 'group': return <GroupMaker onExit={onExit} />;
        case 'dice': return <DiceRoller onExit={onExit} />;
        case 'scoreboard': return <Scoreboard onExit={onExit} />;
        case 'noise': return <NoiseMeter onExit={onExit} />;
        case 'sticky': return <StickyNotes onExit={onExit} />;
        case 'attendance': return <AttendanceTracker onExit={onExit} />;
        case 'seating': return <SeatingChart onExit={onExit} />;
        case 'poll': return <QuickPoll onExit={onExit} />;
        case 'wordcloud': return <WordCloud onExit={onExit} />;
        case 'birthday': return <BirthdayCalendar onExit={onExit} />;
        case 'goals': return <ClassGoals onExit={onExit} />;
        case 'notice': return <NoticeBulletin onExit={onExit} />;
        case 'spinwheel': return <SpinWheel onExit={onExit} />;
        default: return null;
    }
}
...
function SinifPanel({ classrooms, onRefresh }: { classrooms: any[]; onRefresh: () => void }) {
    ...
    const addStudent = async (classroomId: string) => {
        ...
        setAddingStudent(true); setStudentMsg('');
        try {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">➕ Yeni Sınıf Oluştur</h3>
                ...
                {msg && <p className={`mt-2 text-sm font-medium ${msg.type === 'ok' ? 'text-emerald-400' : 'text-red-400'}`}>{msg.text}</p>}
            </div>
                    ...
                    <div className="space-y-3">
                        {classrooms.map((cls: any) => (
                            <div key={cls.id} className="bg-slate-900/50 rounded-xl border border-white/10">
                                <button onClick={() => setExpandedId(expandedId === cls.id ? null : cls.id)}
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-white/40 text-sm">{cls._count?.students || 0} öğrenci · {cls.gradeLevel}. Sınıf</div>
                                        </div>
                                    </div>
                                ...
                                {expandedId === cls.id && (
                                    <div className="px-4 pb-4 border-t border-white/5 pt-3">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button onClick={() => addStudent(cls.id)} disabled={addingStudent}
                                        ...
                                        {studentMsg && <p className={`mt-1 text-xs ${studentMsg.startsWith('✅') ? 'text-emerald-400' : 'text-red-400'}`}>{studentMsg}</p>}
                                    </div>
                                )}
                            </div>
                        ))}
...
function OdevPanel({ classrooms }: { classrooms: any[] }) {
    const [form, setForm] = useState({
        classroomId: '', title: '', gameMode: 'MATH_OPERATIONS',
        difficulty: 'MEDIUM', questionCount: 10, dueDate: ''
    });
    const [loading, setLoading] = useState(false);
    const [msg, setMsg] = useState<{ type: 'ok' | 'err'; text: string } | null>(null);
    ...
    const submit = async () => {
        if (!form.classroomId || !form.title) {
            setMsg({ type: 'err', text: '❌ Sınıf ve başlık zorunludur.' }); return;
        }
        setLoading(true); setMsg(null);
        try {
            // POST to /api/classrooms/:id/assignments (via fetch since classroomAPI doesn't have it yet)
            const token = localStorage.getItem('token');
            const res = await fetch(`${API_URL}/classrooms/${form.classroomId}/assignments`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({
                    title: form.title, gameMode: form.gameMode,
    ...
    if (classrooms.length === 0) {
        return (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
                ...
                <p className="text-white/30 text-sm mt-1">Sol panelden &quot;Sınıf Oluştur&quot; butonuna tıklayın.</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select value={form.classroomId} onChange={e => setForm(f => ({ ...f, classroomId: e.target.value }))}
                        ...
                        {classrooms.map((cls: any) => (
                            <option key={cls.id} value={cls.id}>{cls.name} ({cls.gradeLevel}. Sınıf)</option>
                        ))}
                    </select>
                </div>
                {/* Başlık */}
                ...
                <div>
                    ...
                    <input type="number" min={1} max={50} value={form.questionCount}
                        onChange={e => setForm(f => ({ ...f, questionCount: +e.target.value }))}
                        className="w-full bg-slate-950 text-white p-3 rounded-xl border border-white/10 focus:border-amber-500 outline-none" />
                </div>
                {/* Son Tarih */}
                <div>
                    <label className="text-white/50 text-sm font-medium block mb-1">Son Tarih (isteğe bağlı)</label>
            ...
            {msg && <p className={`mt-3 text-sm font-medium text-center ${msg.type === 'ok' ? 'text-emerald-400' : 'text-red-400'}`}>{msg.text}</p>}
        </div>
    );
}
...
function SoruHavuzuPanel() {
    ...
    const [form, setForm] = useState({ text: '', optA: '', optB: '', optC: '', optD: '', correctAnswer: '', difficulty: 'MEDIUM', gradeLevel: 5, explanation: '' });
    const [formMsg, setFormMsg] = useState('');
    const [formLoading, setFormLoading] = useState(false);
    const [aiPrompt, setAiPrompt] = useState('5. sınıf matematik çarpma işlemleri hakkında A/B/C/D seçenekli 5 soru üret.');
    ...
    useEffect(() => { if (tab === 'list') loadQuestions(); }, [tab, loadQuestions]);

    const addQuestion = async () => {
        if (!form.text || !form.correctAnswer) { setFormMsg('❌ Soru ve doğru cevap zorunlu.'); return; }
        ...
        try {
            ...
            setForm({ text: '', optA: '', optB: '', optC: '', optD: '', correctAnswer: '', difficulty: 'MEDIUM', gradeLevel: 5, explanation: '' });
            loadQuestions();
        } catch { setFormMsg('❌ Hata oluştu.'); }
    ...
    const generateAI = async () => {
        setAiLoading(true); setAiMsg('🤖 Yapay zeka sorular üretiyor...');
        try {
            const res: any = await questionAPI.generate({ prompt: aiPrompt, grade: 5, count: 5 });
            const generated = res.data?.questions || [];
            if (generated.length > 0) {
                await questionAPI.bulkCreate({ questions: generated, setName: `AI - ${new Date().toLocaleDateString('tr-TR')}` });
                setAiMsg(`✅ ${generated.length} soru oluşturuldu ve havuza eklendi!`);
                loadQuestions();
            } else {
                setAiMsg('⚠️ Sorular üretilemedi. Prompt\'u daha ayrıntılı yazın.');
            }
        } catch { setAiMsg('❌ AI servisi şu an kullanılamıyor.'); }
        finally { setAiLoading(false); }
    };

    const DIFF_COLORS: Record<string, string> = { EASY: 'text-emerald-400 bg-emerald-400/10', MEDIUM: 'text-amber-400 bg-amber-400/10', HARD: 'text-red-400 bg-red-400/10' };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
                {([['list', '📋 Sorular'], ['add', '➕ Soru Ekle'], ['ai', '🤖 AI ile Üret']] as const).map(([key, label]) => (
            ...
            {tab === 'list' && (
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                    ...
                    <div className="flex gap-3 mb-4">
                        ...
                        <button onClick={loadQuestions} className="px-4 py-2.5 bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 rounded-lg text-sm font-bold hover:bg-cyan-500/20 transition-all">
                            🔄
                        </button>
                    </div>

                    <p className="text-white/40 text-sm mb-3">Toplam <strong className="text-white">{total}</strong> soru</p>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
                            {questions.map((q: any) => (
                                <div key={q.id} className="flex items-start gap-3 p-3 bg-slate-900/50 rounded-xl border border-white/5 group">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-white text-sm font-medium line-clamp-2">{q.text}</p>
                                        <div className="flex items-center gap-2 mt-1.5">
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${DIFF_COLORS[q.difficulty] || ''}`}>
            ...
            {tab === 'add' && (
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="md:col-span-2">
                            ...
                            <textarea value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))}
                                rows={2} placeholder="Soru metnini yazın..."
                        ...
                        {(['optA', 'optB', 'optC', 'optD'] as const).map((key, i) => (
                            <div key={key}>
                                <label className="text-white/50 text-xs font-medium block mb-1">{String.fromCharCode(65 + i)} Şıkkı</label>
                        ...
                        <div className="md:col-span-2">
                            <label className="text-white/50 text-xs font-medium block mb-1">Açıklama (isteğe bağlı)</label>
                            <input value={form.explanation} onChange={e => setForm(f => ({ ...f, explanation: e.target.value }))}
                                placeholder="Cevap açıklaması..."
                                className="w-full bg-slate-950 text-white p-2.5 rounded-xl border border-white/10 focus:border-cyan-500 outline-none text-sm" />
                        </div>
                    </div>
                    <button onClick={addQuestion} disabled={formLoading}
                    ...
                    {formMsg && <p className={`mt-2 text-sm font-medium text-center ${formMsg.startsWith('✅') ? 'text-emerald-400' : 'text-red-400'}`}>{formMsg}</p>}
                </div>
            )}
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                    <div className="flex items-start gap-3 mb-4 p-3 bg-cyan-500/10 border border-cyan-500/20 rounded-xl">
                        ...
                        <div>
                            ...
                            <p className="text-white/50 text-xs mt-0.5">Gemini AI, verdiğiniz açıklamaya göre otomatik soru üretir ve havuza ekler.</p>
                        </div>
                    </div>
                    ...
                    <button onClick={generateAI} disabled={aiLoading}
                        ...
                        {aiLoading ? <><span className="animate-spin">⚙️</span> Üretiliyor...</> : '🤖 AI ile Soru Üret'}
                    </button>
                    {aiMsg && <p className={`mt-3 text-sm font-medium text-center ${aiMsg.startsWith('✅') ? 'text-emerald-400' : aiMsg.startsWith('⚠️') ? 'text-amber-400' : aiMsg.startsWith('🤖') ? 'text-cyan-400' : 'text-red-400'}`}>{aiMsg}</p>}
                </div>
            )}
        </div>
    );
...
function MesajPanel() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [convRes, teachRes] = await Promise.all([messageAPI.conversations() as any, messageAPI.teachers() as any]);
    ...
    const openThread = async (userId: string, partner: any) => {
        ...
        try { const r: any = await messageAPI.getThread(userId); setMessages(r.data?.messages || []); } catch { setMessages([]); }
    };
    ...
    return (
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden" style={{ minHeight: 480 }}>
            <div className="flex h-[480px]">
                ...
                <div className="w-72 border-r border-white/10 flex flex-col">
                    ...
                    <div className="overflow-y-auto flex-1">
                        ...
                        {teachers.map((t: any) => (
                            <button key={t.id} onClick={() => openThread(t.id, t)}
                        ...
                        {conversations.filter((c: any) => !teachers.find((t: any) => t.id === c.partner.id)).map((conv: any) => (
                            <button key={conv.partner.id} onClick={() => openThread(conv.partner.id, conv.partner)}
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-white/40 text-xs truncate">{conv.lastMessage?.content}</div>
                                </div>
                                {conv.unreadCount > 0 && <span className="bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full">{conv.unreadCount}</span>}
                            </button>
                        ))}
                    ...
                    {selectedUser ? (
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                {messages.length === 0 && <div className="text-white/30 text-sm text-center mt-12">Henüz mesaj yok. İlk mesajı gönderin!</div>}
                                {messages.map((m: any) => (
                                    <div key={m.id} className={`flex ${m.senderId === user?.id ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${m.senderId === user?.id ? 'bg-blue-600 text-white' : 'bg-white/10 text-white'
                                            }`}>{m.content}</div>
                                    </div>
                                ))}
                            </div>
                            <div className="p-4 border-t border-white/10 flex gap-2">
                                ...
                                <button onClick={sendMsg} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all">Gönder ↗</button>
                            </div>
                        </>
                    ) : (
                        <div className="flex-1 flex items-center justify-center text-white/30">
                            <div className="text-center"><div className="text-5xl mb-3">💬</div><p>Sol taraftan kişi seçin</p></div>
                        </div>
                    )}
                </div>
            </div>
...
function SinavPanel({ classrooms }: { classrooms: any[] }) {
    ...
    const [loading, setLoading] = useState(true);
    const [form, setForm] = useState({ classroomId: '', title: '', description: '', durationMin: 30, questionIds: [] as string[], dueDate: '' });
    const [creating, setCreating] = useState(false);
    const [msg, setMsg] = useState('');
    const [results, setResults] = useState<{ examId: string; data: any[] } | null>(null);

    const loadExams = async () => {
        try { const r: any = await examAPI.list(); setExams(r.data || []); } catch { } finally { setLoading(false); }
    };
    useEffect(() => { loadExams(); }, []);

    const createExam = async () => {
        if (!form.classroomId || !form.title) { setMsg('❌ Sınıf ve başlık zorunludur.'); return; }
        setCreating(true);
        try {
            await examAPI.create({ ...form, dueDate: form.dueDate || undefined });
            setMsg('✅ Sınav oluşturuldu!');
            setForm({ classroomId: '', title: '', description: '', durationMin: 30, questionIds: [], dueDate: '' });
            setTab('list'); loadExams();
        } catch { setMsg('❌ Hata oluştu.'); } finally { setCreating(false); }
    };

    const togglePublish = async (exam: any) => {
        try { await examAPI.publish(exam.id, !exam.isPublished); loadExams(); } catch { }
    };

    const viewResults = async (examId: string) => {
        try { const r: any = await examAPI.results(examId); setResults({ examId, data: r.data || [] }); } catch { }
    };
            ...
            {tab === 'list' && (
                <div className="space-y-3">
                    ...
                    {!loading && exams.length === 0 && (
                        <div className="text-center py-10 text-white/40"><div className="text-4xl mb-2">📄</div><p>Henüz sınav oluşturulmadı.</p></div>
                    )}
                    {exams.map((e: any) => (
                        <div key={e.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                            <div className="flex items-start justify-between">
                                ...
                                <div className="flex flex-col gap-2 ml-4">
                                    ...
                                    <button onClick={() => viewResults(e.id)} className="px-3 py-1 rounded-lg text-xs font-bold bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-all">📊 Sonuçlar</button>
                                </div>
                            </div>
                            {results?.examId === e.id && (
                                <div className="mt-4 border-t border-white/10 pt-4">
                                    <h5 className="text-white/70 text-sm font-bold mb-3">Sonuçlar ({results.data.length} öğrenci)</h5>
                                    {results.data.length === 0 ? <p className="text-white/30 text-sm">Henüz teslim yok.</p> : (
                                        <div className="space-y-2">
                                            {results.data.map((r: any) => (
                                                <div key={r.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                                                    ...
                                                    <span className={`font-bold text-sm ${r.percentage >= 70 ? 'text-emerald-400' : r.percentage >= 50 ? 'text-amber-400' : 'text-red-400'}`}>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select value={form.classroomId} onChange={e => setForm(f => ({ ...f, classroomId: e.target.value }))}
                                ...
                                <option value="">-- Sınıf seçin --</option>
                                {classrooms.map((c: any) => <option key={c.id} value={c.id}>{c.name} ({c.gradeLevel}. Sınıf)</option>)}
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                            rows={2} placeholder="Sınav hakkında kısa bilgi..."
                    ...
                    <p className="text-white/30 text-xs">💡 Soru havuzundan soru ID'leri girilmeden de sınav oluşturulabilir. Sorular daha sonra soruhavuzu'ndan eklenebilir.</p>
...
export default function TeacherDashboard() {
    ...
    const toolKeys = ['whiteboard', 'random', 'timer', 'group', 'dice', 'scoreboard', 'noise', 'sticky', 'attendance', 'seating', 'poll', 'wordcloud', 'birthday', 'goals', 'notice', 'spinwheel'] as ToolKey[];
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] flex flex-col">
                ...
                <div className="flex-1 overflow-auto">
                    <Suspense fallback={<div className="flex items-center justify-center h-64"><div className="text-4xl animate-spin">⚙️</div></div>}>
    ...
    const panelLabels: Record<string, string> = { sinif: '🏫 Sınıf Yönetimi', odev: '📝 Ödev Ata', soruhavuzu: '❓ Soru Havuzu', mesaj: '💬 Mesajlar', sinav: '📄 Sınav Yönetimi' };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e]">
            ...
            <header className="bg-[#1a1a2e]/80 backdrop-blur-lg border-b border-white/10 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        {activeView && <span className="text-white font-bold">{panelLabels[activeView] || activeView}</span>}
                    </div>
                    <button onClick={logout} className="px-4 py-2 rounded-lg bg-white/5 text-white/60 hover:bg-white/10 hover:text-white transition-all">
                        Çıkış 🚪
                    </button>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 py-8">
                {/* Inline panels */}
                {activeView === 'sinif' && (
                    <SinifPanel classrooms={classrooms} onRefresh={loadData} />
                )}
                {activeView === 'odev' && (
                    <OdevPanel classrooms={classrooms} />
                )}
                {activeView === 'soruhavuzu' && (
                    <SoruHavuzuPanel />
                )}
                {activeView === 'mesaj' && (
                    <MesajPanel />
                ...
                {activeView === 'sinav' && (
                    <SinavPanel classrooms={classrooms} />
                )}

                {/* Main Dashboard (visible when no inline panel) */}
                {!activeView && (
                    <>
                        <h2 className="text-3xl font-bold text-white mb-6">Hoş geldiniz, {user?.name?.split(' ')[0]}!</h2>
                        ...
                        {/* Quick Actions */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            <button onClick={() => setActiveView('sinif')}
                            ...
                            <button onClick={() => setActiveView('odev')}
                                ...
                                <div className="text-white/30 text-xs mt-1">{classrooms.length} sınıfa atanabilir</div>
                            </button>
                            <button onClick={() => setActiveView('soruhavuzu')}
                            ...
                            <button onClick={() => setActiveView('sinav')}
                            ...
                            <button onClick={() => setActiveView('mesaj')}
                        ...
                        <div className="mb-8">
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                🛠️ <span>Araçlar</span>
                                <span className="bg-white/10 text-white/50 px-3 py-1 rounded-full text-xs font-medium ml-2">Sınıf İçi</span>
                            </h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {TOOLS.map((tool) => (
                                    <button key={tool.key} onClick={() => setActiveView(tool.key)}
                                        ...
                                        <div className="absolute -bottom-6 -right-6 text-7xl opacity-10 group-hover:opacity-20 transition-opacity">{tool.icon}</div>
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                            ...
                            {classrooms.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {classrooms.map((cls: any) => (
                                        <button key={cls.id} onClick={() => setActiveView('sinif')}
                                            ...
                                            <div className="flex items-center justify-between mb-2">
                                                <h4 className="text-white font-semibold group-hover:text-purple-300 transition-colors">{cls.name}</h4>
                                                <span className="bg-purple-500/20 text-purple-300 text-xs px-2 py-1 rounded-full">{cls.gradeLevel}. Sınıf</span>
                                            </div>
                                            <div className="text-white/40 text-sm">{cls._count?.students || 0} öğrenci</div>
                                        </button>
                                    ))}
                                </div>
                            )}
