...
const TakGame: React.FC<Props> = ({ onExit }) => {
    const SIZE = 5;
    const [board, setBoard] = useState(Array(SIZE).fill(null).map(() => Array(SIZE).fill([])));
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-stone-900 text-white p-4">
            <div className="w-full max-w-2xl bg-slate-900/90 backdrop-blur-xl rounded-[40px] p-6 md:p-8 border border-slate-500/30 shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-slate-300 to-gray-400">🏔️ TAK</h2>
                    <div className="text-sm font-bold bg-slate-500/20 px-4 py-2 rounded-xl border border-slate-500/30">Oyuncu {currentPlayer}</div>
                </div>
