...
const ShapeLabyrinthGame: React.FC<ShapeLabyrinthGameProps> = ({ onBack }) => {
  ...
  const handleMove = (direction: 'up' | 'down' | 'left' | 'right') => {
    ...
    const newGrid = grid.map(row => row.map(c =>
      c.x === newX && c.y === newY ? { ...c, visited: true } : c
    ));
    setGrid(newGrid);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8 flex items-center justify-center">
        <div className="bg-slate-800/90 backdrop-blur-xl rounded-3xl p-12 text-center border border-cyan-500/30 max-w-2xl">
          ...
          <div className="flex gap-4 justify-center">
            <button onClick={resetGame} className="px-8 py-4 bg-gradient-to-br from-cyan-500 via-blue-500 to-cyan-600 hover:from-cyan-400 hover:to-blue-500 rounded-xl text-white font-bold text-xl transition-all transform hover:scale-105">Tekrar Oyna</button>
            <button onClick={onBack} className="px-8 py-4 bg-slate-700/50 hover:bg-slate-600/50 rounded-xl text-white font-bold text-xl transition-all transform hover:scale-105">Menüye Dön</button>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          <div className="bg-gradient-to-br from-cyan-500 via-blue-500 to-cyan-600 rounded-2xl p-8 md:p-12 mb-8">
            ...
            <div className="bg-cyan-700/40 rounded-2xl p-6 mb-6 border-2 border-cyan-300">
              <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${gridSize}, 1fr)` }}>
                {grid.map((row, y) => row.map((cell, x) => {
                  ...
                  return (
                    <div key={`${x}-${y}`} className={`aspect-square rounded-lg flex items-center justify-center text-3xl transition-all ${
                      isPlayer ? 'bg-yellow-400 scale-110' :
                      cell.visited ? 'bg-cyan-600/40' :
                      cell.isEnd ? 'bg-green-500' :
            ...
            <div className="grid grid-cols-3 gap-3">
              <div></div>
              <button onClick={() => handleMove('up')} className="p-4 bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-xl text-3xl transition-all transform hover:scale-105">⬆️</button>
              <div></div>
              <button onClick={() => handleMove('left')} className="p-4 bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-xl text-3xl transition-all transform hover:scale-105">⬅️</button>
              <div></div>
              <button onClick={() => handleMove('right')} className="p-4 bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-xl text-3xl transition-all transform hover:scale-105">➡️</button>
              <div></div>
              <button onClick={() => handleMove('down')} className="p-4 bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-xl text-3xl transition-all transform hover:scale-105">⬇️</button>
              <div></div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-cyan-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">1.</span> Sadece hedef şekillerin üzerinden geç</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">2.</span> Yön tuşlarıyla hareket et</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">3.</span> Yanlış şekle basarsan başa dönersin</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">4.</span> Çıkışa (🏁) ulaş ve puan kazan!</li>
            </ul>
