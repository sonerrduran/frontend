...
const ConsecutiveSudokuGame: React.FC<ConsecutiveSudokuGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        ...
        let holes = size === 4
            ? (difficulty === Difficulty.EASY ? 6 : difficulty === Difficulty.HARD ? 10 : 8)
            : (difficulty === Difficulty.EASY ? 15 : difficulty === Difficulty.HARD ? 22 : 18);
        ...
        <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-blue-500/30 shadow-2xl relative overflow-hidden bounce-in">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-center flex-1">
                    <h2 className="text-2xl md:text-3xl font-black italic text-blue-400 drop-shadow-lg uppercase tracking-tight">Ardışık Sudoku</h2>
                    <div className="text-xs font-bold text-blue-200 mt-1 uppercase tracking-widest">{size}x{size} Izgara | (Temel Sürüm) | Hata: {mistakes}/{maxMistakes}</div>
                </div>
                ...
                >
                    {board.map((row, r) =>
                        row.map((cell, c) => {
                            ...
                            const boxRows = size === 4 ? 2 : 2;
                            const boxCols = size === 4 ? 2 : 3;
                            const borderRight = (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-4 border-blue-700/80' : 'border-r border-blue-900/50';
                            const borderBottom = (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-4 border-blue-700/80' : 'border-b border-blue-900/50';

                            // Show consecutive marker (white bar) if adjacent cells in solved board are consecutive
                            const RightRelation = (c < size - 1 && solvedBoard[r] && solvedBoard[r][c] && solvedBoard[r][c + 1] && Math.abs(solvedBoard[r][c] - solvedBoard[r][c + 1]) === 1);
                            const BottomRelation = (r < size - 1 && solvedBoard[r] && solvedBoard[r + 1] && solvedBoard[r][c] && solvedBoard[r + 1][c] && Math.abs(solvedBoard[r][c] - solvedBoard[r + 1][c]) === 1);
                       ...
                       `}
                                    ...
                                    {RightRelation && c !== size - 1 && (c + 1) % boxCols !== 0 && (
                                        <div className="absolute top-1/2 -right-1 transform -translate-y-1/2 bg-white z-30 pointer-events-none rounded-full w-2 h-6 shadow-[0_0_5px_rgba(255,255,255,0.8)] border border-blue-800">
                                        </div>
                                    )}
                                    {BottomRelation && r !== size - 1 && (r + 1) % boxRows !== 0 && (
                                        <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 bg-white z-30 pointer-events-none rounded-full h-2 w-6 shadow-[0_0_5px_rgba(255,255,255,0.8)] border border-blue-800">
                                        </div>
                                    )}
                                </div>
                            )
            ...
            {isGameOver && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-[40px]">
                    <h3 className={`text-5xl md:text-7xl font-black mb-4 drop-shadow-xl ${mistakes >= maxMistakes || timeLeft === 0 ? 'text-red-500' : 'text-yellow-400'}`}>
                        {mistakes >= maxMistakes ? 'ÇOK HATA!' : timeLeft === 0 ? 'SÜRE BİTTİ' : 'HARİKA!'}
                    </h3>
                    <p className="text-xl md:text-2xl text-white font-bold mb-8">Kazanılan Yıldız: <span className="text-yellow-400">⭐ {score}</span></p>
