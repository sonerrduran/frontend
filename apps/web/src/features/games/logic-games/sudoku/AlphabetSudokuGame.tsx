...
const AlphabetSudokuGame: React.FC<Props> = ({ difficulty, onComplete, onExit }) => {
    ...
    const generatePuzzle = useCallback(() => {
        const board = Array(9).fill(0).map(() => Array(9).fill(null));
    ...
    const getStars = () => {
        switch (difficulty) {
            ...
            case Difficulty.HARD: return 80;
            case Difficulty.VERY_HARD: return 120;
            default: return 35;
        }
    };
        ...
        <div className="flex flex-col items-center max-w-4xl mx-auto w-full">
            <div className="flex justify-between items-center w-full mb-6 relative px-4">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 text-center w-max">
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-500 drop-shadow-lg uppercase tracking-wider">Alphabet Sudoku</h2>
                </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-4 md:p-8 rounded-[40px] shadow-2xl border border-yellow-500/20 flex flex-col items-center relative overflow-hidden w-full max-w-2xl">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-md mb-8">
                            <li className="flex items-start gap-3">
                                <span className="text-xl">🔤</span><span>Tıpkı klasik Sudoku gibidir, <b>fakat rakamlar yerine harfler</b> kullanılır.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">1️⃣</span><span>Her satır, her sütun ve her 3x3 kutu içinde <b>A'dan I'ya kadar</b> olan 9 harfin tamamı tam olarak birer kez bulunmalıdır.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="bg-gradient-to-r from-yellow-500 to-amber-700 text-white font-black py-4 px-12 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all text-xl">
                            ANLADIM, BAŞLA!
                        </button>
                    </div>
                )}
                ...
                <div className="mb-4 flex justify-between items-center w-full opacity-90">
                    ...
                    <button onClick={generatePuzzle} className="text-white hover:text-yellow-400 font-bold transition-colors">Yeniden 🔄</button>
                </div>
                ...
                <div className="grid grid-cols-9 gap-[1px] p-[2px] bg-amber-900/40 rounded-xl shadow-[0_0_30px_rgba(245,158,11,0.2)] select-none">
                    {grid.map((row, r) => (
                        row.map((cell, c) => {
                            ...
                            const isSameChar = selectedCell && grid[selectedCell.r][selectedCell.c].value === cell.value && cell.value !== null;
                ...
                <div className="mt-8 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-lg">
                    {ALPHABET.map(char => (
                        <button key={char} onClick={() => handleCharInput(char)} className="bg-white/10 hover:bg-white/20 text-white font-black text-xl py-3 rounded-xl transition-all shadow-md hover:scale-105 active:scale-95">
                            {char}
                        </button>
                    ))}
                    <button onClick={() => handleCharInput(null)} className="bg-red-500/20 hover:bg-red-500/40 text-red-200 font-bold text-sm rounded-xl py-3 transition-all flex items-center justify-center">SİL</button>
                </div>
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/70 backdrop-blur-sm rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-yellow-500 to-amber-700 p-8 rounded-[32px] text-center shadow-[0_0_50px_rgba(245,158,11,0.5)] scale-110 border-4 border-white">
                            ...
                            <button onClick={() => onComplete(getStars())} className="bg-white text-amber-700 px-8 py-4 mt-6 rounded-full font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all w-full">
                                +{getStars()} ⭐ KAZAN
                            </button>
                        </div>
                    </div>
