...
const ShapeHuntGame: React.FC<ShapeHuntGameProps> = ({ onBack }) => {
  ...
  const renderShape = (shape: Shape) => {
    ...
    const baseClass = "absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all hover:scale-110 active:scale-95";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <button onClick={onBack} className="px-6 py-3 bg-red-600/90 hover:bg-red-500/90 text-white rounded-xl font-bold transition-all">
            ← Çıkış
          </button>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-cyan-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">1.</span> Ekranda gösterilen hedef şekli bul.</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">2.</span> Doğru şekle tıkla ve puan kazan!</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">3.</span> Yanlış şekle tıklarsan puan kaybedersin.</li>
              <li className="flex gap-2"><span className="text-cyan-400 font-bold">4.</span> 60 saniye içinde en yüksek puanı topla!</li>
            </ul>
            <button
