        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 text-white p-4">
            <div className="w-full max-w-3xl bg-slate-900/90 backdrop-blur-xl rounded-[40px] p-6 md:p-8 border border-indigo-500/30 shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">🧩 QUORIDOR</h2>
                ...
                <div className="flex justify-center gap-8 mb-4">
                    <div className="text-center"><div className="text-2xl">🔵</div><div className="text-sm">Duvar: {wallsLeft[1]}</div></div>
                    <div className="text-center"><div className="text-2xl">🔴</div><div className="text-sm">Duvar: {wallsLeft[2]}</div></div>
                </div>
                    ...
                    <div className="grid grid-cols-9 gap-1">
                        {Array(9).fill(0).map((_, r) =>
                            Array(9).fill(0).map((_, c) => (
                                <button key={`${r}-${c}`} onClick={() => movePawn(r, c)}
                                    className={`aspect-square rounded-lg flex items-center justify-center text-3xl transition-all ${
                                        p1Pos.row === r && p1Pos.col === c ? 'bg-blue-500' :
                                        p2Pos.row === r && p2Pos.col === c ? 'bg-red-500' :
                ...
                <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-4 text-center">
                    <p className="text-indigo-200 text-sm">🧩 Karşı tarafa ulaşın! Duvarlarla rakibinizi engelleyin!</p>
                </div>
            </div>
        </div>
    );
