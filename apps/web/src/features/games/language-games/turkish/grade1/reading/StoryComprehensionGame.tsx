...
const stories = [
  ...
  },
  {
    ...
    questions: [
      ...
      { q: "Ayşe nerede sallandı?", options: ["Salıncakta", "Kaydırakta", "Tahterevallide"], answer: 0 },
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-rose-500 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <button onClick={onExit} className="px-6 py-3 bg-white/20 backdrop-blur-md text-white rounded-xl font-bold hover:bg-white/30 transition-all">
            ← Çıkış
          </button>
        ...
        <div className="bg-white/20 backdrop-blur-md rounded-2xl p-4 mb-6">
          <div className="flex justify-between items-center mb-2">
            <span className="text-white font-bold">Hikaye {currentStory + 1}/{stories.length}</span>
            <span className="text-white font-bold">Soru {currentQuestion + 1}/{story.questions.length}</span>
          </div>
          <div className="w-full bg-white/30 rounded-full h-3">
            <div
              className="bg-gradient-to-r from-yellow-400 to-orange-500 h-3 rounded-full transition-all duration-500"
              style={{ width: `${((currentStory * 3 + currentQuestion + 1) / (stories.length * 3)) * 100}%` }}
            />
          </div>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          ...
          {showFeedback && (
            <div className={`mt-6 p-4 rounded-2xl text-center font-bold text-lg ${
              selectedAnswer === question.answer ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
            }`}>
              {selectedAnswer === question.answer ? '🎉 Doğru! Aferin!' : '❌ Yanlış! Doğru cevap: ' + question.options[question.answer]}
            </div>
          )}
        </div>
      </div>
