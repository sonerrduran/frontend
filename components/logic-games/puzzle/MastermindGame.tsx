...
const MastermindGame: React.FC<MastermindProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getConfig = () => {
        switch (difficulty) {
            case Difficulty.VERY_EASY: return { codeLength: 4, colorsCount: 4, maxGuesses: 10, allowDuplicates: false };
            case Difficulty.EASY: return { codeLength: 4, colorsCount: 6, maxGuesses: 10, allowDuplicates: true };
            case Difficulty.MEDIUM: return { codeLength: 4, colorsCount: 8, maxGuesses: 10, allowDuplicates: true };
            case Difficulty.HARD: return { codeLength: 5, colorsCount: 8, maxGuesses: 8, allowDuplicates: true };
            case Difficulty.VERY_HARD: return { codeLength: 6, colorsCount: 8, maxGuesses: 8, allowDuplicates: true };
            default: return { codeLength: 4, colorsCount: 6, maxGuesses: 10, allowDuplicates: true };
        }
    };
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            ...
                            <li className="flex gap-3">
                                ...
                                <div>Tahmininden sonraki her **Siyah Çivi (●)**, hem rengi hem de yeri doğru olan bir topu temsil eder.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-slate-100 font-bold border border-slate-600 rounded-full w-5 h-5 flex items-center justify-center text-xs mt-1">○</span>
                                <div>Her **Beyaz Çivi (○)**, rengi doğru ancak yanlış yere koyduğun bir topu temsil eder.</div>
                            </li>
                        ...
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-fuchsia-600 hover:bg-fuchsia-500 text-white shadow-lg active:scale-95">KODU KIR!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-3xl px-2 sm:px-4 py-4 md:py-8 h-full flex flex-col">
                ...
                <div className="flex justify-between items-center mb-6 bg-slate-800/50 p-4 rounded-3xl border border-white/5 backdrop-blur-md shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-600">Mastermind</h2>
                            <div className="text-xs md:text-sm font-bold text-slate-400 bg-slate-900/50 px-3 py-1 rounded-full mt-1 inline-block">ŞİFRE UZUNLUĞU: {config.codeLength}</div>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-sm text-slate-400 font-bold uppercase tracking-wider mb-1">Deneme Hakkı</div>
                        <div className="text-2xl font-black text-fuchsia-400">{config.maxGuesses - guesses.length}</div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex justify-center w-full max-h-[60vh] overflow-y-auto mb-6 scrollbar-hide">
                    <div className="bg-slate-800/80 p-4 sm:p-6 rounded-3xl shadow-2xl border-4 border-slate-700 w-full max-w-md flex flex-col gap-3">
                        ...
                        {guesses.map((guess, gIndex) => (
                            <div key={`guess-${gIndex}`} className="flex items-center gap-4 bg-slate-900/50 p-3 rounded-2xl border border-white/5">
                                <div className="text-slate-500 font-black w-6 text-right select-none">{gIndex + 1}</div>

                                <div className="flex flex-1 gap-2 sm:gap-3 justify-center">
                                    {guess.code.map((c, i) => (
                                        <div key={i} className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full shadow-[inset_0_-4px_8px_rgba(0,0,0,0.4)] ${COLORS[c]}`} />
                                    ))}
                                </div>
                                ...
                                <div className="w-10 h-10 sm:w-12 sm:h-12 grid grid-cols-2 gap-1 items-center justify-center p-1 bg-slate-950/50 rounded-lg">
                                    {Array.from({ length: config.codeLength }).map((_, i) => {
                                        ...
                                        if (i < guess.feedback.black) pegClass = "bg-slate-900 border border-black shadow-[inset_0_-2px_4px_rgba(0,0,0,0.5)]";
                                        else if (i < guess.feedback.black + guess.feedback.white) pegClass = "bg-white border border-slate-300 shadow-[inset_0_-2px_4px_rgba(0,0,0,0.2)]";

                                        return <div key={`peg-${i}`} className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full mx-auto ${pegClass}`} />
                                    })}
                                </div>
                            </div>
                        ))}
                                ...
                                <div className="flex flex-1 gap-2 sm:gap-3 justify-center">
                                    {currentGuess.map((c, i) => (
                                        <button
                                            ...
                                            `}
                                        >
                                            {c !== -1 && <span className="opacity-0 hover:opacity-100 text-white font-black text-shadow drop-shadow-md">X</span>}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {isGameOver && (
                            <div className="flex items-center gap-4 bg-slate-950 p-4 rounded-2xl border-2 border-dashed border-slate-600 mt-4">
                                <div className="text-slate-400 font-black text-center w-full mb-2 uppercase tracking-widest text-sm">Gizli Kod</div>
                                <div className="flex flex-1 gap-2 sm:gap-3 justify-center">
                                    {secretCode.map((c, i) => (
                                        <div key={`secret-${i}`} className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full shadow-[inset_0_-4px_8px_rgba(0,0,0,0.4)] ${COLORS[c]} animate-bounce`} style={{ animationDelay: `${i * 0.1}s` }} />
                                    ))}
                                </div>
                            </div>
                        )}
                ...
                {!isGameOver && (
                    <div className="bg-slate-800 p-4 sm:p-6 rounded-3xl border border-white/10 shadow-xl shrink-0">
                        <h3 className="text-center text-slate-400 font-bold mb-4 text-sm uppercase tracking-wider">Renk Seç</h3>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <p className="text-slate-300 mb-8 text-lg">
                            {hasWon ? `Gizli kodu ${guesses.length} denemede çözmeyi başardın. Sıkı işti! Kazanılan Yıldız: ${score}` : 'Deneme hakkın malesef tükendi. Gizli kodu bulamadık.'}
                        </p>

                        <div className="flex flex-col gap-3">
                            {hasWon && (
                                <button onClick={() => onComplete(score)} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-gradient-to-r from-purple-500 to-fuchsia-600 hover:from-purple-400 hover:to-fuchsia-500 text-white shadow-lg active:scale-95 text-transparent bg-clip-text text-white">Ödülü Al ⭐️</button>
                            )}
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-slate-600 hover:bg-slate-500 text-white shadow-lg active:scale-95">Yeni Şifre İste</button>
