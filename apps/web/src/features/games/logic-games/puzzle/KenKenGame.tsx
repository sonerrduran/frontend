...
const BOARDS: Record<number, BoardData> = {
    4: { // 4x4
        ...
        cages: [
            { id: 1, target: 8, op: '*', cells: [{ r: 0, c: 0 }, { r: 0, c: 1 }, { r: 1, c: 1 }] },
            { id: 2, target: 7, op: '+', cells: [{ r: 0, c: 2 }, { r: 0, c: 3 }] },
            { id: 3, target: 2, op: '-', cells: [{ r: 1, c: 0 }, { r: 2, c: 0 }] },
            { id: 4, target: 12, op: '*', cells: [{ r: 1, c: 2 }, { r: 1, c: 3 }, { r: 2, c: 3 }] },
            { id: 5, target: 6, op: '*', cells: [{ r: 2, c: 1 }, { r: 2, c: 2 }, { r: 3, c: 2 }] },
            { id: 6, target: 1, op: '-', cells: [{ r: 3, c: 0 }, { r: 3, c: 1 }] },
...
const KenKenGame: React.FC<KenKenGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        const initialGrid = Array(size).fill(0).map(() => Array(size).fill(0));
        setGrid(initialGrid);
    ...
    const getCageBorders = (r: number, c: number) => {
        const cage = boardData.cages.find(cg => cg.cells.some(cell => cell.r === r && cell.c === c));
        if (!cage) return '';

        let borders = '';
        const sameCageAbove = r > 0 && cage.cells.some(cell => cell.r === r - 1 && cell.c === c);
        const sameCageBelow = r < size - 1 && cage.cells.some(cell => cell.r === r + 1 && cell.c === c);
        const sameCageLeft = c > 0 && cage.cells.some(cell => cell.r === r && cell.c === c - 1);
        const sameCageRight = c < size - 1 && cage.cells.some(cell => cell.r === r && cell.c === c + 1);
        ...
        <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-blue-500/30 shadow-2xl relative overflow-hidden bounce-in">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-center flex-1">
                    <h2 className="text-2xl md:text-3xl font-black italic text-blue-400 drop-shadow-lg uppercase tracking-tight">KenKen</h2>
                    <div className="text-sm font-bold text-blue-200 mt-1 uppercase tracking-widest">{size}x{size} Izgara | Hata: {mistakes}</div>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setShowRules(true)} className="bg-blue-500/20 text-blue-300 border border-blue-500/50 hover:bg-blue-500/40 w-12 h-12 md:w-auto md:px-4 rounded-xl flex items-center justify-center transition-all font-black text-xl md:text-sm">
                        <span className="md:hidden">📖</span><span className="hidden md:inline">NASIL OYNANIR?</span>
                    </button>
                    <div className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-blue-400 text-blue-400'}`}>
                        {formatTime(timeLeft)}
                    </div>
                </div>
            </div>
            ...
            {showRules && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center rounded-[40px] animate-in fade-in zoom-in">
                    <div className="bg-slate-800/80 p-6 md:p-8 rounded-3xl border border-blue-500/30 max-w-md w-full shadow-2xl">
                        ...
                        <h3 className="text-2xl md:text-3xl font-black text-blue-400 mb-4 uppercase">Nasıl Oynanır?</h3>
                        <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base font-medium">
                            <li className="flex gap-2"><span className="text-blue-400">1.</span> Her satır ve sütunda 1'den {size}'e kadar olan rakamlar tam olarak bir kez yer almalıdır (Sudoku gibi).</li>
                            <li className="flex gap-2"><span className="text-blue-400">2.</span> Kalın çizgilerle çevrili alanlara **"Kafes"** denir.</li>
                            <li className="flex gap-2"><span className="text-blue-400">3.</span> Her kafesin sol üst köşesindeki sayı ve işlem (örn. 12*), o kafesin içindeki sayıların belirtilen işlem kullanılarak o hedef sayıyı vermesi gerektiğini söyler.</li>
                            <li className="flex gap-2"><span className="text-blue-400">4.</span> Kafes içindeki sayılar, kafes aynı satır/sütunda bükülmediği sürece kendisini tekrar edebilir!</li>
                        </ul>
                        <button
            ...
            {isGameOver && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-[40px]">
                    <h3 className={`text-5xl md:text-7xl font-black mb-4 drop-shadow-xl ${timeLeft === 0 ? 'text-red-500' : 'text-blue-400'}`}>
                        {timeLeft === 0 ? 'SÜRE BİTTİ' : 'MATEMATİK KAFESİ ÇÖZÜLDÜ!'}
                    </h3>
                    <p className="text-xl md:text-2xl text-white font-bold mb-8">Kazanılan Yıldız: <span className="text-blue-400">⭐ {score}</span></p>
