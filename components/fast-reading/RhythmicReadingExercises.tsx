...
const RhythmicReadingExercises: React.FC<{ onExit: () => void }> = ({ onExit }) => {
    ...
    const SAMPLE_TEXT = "Hızlı okuma teknikleri ile beyninizi daha verimli kullanabilir ve bilgiye ulaşma hızınızı artırabilirsiniz. Göz kaslarınızı güçlendirerek daha geniş alanları tek seferde görmeyi öğrenin.".split(' ');
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-10 bg-slate-900 rounded-[50px] border border-white/10 text-white text-center shadow-2xl animate-in slide-in-from-bottom duration-500">
            <div className="flex justify-between items-center mb-10">
                <button onClick={onExit} className="p-4 bg-white/5 rounded-2xl hover:bg-white/10 transition-all">⬅</button>
                <h2 className="text-3xl font-black italic uppercase tracking-tighter">Ritmik Okuma Çalışmaları</h2>
                <div className="w-12" />
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-10">
                {[
                    ...
                    <button
                        key={m.id}
                        onClick={() => { setMode(m.id as DrillMode); setIsPlaying(false); setIndex(0); setCurrentText(''); }}
            ...
            <div className="h-[400px] bg-black/40 rounded-[40px] border border-white/5 flex items-center justify-center p-12 relative overflow-hidden">
                {!isPlaying ? (
                    <button onClick={() => setIsPlaying(true)} className="bg-white text-slate-900 px-12 py-5 rounded-2xl font-black text-xl hover:scale-110 transition-all shadow-2xl">
                        ÇALIŞMAYI BAŞLAT ⚡
                    </button>
                ) : (
                    <div className="animate-in fade-in duration-150">
                        ...
                        <button onClick={() => setIsPlaying(false)} className="absolute bottom-6 right-6 p-4 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-2xl transition-all font-black text-[10px] uppercase border border-red-500/10">Durdur</button>
                    </div>
                )}
            </div>
            ...
            <div className="mt-10 flex flex-col md:flex-row items-center justify-center gap-8 px-10">
                ...
                <div className="p-6 bg-white/5 rounded-3xl border border-white/10 text-left flex-1">
                    <p className="text-white/60 text-sm leading-relaxed italic">
                        {mode === 'TAKISTOSKOP' ? 'Kelimeler çok kısa süre görünür. Kelimeyi okumaya değil, resim gibi algılamaya çalışın.' :
                            mode === 'BLOCK' ? 'Gözünüzü satır üzerinde değil, bloklar arasında sıçratarak okuyun.' :
