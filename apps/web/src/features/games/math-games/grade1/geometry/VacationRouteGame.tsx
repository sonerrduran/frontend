    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <button onClick={onBack} className="px-6 py-3 bg-red-600/90 hover:bg-red-500/90 text-white rounded-xl font-bold transition-all transform hover:scale-105">
            ← Çıkış
          </button>
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          <div className="bg-gradient-to-br from-green-500 via-emerald-500 to-green-600 rounded-2xl p-8 md:p-12 mb-8">
            <div className="flex justify-between items-center mb-6">
              <button onClick={() => setShowRules(true)} className="px-4 py-2 bg-green-700/40 hover:bg-green-600/40 border-2 border-green-300 text-white rounded-xl font-bold transition-all transform hover:scale-105">
                📖 NASIL OYNANIR?
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-green-700/40 rounded-2xl p-6 border-2 border-green-300">
                ...
                <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${gridSize}, 1fr)` }}>
                  {Array.from({ length: gridSize * gridSize }).map((_, idx) => {
                    ...
                    return (
                      <div key={idx} className={`aspect-square rounded-lg flex items-center justify-center text-3xl ${
                        isPlayer ? 'bg-blue-500' : isTarget ? 'bg-yellow-500' : 'bg-green-800/40 border border-green-300'
                      }`}>
                        {isPlayer && '🚗'}
                        {isTarget && '🏖️'}
                      </div>
              ...
              <div className="bg-green-700/40 rounded-2xl p-6 border-2 border-green-300">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div></div>
                  <button onClick={() => addDirection('up')} disabled={isMoving} className="p-4 bg-green-800/40 hover:bg-green-700/40 border-2 border-green-300 rounded-xl text-3xl disabled:opacity-50">⬆️</button>
                  <div></div>
                  <button onClick={() => addDirection('left')} disabled={isMoving} className="p-4 bg-green-800/40 hover:bg-green-700/40 border-2 border-green-300 rounded-xl text-3xl disabled:opacity-50">⬅️</button>
                  <div></div>
                  <button onClick={() => addDirection('right')} disabled={isMoving} className="p-4 bg-green-800/40 hover:bg-green-700/40 border-2 border-green-300 rounded-xl text-3xl disabled:opacity-50">➡️</button>
                  <div></div>
                  <button onClick={() => addDirection('down')} disabled={isMoving} className="p-4 bg-green-800/40 hover:bg-green-700/40 border-2 border-green-300 rounded-xl text-3xl disabled:opacity-50">⬇️</button>
                  <div></div>
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={executeRoute} disabled={isMoving || route.length === 0} className="flex-1 py-3 bg-yellow-500 hover:bg-yellow-400 text-white font-black rounded-xl disabled:opacity-50 disabled:cursor-not-allowed">
                    ▶️ Başlat
                  </button>
                  <button onClick={resetRoute} disabled={isMoving} className="flex-1 py-3 bg-red-500 hover:bg-red-400 text-white font-black rounded-xl disabled:opacity-50">
                    🔄 Sıfırla
                  </button>
                </div>
              </div>
            ...
            {feedback && (
              <div className={`mt-6 text-center text-2xl font-black p-6 rounded-xl ${
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-green-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-green-400 font-bold">1.</span> Yön butonlarına tıklayarak rota oluştur</li>
              <li className="flex gap-2"><span className="text-green-400 font-bold">2.</span> Arabayı (🚗) plaja (🏖️) götür</li>
              <li className="flex gap-2"><span className="text-green-400 font-bold">3.</span> Başlat butonuna bas ve rotayı izle</li>
              <li className="flex gap-2"><span className="text-green-400 font-bold">4.</span> Hedefe ulaş ve puan kazan!</li>
            </ul>
            <button onClick={() => setShowRules(false)} className="w-full bg-gradient-to-br from-green-500 via-emerald-500 to-green-600 hover:from-green-400 hover:to-emerald-500 text-white font-black py-4 rounded-xl transition-all transform hover:scale-105">
              ANLADIM, BAŞLA! 🚀
            </button>
          </div>
        </div>
