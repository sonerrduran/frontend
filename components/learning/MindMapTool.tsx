...
const MindMapTool: React.FC = () => {
    ...
    const addNode = () => {
        ...
        const newNode: Node = {
            ...
            x: 50 + Math.random() * 20,
            y: 50 + Math.random() * 20,
            color: ['bg-rose-500', 'bg-cyan-500', 'bg-indigo-500', 'bg-emerald-500', 'bg-amber-500'][Math.floor(Math.random() * 5)]
        };
        ...
        <div className="flex flex-col items-center justify-center min-h-[600px] p-4 text-white">
            <div className="w-full max-w-5xl bg-slate-900/80 backdrop-blur-3xl rounded-[40px] p-8 md:p-12 border border-white/10 shadow-2xl flex flex-col h-[700px]">
                ...
                <div className="text-center mb-8">
                    <h3 className="font-black tracking-widest text-xs uppercase mb-2 opacity-40">GÖRSEL HAFIZA HARİTASI</h3>
                ...
                <div className="flex-1 bg-black/30 rounded-[32px] border border-white/5 relative overflow-hidden cursor-crosshair">
                    ...
                    {nodes.length === 0 && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <p className="text-white/20 font-black text-xl uppercase italic tracking-widest">Henüz bir kavram yok. Yukarıdan ekle!</p>
                        </div>
                    )}
                        ...
                        <div
                            ...
                            onMouseDown={(e) => {
                                ...
                                const moveHandler = (moveEvent: MouseEvent) => {
                                    ...
                                    setNodes(curr => curr.map(n => n.id === node.id ? { ...n, x: initialX + dx, y: initialY + dy } : n));
                                };
                ...
                <p className="mt-6 text-center text-[10px] font-black opacity-30 uppercase tracking-widest italic">Kavramları istediğin yerlere sürükleyerek görsel bir yapı oluşturabilirsin.</p>
            </div>
        </div>
    );
};
