...
const NoiseMeter: React.FC<Props> = ({ onExit }) => {
    ...
    const startListening = async () => {
        try {
            ...
            setError("Mikrofon izni alınamadı. Lütfen tarayıcı ayarlarından mikrofon erişimine izin verin.");
            console.error(err);
        }
    };
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 relative">
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-8 md:p-12 rounded-[50px] shadow-2xl border border-white/10 w-full max-w-4xl flex flex-col items-center">
                ...
                {error ? (
                    ...
                    <>
                        {/* Visualizer */}
                        <div className={`relative w-full max-w-2xl h-64 bg-slate-950 rounded-[40px] flex items-end justify-center gap-2 p-8 mb-8 overflow-hidden transition-all duration-300 border-2 ${volume > 80 ? 'border-rose-500 shadow-[0_0_50px_rgba(225,29,72,0.4)]' : 'border-slate-800'}`}>
                        ...
                        <div className="flex items-center gap-6 mb-10">
                            ...
                            <div className="flex flex-col">
                                <span className={`text-5xl font-black ${statusColor.replace('bg-', 'text-')}`}>
                                    {volume > 0 ? Math.round(volume) : 0} <span className="text-xl text-white/50">dB(tahmini)</span>
                                </span>
                                <span className={`text-xl font-bold tracking-widest uppercase ${statusColor.replace('bg-', 'text-')} mt-1`}>
                                    {statusText}
                                </span>
                            </div>
                        </div>
