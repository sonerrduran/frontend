...
const DotSudokuGame: React.FC<DotSudokuGameProps> = ({ onBack }) => {
  ...
  const generatePuzzle = () => {
    ...
    const newSolution: number[][] = Array(6).fill(0).map(() => Array(6).fill(0));
    ...
    const newDots: DotType[][][] = Array(6).fill(0).map(() => 
      Array(6).fill(0).map(() => [null, null]) // [right dot, bottom dot]
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            <h1 className="text-3xl md:text-4xl font-black text-white">Dot Sudoku</h1>
            <p className="text-purple-200">Seviye {currentLevel} - {levels[currentLevel - 1].name}</p>
          </div>
          <div className="w-24"></div>
        </div>
              ...
              <div key={i} className="flex">
                {row.map((cell, j) => (
                  <div key={j} className="relative">
                    ...
                    {/* Right dot */}
                    {j < 5 && dots[i]?.[j]?.[0] && (
                      <div className={`absolute -right-2 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full z-10 ${
                        dots[i][j][0] === 'white' ? 'bg-white border-2 border-gray-800' : 'bg-gray-800'
                      }`} />
                    )}
                    
                    {/* Bottom dot */}
                    {i < 5 && dots[i]?.[j]?.[1] && (
                      <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full z-10 ${
                        dots[i][j][1] === 'white' ? 'bg-white border-2 border-gray-800' : 'bg-gray-800'
                      }`} />
                    )}
                  </div>
                ))}
