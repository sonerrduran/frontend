...
const NonConsecutiveSudokuGame: React.FC<NonConsecutiveSudokuGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
        ...
        <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-sky-600/30 shadow-2xl relative overflow-hidden bounce-in">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-center flex-1">
                    <h2 className="text-2xl md:text-3xl font-black italic text-sky-400 drop-shadow-lg uppercase tracking-tight">Ardışıksız Sudoku</h2>
                    <div className="text-xs font-bold text-sky-200 mt-1 uppercase tracking-widest">{size}x{size} Izgara | (Temel Sürüm) | Hata: {mistakes}/{maxMistakes}</div>
                </div>
                <div className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-sky-400 text-sky-400'}`}>
                    {formatTime(timeLeft)}
                </div>
            </div>
                ...
                >
                    ...
                    {board.map((row, r) =>
                        row.map((cell, c) => {
                            ...
                            const boxRows = size === 4 ? 2 : 2;
                            const boxCols = size === 4 ? 2 : 3;
                            const borderRight = (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-4 border-sky-600/80 mr-1' : 'border-r border-sky-900/50';
                            const borderBottom = (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-4 border-sky-600/80 mb-1' : 'border-b border-sky-900/50';
