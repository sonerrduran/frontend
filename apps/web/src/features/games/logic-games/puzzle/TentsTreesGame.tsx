...
const TentsTreesGame: React.FC<TentsTreesGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 6;
            default: return 5;
        }
    };
    ...
    const validateBoard = (currentBoard: Cell[][]) => {
        ...
        for (const tree of boardData.trees) {
            ...
            if (tree.r < size - 1 && currentBoard[tree.r + 1][tree.c].type === 'tent') hasAdjacentTent = true;
            if (tree.c > 0 && currentBoard[tree.r][tree.c - 1].type === 'tent') hasAdjacentTent = true;
            if (tree.c < size - 1 && currentBoard[tree.r][tree.c + 1].type === 'tent') hasAdjacentTent = true;
    ...
    return (
        <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-emerald-500/30 shadow-2xl relative overflow-hidden bounce-in">
            <div className="flex justify-between items-center mb-6">
                <button onClick={onExit} className="bg-white/10 hover:bg-white/20 w-12 h-12 rounded-xl flex items-center justify-center border border-white/10 transition-all font-black text-xl">🔙</button>
                <div className="text-center flex-1">
                    <h2 className="text-2xl md:text-3xl font-black italic text-emerald-400 drop-shadow-lg uppercase tracking-tight">Çadır ve Ağaç</h2>
                    <div className="text-sm font-bold text-emerald-200 mt-1 uppercase tracking-widest">{size}x{size} Kamp Alanı</div>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setShowRules(true)} className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 hover:bg-emerald-500/40 w-12 h-12 md:w-auto md:px-4 rounded-xl flex items-center justify-center transition-all font-black text-xl md:text-sm">
                        <span className="md:hidden">📖</span><span className="hidden md:inline">NASIL OYNANIR?</span>
                    </button>
                    <div className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-emerald-400 text-emerald-400'}`}>
                        {formatTime(timeLeft)}
                    </div>
                </div>
            </div>
                ...
                <div className="flex flex-col max-w-[450px] w-full mt-4 select-none">
                    ...
                    <div className="flex ml-[40px] md:ml-[50px] mb-2 justify-around">
                        {boardData.colClues.map((clue, idx) => {
                            ...
                            return (
                                <div key={`col-${idx}`} className={`w-8 h-8 md:w-12 md:h-12 flex items-center justify-center font-black text-xl md:text-2xl transition-all ${isSatisfied ? 'text-white/30' : isOver ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}>
                                    {clue}
                                </div>
                            );
                        })}
                    ...
                    <div className="flex">
                        ...
                        <div className="flex flex-col mr-2 justify-around">
                            {boardData.rowClues.map((clue, idx) => {
                                ...
                                return (
                                    <div key={`row-${idx}`} className={`w-8 h-8 md:w-10 md:h-12 flex items-center justify-center font-black text-xl md:text-2xl transition-all ${isSatisfied ? 'text-white/30' : isOver ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}>
                                        {clue}
                                    </div>
                                );
                            })}
                                ...
                                row.map((cell, c) => (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${cell.type === 'tree' ? 'bg-[#8fbc8f] cursor-default' :
                                                cell.type === 'tent' ? (cell.isError ? 'bg-red-500 ring-4 ring-red-700 animate-pulse' : 'bg-orange-500 shadow-[inset_0_4px_8px_rgba(255,255,255,0.4)]') :
                                                    'bg-[#9acd32] hover:bg-[#adff2f]'}
                                        `}
                                    >
                                        {cell.type === 'tree' && '🌲'}
                                        {cell.type === 'tent' && '🏕️'}
                                        {cell.type === 'crossed' && <div className="w-1/2 h-1/2 opacity-30 text-emerald-900 font-black flex items-center justify-center text-xl">✖</div>}
                                    </button>
                                ))
                            )}
                        </div>
            ...
            {showRules && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center rounded-[40px] animate-in fade-in zoom-in">
                    <div className="bg-slate-800/80 p-6 md:p-8 rounded-3xl border border-emerald-500/30 max-w-md w-full shadow-2xl">
                        ...
                        <h3 className="text-2xl md:text-3xl font-black text-emerald-400 mb-4 uppercase">Nasıl Oynanır?</h3>
                        <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base font-medium">
                            <li className="flex gap-2"><span className="text-emerald-400">1.</span> Amacın, ormandaki **her ağacın tam yanına (sağ/sol/alt/üst)** birer çadır kurmaktır.</li>
                            <li className="flex gap-2"><span className="text-emerald-400">2.</span> Satır ve sütunların dışındaki sayılar, o hizada toplam kaç tane çadır olması gerektiğini gösterir.</li>
                            <li className="flex gap-2"><span className="text-emerald-400">3.</span> Çadırlar birbirlerine **ÇAPRAZDAN BİLESİ DEĞEMEZ**!</li>
                            <li className="flex gap-2"><span className="text-emerald-400">4.</span> Bir kutuya ilk tıklayışta Çadır kurarsın 🏕️, ikinci tıklayışta Çimen olduğunu (✖) belirtirsin. Doğru sayıda çadırı doğru yerlere kur!</li>
                        </ul>
                        <button
            ...
            {isGameOver && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-[40px]">
                    <h3 className={`text-5xl md:text-7xl font-black mb-4 drop-shadow-xl ${timeLeft === 0 ? 'text-red-500' : 'text-emerald-400'}`}>
                        {timeLeft === 0 ? 'SÜRE BİTTİ' : 'ORMAN GÜVENDE!'}
                    </h3>
                    <p className="text-xl md:text-2xl text-white font-bold mb-8">Kazanılan Yıldız: <span className="text-emerald-400">⭐ {score}</span></p>
