...
const MiniSudokuGame: React.FC<Props> = ({ difficulty, grade, onComplete, onExit }) => {
    const [grid, setGrid] = useState<(number | null)[][]>(Array(6).fill(null).map(() => Array(6).fill(null)));
    const [initialGrid, setInitialGrid] = useState<boolean[][]>(Array(6).fill(null).map(() => Array(6).fill(false)));
    const [selectedCell, setSelectedCell] = useState<[number, number] | null>(null);

    useEffect(() => {
        const demoGrid = Array(6).fill(null).map(() => Array(6).fill(null));
        demoGrid[0][0] = 1; demoGrid[2][3] = 4; demoGrid[5][5] = 6;
        setGrid(demoGrid);

        const initFlags = Array(6).fill(null).map(() => Array(6).fill(false));
        initFlags[0][0] = true; initFlags[2][3] = true; initFlags[5][5] = true;
        ...
        <div className="w-full max-w-2xl mx-auto p-4 flex flex-col items-center">
            ...
            <div className="bg-slate-900/80 p-6 rounded-3xl border border-white/10 shadow-2xl w-full">
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-2 max-w-sm mx-auto">
                    {[1, 2, 3, 4, 5, 6].map(num => (
                        <button key={num} onClick={() => handleNumberClick(num)} disabled={!selectedCell} className="bg-emerald-500/20 hover:bg-emerald-500/40 disabled:opacity-50 text-emerald-100 font-bold py-3 md:py-4 rounded-xl text-xl shadow-lg transition-all active:scale-95 border border-emerald-500/30">
                            {num}
                        </button>
                    ))}
                </div>
