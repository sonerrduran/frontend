...
const ChaosSudokuGame: React.FC<Props> = ({ difficulty, onComplete, onExit }) => {
    ...
    const regionColors = [
        'bg-rose-900/40', 'bg-blue-900/40', 'bg-emerald-900/40',
        'bg-amber-900/40', 'bg-fuchsia-900/40', 'bg-cyan-900/40',
        'bg-indigo-900/40', 'bg-lime-900/40', 'bg-orange-900/40'
    ];
    ...
    const generatePuzzle = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            while (!maskSuccess) {
                mask = Array(9).fill(0).map(() => Array(9).fill(-1));
                        ...
                        for (let r = 0; r < 9; r++) {
                            for (let c = 0; c < 9; c++) {
                                if (mask[r][c] === regionId) {
                                    ...
                                    ].filter(n => n.r >= 0 && n.r < 9 && n.c >= 0 && n.c < 9 && mask[n.r][n.c] === -1);
                                    neighbors.push(...ns);
                                }
                            }
                        }
        ...
        setGrid(prev => {
            ...
            for (let i = 0; i < 9; i++) {
                for (let j = 0; j < 9; j++) {
                    ...
                    for (let x = 0; x < 9; x++) {
                        for (let y = 0; y < 9; y++) {
                            if ((x !== i || y !== j) && next[x][y].regionId === regId && next[x][y].value === val) {
                                next[i][j].hasError = true; next[x][y].hasError = true; anyError = true;
                            }
                        }
                    }
                }
    ...
    const getStars = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 200;
            default: return 60;
        }
    };
        ...
        <div className="flex flex-col items-center max-w-4xl mx-auto w-full">
            <div className="flex justify-between items-center w-full mb-6 relative px-4">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 text-center w-max">
                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-600 drop-shadow-lg uppercase tracking-wider">Chaos Sudoku</h2>
                </div>
                <button onClick={() => setShowRules(true)} className="p-3 bg-fuchsia-500/20 text-fuchsia-300 rounded-full hover:bg-fuchsia-500/30 transition-all">
                    ℹ️ Kurallar
                </button>
            </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl p-4 md:p-8 rounded-[40px] shadow-2xl border border-fuchsia-500/20 flex flex-col items-center relative overflow-hidden w-full max-w-2xl">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/95 p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
                        ...
                        <ul className="text-white/80 space-y-4 text-left max-w-md mb-8">
                            <li className="flex items-start gap-3">
                                <span className="text-xl">🌪️</span><span><b>Chaos Sudoku</b>, Jigsaw (Irregular) Sudoku'nun tam ve en zor 9x9 versiyonudur.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">1️⃣</span><span>Klasik Sudoku'daki 3x3 kare bloklar yerine, <b>karmaşık şekillere (renklere) sahip düzensiz bölgeler</b> vardır.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="text-xl">🛑</span><span>Her satırda, sütunda ve <b>her düzensiz kalın çizgili renkli bölgede</b> 1'den 9'a rakamlar tam olarak birer kez bulunmalıdır!</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="bg-gradient-to-r from-fuchsia-600 to-purple-800 text-white font-black py-4 px-12 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all text-xl">
                            ANLADIM, BAŞLA!
                        </button>
                    </div>
                )}
                ...
                <div className="mb-4 flex justify-between items-center w-full opacity-90">
                    ...
                    {isGenerating && <span className="text-purple-400 font-bold animate-pulse">Kaos Düzenleniyor...</span>}
                    <button onClick={generatePuzzle} disabled={isGenerating} className="text-white hover:text-fuchsia-400 font-bold transition-colors disabled:opacity-50">Yeniden 🔄</button>
                </div>
                    ...
                    {!isGenerating && grid.map((row, r) => (
                        row.map((cell, c) => {
                            ...
                            const isRelated = selectedCell &&
                                (selectedCell.r === r || selectedCell.c === c || grid[selectedCell.r][selectedCell.c].regionId === cell.regionId);
                ...
                <div className="mt-8 grid grid-cols-5 md:grid-cols-10 gap-2 w-full max-w-lg">
                    ...
                    <button onClick={() => handleNumInput(null)} className="bg-red-500/20 hover:bg-red-500/40 text-red-200 font-bold text-sm rounded-xl py-3 transition-all flex items-center justify-center">SİL</button>
                </div>
                ...
                {isWon && (
                    <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/70 backdrop-blur-sm rounded-[40px] animate-in fade-in zoom-in duration-500">
                        <div className="bg-gradient-to-b from-fuchsia-600 to-purple-900 p-8 rounded-[32px] text-center shadow-[0_0_50px_rgba(192,38,211,0.5)] scale-110 border-4 border-white">
                            ...
                            <button onClick={() => onComplete(getStars())} className="bg-white text-purple-900 px-8 py-4 mt-2 rounded-full font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all w-full">
                                +{getStars()} ⭐ KAZAN
                            </button>
                        </div>
                    </div>
