...
const JigsawSudokuGame: React.FC<Props> = ({ difficulty, grade, onComplete, onExit }) => {
    ...
    useEffect(() => {
        ...
        demoRegions[1][3] = 0; demoRegions[1][4] = 0;
        demoRegions[3][1] = 4; demoRegions[4][1] = 4;

        setRegions(demoRegions);

        const demoGrid = Array(9).fill(null).map(() => Array(9).fill(null));
        demoGrid[0][0] = 5; demoGrid[4][4] = 1; demoGrid[8][8] = 9;
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-4 flex flex-col items-center">
            <div className="w-full flex justify-between items-center mb-6">
                <button onClick={onExit} className="px-4 py-2 bg-white/10 text-white rounded-xl hover:bg-white/20 transition-colors">
                    ⬅ Çıkış
                </button>
                ...
                <div className="grid grid-cols-9 gap-1 bg-slate-800 p-2 rounded-xl mb-6 border border-slate-700">
                    {grid.map((row, r) =>
                        row.map((val, c) => {
                            ...
                            if (regions.length > 0) {
                                if (r > 0 && regions[r - 1][c] !== regionId) borderClasses += "border-t-[3px] border-t-white/60 ";
                                if (r < 8 && regions[r + 1][c] !== regionId) borderClasses += "border-b-[3px] border-b-white/60 ";
                                if (c > 0 && regions[r][c - 1] !== regionId) borderClasses += "border-l-[3px] border-l-white/60 ";
                                if (c < 8 && regions[r][c + 1] !== regionId) borderClasses += "border-r-[3px] border-r-white/60 ";
                            }
