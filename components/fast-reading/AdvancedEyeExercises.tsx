...
type AdvancedMode = '13_POINT' | 'HOURGLASS' | 'ZIGZAG_VERTICAL' | 'ZIGZAG_HORIZONTAL' | 'OPENING_VERTICAL' | 'OPENING_HORIZONTAL' | 'CIRCULAR';
...
const AdvancedEyeExercises: React.FC<{ onExit: () => void }> = ({ onExit }) => {
        ...
        const animate = () => {
            ...
            switch (mode) {
                ...
                case 'HOURGLASS':
                    ...
                    const hg = t % 4;
                    if (hg < 1) { // Top-Left to Bottom-Right
                        x = hg * maxX;
                        y = hg * maxY;
                    } else if (hg < 2) { // Bottom-Right to Top-Right
                        x = maxX;
                        y = maxY - (hg - 1) * maxY;
                    } else if (hg < 3) { // Top-Right to Bottom-Left
                        x = maxX - (hg - 2) * maxX;
                        y = (hg - 2) * maxY;
                ...
                case 'ZIGZAG_VERTICAL':
                    ...
                    const zv = t % 4;
                    if (zv < 1) { // Top-Left to Top-Right
                        x = zv * maxX;
                        y = 0;
                    } else if (zv < 2) { // Top-Right to Bottom-Left
                        x = maxX - (zv - 1) * maxX;
                        y = (zv - 1) * maxY;
                    } else if (zv < 3) { // Bottom-Left to Bottom-Right
                        x = (zv - 2) * maxX;
                        y = maxY;
                ...
                case 'ZIGZAG_HORIZONTAL':
                    ...
                    const zh = t % 4;
                    if (zh < 1) { // Top-Left to Bottom-Left
                        x = 0;
                        y = zh * maxY;
                    } else if (zh < 2) { // Bottom-Left to Top-Right
                        x = (zh - 1) * maxX;
                        y = maxY - (zh - 1) * maxY;
                    } else if (zh < 3) { // Top-Right to Bottom-Right
                        x = maxX;
                        y = (zh - 2) * maxY;
                    } else { // Bottom-Right to Top-Left
                        x = maxX - (zh - 3) * maxX;
        ...
        return (
            <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                ...
                {THIRTEEN_SEQUENCE.map((ptIdx, i) => {
                    ...
                    return (
                        <text key={i} x={midX} y={midY} fontSize="2" fill="currentColor" transform={`rotate(${angle}, ${midX}, ${midY})`} textAnchor="middle">
                            ▶
                        </text>
                    );
                })}
    ...
    const renderHourglassBg = () => (
        <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <path d="M5,5 L95,95 L95,5 L5,95 Z" fill="none" stroke="#2563eb" strokeWidth="0.5" strokeDasharray="2,2" />
        </svg>
    );
    ...
    const renderZigzagBg = (horizontal: boolean) => (
        <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            {horizontal ? (
                ...
                <path d="M5,5 L5,95 L95,5 L95,95" fill="none" stroke="#2563eb" strokeWidth="0.5" strokeDasharray="2,2" />
            ) : (
                // Zigzag Vertical: Top-Left -> Top-Right -> Bottom-Left -> Bottom-Right
                <path d="M5,5 L95,5 L5,95 L95,95" fill="none" stroke="#2563eb" strokeWidth="0.5" strokeDasharray="2,2" />
            )}
        </svg>
    );
    ...
    const renderCircularBg = () => (
        <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            <path d="M50,5 A45,45 0 1,1 50,95 A45,45 0 1,1 50,5" fill="none" stroke="#2563eb" strokeWidth="0.5" strokeDasharray="2,2" />
        </svg>
    );
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-slate-100/40 backdrop-blur-[4px] z-50 rounded-[32px] animate-in fade-in duration-500">
            <div className="bg-white p-12 rounded-[40px] shadow-2xl text-slate-800 text-center max-w-xl border border-slate-200/50">
                ...
                <h3 className="text-3xl font-black mb-4 uppercase tracking-tighter text-slate-700">
                    {mode === '13_POINT' ? '13 Nokta Göz Egzersizi' :
                        mode === 'CIRCULAR' ? 'PONTE: Dairesel Göz Egzersizi' :
                            mode.startsWith('OPENING') ? 'Açılan Nesneler' : 'Göz Egzersizi'}
                </h3>
                <p className="text-slate-500 text-sm mb-10 leading-relaxed font-medium px-6">
                    M5 Bilişim standartlarına göre optimize edilmiş bu rota, göz kaslarınızın esnekliğini artırır. Başınızı oynatmadan mavi noktayı takip edin.
                </p>
                <div className="flex gap-4 justify-center">
                    <button
                        onClick={() => { setIsPlaying(true); setTimer(60); timeRef.current = 0; }}
            ...
            <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between shadow-sm z-30">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">HIZLI OKUMA EGZERSİZLERİ</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex flex-col">
                        ...
                        <select value={timer} onChange={e => setTimer(Number(e.target.value))} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-black text-slate-600 outline-none focus:ring-2 ring-sky-500/20">
                            <option value={60}>1:00</option>
                            <option value={120}>2:00</option>
                        </select>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-400 uppercase ml-1 mb-1">Hız (ms)</span>
                        <select value={speed} onChange={e => setSpeed(Number(e.target.value))} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-black text-slate-600 outline-none focus:ring-2 ring-sky-500/20">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex flex-col items-end">
                        <span className="text-[9px] font-black text-rose-400 uppercase tracking-widest leading-none mb-1">CANLI SÜRE</span>
            ...
            <div className="flex-1 bg-slate-100 p-8 flex gap-8">
                ...
                <div className="flex-1 h-full bg-slate-300/50 rounded-[48px] border-4 border-white shadow-2xl relative overflow-hidden flex items-center justify-center group">
                    ...
                    {mode === '13_POINT' && (
                        <div className="w-full h-full relative p-12">
                            ...
                            {THIRTEEN_POINTS.map((p, i) => (
                                <div key={i} className="absolute flex flex-col items-center justify-center transform -translate-x-1/2 -translate-y-1/2" style={{ left: `${p.x}%`, top: `${p.y}%` }}>
                    ...
                    {mode.startsWith('OPENING') && isPlaying && (
                        <div className={`flex ${mode === 'OPENING_VERTICAL' ? 'flex-col' : 'flex-row'} gap-4 animate-in zoom-in duration-700`}>
                            {EMOJIS.map((e, i) => (
                                <div key={i} className="w-16 h-16 bg-white border-2 border-slate-200 rounded-2xl flex items-center justify-center text-3xl shadow-lg transform hover:scale-110 transition-all">
                                    {e}
                                </div>
                            ))}
                        </div>
                ...
                <div className="w-24 h-full flex flex-col gap-3 py-4">
                    {[
                        ...
                        <button
                            key={m.id}
                            onClick={() => { setMode(m.id as AdvancedMode); setIsPlaying(false); }}
                        ...
                        >
                            ...
                            <span className="text-[8px] font-black uppercase tracking-tighter text-center leading-none">{m.label}</span>
                        </button>
                    ))}
