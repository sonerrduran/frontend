...
const TakuzuGame: React.FC<TakuzuGameProps> = ({ onBack }) => {
  ...
  const generateValidSolution = (gridSize: number): (0 | 1)[][] => {
    const newSolution: (0 | 1)[][] = Array(gridSize).fill(0).map(() => Array(gridSize).fill(0));
  ...
  const isValidMove = (row: number, col: number, value: 0 | 1, currentGrid: (0 | 1 | null)[][]) => {
    ...
    // Yatay kontrol
    if (col >= 2 && currentGrid[row][col - 1] === value && currentGrid[row][col - 2] === value) return false;
    if (col >= 1 && col < size - 1 && currentGrid[row][col - 1] === value && currentGrid[row][col + 1] === value) return false;
    if (col < size - 2 && currentGrid[row][col + 1] === value && currentGrid[row][col + 2] === value) return false;

    // Dikey kontrol
    if (row >= 2 && currentGrid[row - 1][col] === value && currentGrid[row - 2][col] === value) return false;
    if (row >= 1 && row < size - 1 && currentGrid[row - 1][col] === value && currentGrid[row + 1][col] === value) return false;
    if (row < size - 2 && currentGrid[row + 1][col] === value && currentGrid[row + 2][col] === value) return false;
      ...
      <div className="max-w-4xl mx-auto">
        {showLevelSelect ? (
          ...
          <div className="text-center">
            <button onClick={onBack} className="mb-6 px-6 py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl font-bold transition-all">
              ⬅ GERİ DÖN
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
              {levels.map((lvl, idx) => (
                ...
                >
                  ...
                  <div className="text-xs opacity-60 mt-2">{lvl.size}x{lvl.size}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              <button onClick={() => setShowLevelSelect(true)} className="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl font-bold transition-all">
                ⬅ GERİ DÖN
              </button>
