...
const CaveGame: React.FC<CaveProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 7;
            default: return 6;
        }
    };
    ...
    const getTimeForDifficulty = () => {
        switch (difficulty) {
            case Difficulty.VERY_EASY:
            case Difficulty.EASY: return 600;
            case Difficulty.MEDIUM: return 900;
            case Difficulty.HARD:
    ...
    const initGame = useCallback(() => {
        ...
        const initial = Array(size).fill('empty').map(() => Array(size).fill('empty'));
    ...
    const checkWinCondition = (currentGrid: CellState[][]) => {
        ...
        if (isValid) {
            const wallVisited = Array(size).fill(false).map(() => Array(size).fill(false));

            const checkWallReachEdge = (startR: number, startC: number): boolean => {
                const q = [{ r: startR, c: startC }];
                const currentVisited = Array(size).fill(false).map(() => Array(size).fill(false));
            ...
            for (let r = 0; r < size; r++) {
                for (let c = 0; c < size; c++) {
                    ...
                    if (clue > 0) {
                        ...
                        // look left
                        for (let i = c - 1; i >= 0; i--) {
                            if (currentGrid[r][i] === 'cave') count++; else break;
                        }
                        // look right
                        ...
                        // look up
                        for (let i = r - 1; i >= 0; i--) {
                            if (currentGrid[i][c] === 'cave') count++; else break;
                        }
                        // look down
    ...
    const getRaycastFeedback = (r: number, c: number) => {
        ...
        let count = 1;
        // left
        for (let i = c - 1; i >= 0; i--) { if (grid[r][i] === 'cave') count++; else if (grid[r][i] === 'wall') break; }
        // right
        for (let i = c + 1; i < size; i++) { if (grid[r][i] === 'cave') count++; else if (grid[r][i] === 'wall') break; }
        // up
        for (let i = r - 1; i >= 0; i--) { if (grid[i][c] === 'cave') count++; else if (grid[i][c] === 'wall') break; }
        // down
        for (let i = r + 1; i < size; i++) { if (grid[i][c] === 'cave') count++; else if (grid[i][c] === 'wall') break; }
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                <span>🔦</span>
                                <div>Maden karanlıklarla dolu. Amacın, birbiriyle bağlantılı, tek parça bir **aydınlık mağara** bölgesi oluşturmak (Beyaz hücreler).</div>
                            </li>
                            <li className="flex gap-3 text-amber-400 font-bold">
                                <span>🔢</span>
                                <div>Sayılar, o hücreden sağa, sola, yukarı ve aşağıya doğru (karanlık duvara çarpana kadar) görülebilen toplam **aydınlık hücre sayısını** (kendi dahil) belirtir.</div>
                            </li>
                            <li className="flex gap-3 text-rose-400">
                                <span>🚫</span>
                                <div>Karanlık (Siyah) hücre grupları dışarı çıkacak bir **yola (tahta kenarına temas)** sahip olmalıdır. (Tam kapalı bir duvar halkası oluşamaz).</div>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-amber-600 hover:bg-amber-500 text-white shadow-lg shadow-amber-500/30 active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-4xl px-4 py-6 md:py-12 flex flex-col items-center">
                ...
                <div className="w-full flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        <button onClick={onExit} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all font-bold text-white relative z-50">⬅ Geri Dön</button>
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-600 tracking-wider">Cave</h2>
                    ...
                    <div className="text-right flex flex-col items-end gap-2 text-white">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-amber-300'}`}>
                ...
                <div className="bg-slate-700 p-2 sm:p-4 rounded-2xl shadow-xl border-4 border-slate-600">
                    <div className="flex flex-col bg-slate-800 rounded-lg overflow-hidden border-2 border-slate-800" style={{ width: 'fit-content' }}>
                        {grid.map((row, r) => (
                                ...
                                {row.map((cell, c) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {clue === 0 && cell === 'wall' && <span className="text-slate-800/20 text-xs">⬛</span>}
                                        </button>
                                    );
                                })}
                            </div>
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto shadow-lg">
                    <p className="text-slate-400 text-sm font-medium">Duvar (Siyah) için <span className="text-white font-bold cursor-pointer hover:text-amber-400">Tıkla</span>, Mağara (Beyaz) için tekrar tıkla.</p>
                </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Mağara Çöktü!' : timeLeft <= 0 ? 'Fener Söndü!' : 'Güvenli Çıkış!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Mağara kuralları ihlal edildi.' : timeLeft <= 0 ? 'Karanlıkta fazla kaldın.' : `Mükemmel bir harita çıkardın! Kazanılan Yıldız: ${score}`}
                        </p>

                        <div className="flex flex-col gap-3">
                            {mistakes < maxMistakes && timeLeft > 0 && (
                                <button onClick={() => onComplete(score)} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-white shadow-lg active:scale-95 text-transparent bg-clip-text text-white">Ödülü Al ⭐️</button>
                            )}
