...
const SkyscrapersGame: React.FC<SkyscrapersGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const rules = [
        "Her satıra ve sütuna 1'den " + size + "'e kadar tüm sayıları tam olarak birer kez yerleştirmelisin (Sudoku gibi).",
                    ...
                    <div className="relative inline-block">
                        <div className="flex ml-12 mb-2 w-fit">
                            {boardData.clues.top.map((c, i) => (
                                <div key={`t-${i}`} className={`w-12 md:w-16 h-8 flex items-end justify-center font-black text-xl ${c ? 'text-emerald-400' : 'text-transparent'}`}>{c}</div>
                            ))}
                        </div>
                        ...
                        <div className="flex">
                            <div className="flex flex-col mr-2">
                                {boardData.clues.left.map((c, i) => (
                                    <div key={`l-${i}`} className={`w-8 h-12 md:h-16 flex items-center justify-end font-black text-xl pr-2 ${c ? 'text-emerald-400' : 'text-transparent'}`}>{c}</div>
                                ))}
                            </div>
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded-xl font-black text-2xl md:text-3xl transition-all
                                                    ${isGiven ? 'bg-slate-700 text-slate-300 font-bold cursor-not-allowed shadow-[inset_0_2px_8px_rgba(0,0,0,0.5)]' :
                                                        isSelected ? 'bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.8)] scale-110 z-10' :
                            ...
                            <div className="flex flex-col ml-2">
                                {boardData.clues.right.map((c, i) => (
                                    <div key={`r-${i}`} className={`w-8 h-12 md:h-16 flex items-center justify-start font-black text-xl pl-2 ${c ? 'text-emerald-400' : 'text-transparent'}`}>{c}</div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex ml-12 mt-2 w-fit">
                            {boardData.clues.bottom.map((c, i) => (
                                <div key={`b-${i}`} className={`w-12 md:w-16 h-8 flex items-start justify-center font-black text-xl ${c ? 'text-emerald-400' : 'text-transparent'}`}>{c}</div>
                            ))}
                        </div>
                    </div>
                </div>
