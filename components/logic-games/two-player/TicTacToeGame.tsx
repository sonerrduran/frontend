    ...
    const checkWinner = () => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (board[a] && board[a] === board[b] && board[a] === board[c]) {
                ...
                setScores(prev => ({ ...prev, [board[a] as string]: prev[board[a] as keyof typeof prev] + 1 }));
                return;
            }
        }
    ...
    return (
        ...
        >
            ...
            <div className="flex gap-8 mb-8 justify-center">
                <div className={`flex flex-col items-center p-4 rounded-2xl transition-all bg-slate-800/60 backdrop-blur-md border border-white/20 ${xIsNext && !winner ? 'ring-4 ring-rose-400 scale-110' : 'opacity-60'}`}>
                ...
                <div className={`flex flex-col items-center p-4 rounded-2xl transition-all bg-slate-800/60 backdrop-blur-md border border-white/20 ${!xIsNext && !winner ? 'ring-4 ring-blue-400 scale-110' : 'opacity-60'}`}>
                    <span className="text-4xl">⭕</span>
            ...
            <div className="h-12 mb-6 flex items-center justify-center">
                {winner ? (
                    <div className={`text-2xl md:text-3xl font-black animate-bounce ${winner === 'X' ? 'text-rose-400' : winner === 'O' ? 'text-blue-400' : 'text-gray-400'}`}>
                        {winner === 'X' ? '❌ KAZANDI!' : winner === 'O' ? '⭕ KAZANDI!' : '🤝 BERABERE!'}
                    </div>
                ) : (
                    <div className="text-xl text-white/80 font-bold flex items-center gap-2">
                        Sıra: <span className={`text-2xl ${xIsNext ? 'text-rose-400' : 'text-blue-400'}`}>{xIsNext ? '❌' : '⭕'}</span>
                    </div>
                )}
            </div>
                ...
                <div className="grid grid-cols-3 gap-3 md:gap-4 bg-slate-900/40 backdrop-blur-sm p-4 md:p-6 rounded-3xl border border-white/20">
                    {board.map((cell, idx) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`transform transition-all duration-300 ${cell ? 'scale-100 rotate-0 opacity-100' : 'scale-50 rotate-90 opacity-0'}`}>
                                    {cell === 'X' ? 'X' : cell === 'O' ? 'O' : ''}
                                </div>
                            </button>
                        );
