...
const YajilinGame: React.FC<YajilinGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        setBlocks(Array(size).fill(0).map(() => Array(size).fill(0)));
        setHEdges(Array(size).fill(false).map(() => Array(size - 1).fill(false)));
        setVEdges(Array(size - 1).fill(false).map(() => Array(size).fill(false)));
        setMistakes(0);
    ...
    const checkWinCondition = (currentBlocks: number[][], currentH: boolean[][], currentV: boolean[][]) => {
        ...
        const degrees = Array(size).fill(0).map(() => Array(size).fill(0));
        ...
        while (q.length > 0) {
            ...
            if (cr > 0 && currentV[cr - 1][cc] && !visited[cr - 1][cc]) { visited[cr - 1][cc] = true; q.push([cr - 1, cc]); }
            if (cr < size - 1 && currentV[cr][cc] && !visited[cr + 1][cc]) { visited[cr + 1][cc] = true; q.push([cr + 1, cc]); }
            if (cc > 0 && currentH[cr][cc - 1] && !visited[cr][cc - 1]) { visited[cr][cc - 1] = true; q.push([cr, cc - 1]); }
            if (cc < size - 1 && currentH[cr][cc] && !visited[cr][cc + 1]) { visited[cr][cc + 1] = true; q.push([cr, cc + 1]); }
        }
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                <span>🔗</span>
                                <div>Oyun alanında <b>tek parça, kapalı bir döngü</b> çizmelisin.</div>
                            </li>
                            <li className="flex gap-3 text-purple-400 font-bold">
                                <span>⬛</span>
                                <div>Döngüye dahil olmayan hücreleri siyaha boyamalısın (Tıklayarak). <b>Siyah hücreler birbirine yatay veya dikey temas edemez!</b></div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-amber-400">🏹</span>
                                <div><b>İpuçları:</b> Hücrelerdeki oklar ve sayılar, ok yönünde <b>tam olarak kaç tane siyah hücre</b> olduğunu söyler.</div>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-purple-600 hover:bg-purple-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-3xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        <button onClick={onExit} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all font-bold relative z-50">⬅ Geri Dön</button>
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 tracking-wider">Yajilin</h2>
                            <div className="text-xs md:text-sm font-bold text-slate-400 bg-slate-900/50 px-3 py-1 rounded-full mt-1 inline-block">SEVİYE: {size}x{size}</div>
                        </div>
                    </div>
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            <span className="text-lg">⏱️</span>
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-purple-300'}`}>
                ...
                <div className="flex justify-center mb-8 relative">
                    ...
                    >
                        {/* Grid Background */}
                        <div className="absolute inset-4 grid" style={{ gridTemplateColumns: `repeat(${size}, 1fr)` }}>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`absolute w-full h-full flex items-center justify-center transition-colors
                                                ${clue ? 'bg-slate-300 cursor-default' :
                                                    isBlock ? 'bg-slate-800 cursor-pointer shadow-inner' :
                                                        'bg-transparent hover:bg-slate-300/30 cursor-pointer'}
                                            `}
                                            style={{
                                                top: `${r * cellSize}px`, left: `${c * cellSize}px`, width: `${cellSize}px`, height: `${cellSize}px`
                                            }}
                                            disabled={!!clue}
                                        >
                                            {clue && (
                                                <div className="flex flex-col items-center justify-center font-black text-slate-800">
                                                    ...
                                                    <span className={`text-md leading-none
                                                        ${clue.dir === 'U' ? 'rotate-[-90deg]' :
                                                            clue.dir === 'D' ? 'rotate-90' :
                                                                clue.dir === 'L' ? 'rotate-180' : ''}
                                                    `}>➔</span>
                                                </div>
                                            )}
                                        </button>
                            ...
                            {hEdges.map((row, r) =>
                                row.map((edge, c) => (
                                    <button
                                        ...
                                        style={{ top: `${r * cellSize + 10}px`, left: `${c * cellSize + cellSize / 2}px`, width: `${cellSize}px`, height: `30px` }}
                                    >
                                        <div className={`w-full h-2 rounded-full mx-auto transition-colors ${edge ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-transparent group-hover:bg-emerald-500/30'}`}></div>
                                    </button>
                                ))
                            )}
                        </div>
                            ...
                            {vEdges.map((row, r) =>
                                row.map((edge, c) => (
                                    <button
                                        ...
                                        className="absolute group outline-none pointer-events-auto flex items-center justify-center"
                                        style={{ top: `${r * cellSize + cellSize / 2}px`, left: `${c * cellSize + 10}px`, width: `30px`, height: `${cellSize}px` }}
                                    >
                                        <div className={`h-full w-2 rounded-full my-auto mx-auto transition-colors ${edge ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-transparent group-hover:bg-emerald-500/30'}`}></div>
                                    </button>
                                ))
                            )}
                        </div>
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Hücrenin içine tıklayarak siyah blok koyabilir, kenarlara tıklayarak yeşil döngü çizebilirsin.</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'İpucu veya Kural İhlali!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika Döngü!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Ok yanındaki siyah hücre sayısı uymuyor veya bloklar temas etti.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Yajilin zeka testini hatasız tamamladın! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-purple-600 hover:bg-purple-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
