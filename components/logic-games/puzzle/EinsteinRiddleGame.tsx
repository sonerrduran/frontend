...
const EinsteinRiddleGame: React.FC<EinsteinRiddleGameProps> = ({ onBack }) => {
  ...
  const [selectedCell, setSelectedCell] = useState<{ house: number; category: Category } | null>(null);
  ...
  const categories: { key: Category; label: string; options: string[] }[] = currentLevel === 1 ? [
    { key: 'nationality', label: 'Uyruk', options: ['İngiliz', 'İsveçli', 'Danimarkalı', 'Norveçli', 'Alman'] },
    ...
    { key: 'sport', label: 'Spor', options: ['Futbol', 'Basketbol', 'Tenis', 'Yüzme', 'Koşu'] }
  ] : currentLevel === 2 ? [
    { key: 'nationality', label: 'Meslek', options: ['Doktor', 'Öğretmen', 'Mühendis', 'Sanatçı', 'Avukat'] },
    { key: 'color', label: 'Araba', options: ['BMW', 'Audi', 'Toyota', 'Ford', 'Honda'] },
    { key: 'pet', label: 'Hobi', options: ['Resim', 'Müzik', 'Okuma', 'Bahçe', 'Yemek'] },
    { key: 'drink', label: 'Yemek', options: ['Pizza', 'Sushi', 'Kebap', 'Makarna', 'Salata'] },
    { key: 'sport', label: 'Renk', options: ['Kırmızı', 'Mavi', 'Yeşil', 'Sarı', 'Mor'] }
  ] : currentLevel === 3 ? [
    { key: 'nationality', label: 'İsim', options: ['Ali', 'Ayşe', 'Mehmet', 'Fatma', 'Can'] },
    { key: 'color', label: 'Yaş', options: ['20', '25', '30', '35', '40'] },
    { key: 'pet', label: 'Şehir', options: ['İstanbul', 'Ankara', 'İzmir', 'Bursa', 'Antalya'] },
    { key: 'drink', label: 'Meyve', options: ['Elma', 'Armut', 'Muz', 'Portakal', 'Üzüm'] },
    { key: 'sport', label: 'Müzik', options: ['Rock', 'Pop', 'Jazz', 'Klasik', 'Rap'] }
  ] : currentLevel === 4 ? [
    { key: 'nationality', label: 'Ülke', options: ['Türkiye', 'Fransa', 'İtalya', 'İspanya', 'Yunanistan'] },
    { key: 'color', label: 'Dil', options: ['Türkçe', 'Fransızca', 'İtalyanca', 'İspanyolca', 'Yunanca'] },
    { key: 'pet', label: 'Yemek', options: ['Kebap', 'Croissant', 'Pizza', 'Paella', 'Moussaka'] },
    { key: 'drink', label: 'İçecek', options: ['Çay', 'Şarap', 'Espresso', 'Sangria', 'Ouzo'] },
    { key: 'sport', label: 'Dans', options: ['Halay', 'Vals', 'Tango', 'Flamenco', 'Sirtaki'] }
  ] : [
    { key: 'nationality', label: 'Gezegen', options: ['Merkür', 'Venüs', 'Mars', 'Jüpiter', 'Satürn'] },
    { key: 'color', label: 'Element', options: ['Ateş', 'Su', 'Toprak', 'Hava', 'Metal'] },
    { key: 'pet', label: 'Kristal', options: ['Elmas', 'Yakut', 'Zümrüt', 'Safir', 'Ametist'] },
    { key: 'drink', label: 'Güç', options: ['Işık', 'Karanlık', 'Zaman', 'Uzay', 'Enerji'] },
    { key: 'sport', label: 'Sembol', options: ['Yıldız', 'Ay', 'Güneş', 'Yıldırım', 'Bulut'] }
  ];

  const clues = currentLevel === 1 ? [
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="border-2 border-white/30 p-2 bg-purple-600/50 text-white font-bold">Ev</th>
                  {categories.map(cat => (
                    <th key={cat.key} className="border-2 border-white/30 p-2 bg-purple-600/50 text-white font-bold text-sm">
                      {cat.label}
                    </th>
                  ))}
                </tr>
