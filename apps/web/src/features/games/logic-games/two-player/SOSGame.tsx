...
const SOSGame: React.FC<Props> = ({ onExit }) => {
    const [board, setBoard] = useState<CellData[][]>(Array(GRID_SIZE).fill(null).map(() => Array(GRID_SIZE).fill(null)));
    const [currentPlayer, setCurrentPlayer] = useState<Player>(1);
    const [selectedLetter, setSelectedLetter] = useState<Letter>('S');
    const [scores, setScores] = useState({ 1: 0, 2: 0 });
    const [sosLines, setSosLines] = useState<{ p1: { r: number, c: number }, p2: { r: number, c: number }, player: Player }[]>([]);
                ...
                if (isValid(r1, c1) && isValid(r2, c2)) {
                    if (board[r1][c1] === 'S' && board[r2][c2] === 'S') {
                        ...
                        if (!lineExists(newSosLines, r1, c1, r2, c2)) {
                            scored++;
                            newSosLines.push({ p1: { r: r1, c: c1 }, p2: { r: r2, c: c2 }, player: currentPlayer });
                        }
                    }
                }
            }
    ...
    const lineExists = (lines: { p1: any, p2: any }[], r1: number, c1: number, r2: number, c2: number) => {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4 md:p-6 bounce-in">
            <div className="flex items-center justify-between mb-8">
                ...
                <h2 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 italic drop-shadow-lg">SOS OYUNU</h2>
            </div>
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[40px] shadow-2xl grid md:grid-cols-[300px_1fr] gap-8">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-4">
                        <div className={`p-4 rounded-2xl border-2 transition-all flex items-center justify-between ${currentPlayer === 1 && !gameOver ? 'bg-orange-500/20 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)] scale-105' : 'bg-slate-800 border-white/10 opacity-70'}`}>
                        ...
                        <div className={`p-4 rounded-2xl border-2 transition-all flex items-center justify-between ${currentPlayer === 2 && !gameOver ? 'bg-cyan-500/20 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.3)] scale-105' : 'bg-slate-800 border-white/10 opacity-70'}`}>
                            <div className="flex items-center gap-3">
                                <span className="text-3xl">👽</span>
                    ...
                    {!gameOver && (
                        <div className="bg-slate-800/80 p-5 rounded-3xl border border-white/5">
                            <p className="text-white/60 text-xs font-black uppercase mb-3 text-center">Harf Seçimi (Sıra {currentPlayer}. Oyuncuda)</p>
                    ...
                    {gameOver && (
                        <div className="bg-emerald-500/20 border border-emerald-500/50 p-6 rounded-3xl text-center">
                            ...
                            <button onClick={resetGame} className="w-full py-3 bg-emerald-500 text-white font-black rounded-xl hover:bg-emerald-600 transition-colors">YENİ OYUN</button>
                        </div>
                    )}
                </div>
                ...
                <div className="flex justify-center items-center relative">
                    ...
                    >
                        {board.map((row, rIdx) =>
                            row.map((cell, cIdx) => (
                                ...
                                >
                                    {/* Letter Content */}
                                    <span className={`text-3xl sm:text-4xl md:text-5xl font-black ${cell ? 'animate-in zoom-in duration-200' : ''} ${cell === 'S' ? 'text-indigo-400' : 'text-rose-400'}`}>
                                        {cell}
                                    </span>
                    ...
                    <svg className="absolute inset-0 pointer-events-none w-full h-full z-10" style={{ padding: '8px' /* Match grid padding */ }}>
