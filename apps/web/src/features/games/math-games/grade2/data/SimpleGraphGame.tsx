...
const SimpleGraphGame: React.FC<SimpleGraphGameProps> = ({ onBack }) => {
  ...
  const generateQuestion = () => {
    ...
    const allOptions = [answer, ...wrongOptions.slice(0, 2)]
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-600 via-cyan-500 to-blue-500 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          ...
          <div className="bg-gradient-to-br from-yellow-100 to-orange-100 rounded-2xl p-8 mb-8 border-4 border-yellow-400">
            <div className="flex justify-around items-end h-64">
              <div className="flex flex-col items-center">
                <div className="bg-red-500 rounded-t-lg" style={{ width: '60px', height: `${data.apples * 20}px` }}></div>
                <div className="text-4xl mt-2">🍎</div>
                <div className="font-bold">{data.apples}</div>
              </div>
              <div className="flex flex-col items-center">
                <div className="bg-orange-500 rounded-t-lg" style={{ width: '60px', height: `${data.oranges * 20}px` }}></div>
                <div className="text-4xl mt-2">🍊</div>
                <div className="font-bold">{data.oranges}</div>
              </div>
              <div className="flex flex-col items-center">
                <div className="bg-yellow-500 rounded-t-lg" style={{ width: '60px', height: `${data.bananas * 20}px` }}></div>
                <div className="text-4xl mt-2">🍌</div>
            ...
            {options.map((option, index) => (
              <button
                ...
                className={`p-8 rounded-2xl font-black text-5xl transition-all ${
                  showFeedback
                    ? option === correctAnswer ? 'bg-green-500 text-white scale-105' : option === selectedAnswer ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-400'
          ...
          {showFeedback && (
            <div className={`mt-6 p-4 rounded-2xl text-center font-bold text-xl ${selectedAnswer === correctAnswer ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {selectedAnswer === correctAnswer ? '🎉 Doğru!' : `❌ Yanlış! Doğru cevap: ${correctAnswer}`}
            </div>
          )}
        </div>
      </div>
