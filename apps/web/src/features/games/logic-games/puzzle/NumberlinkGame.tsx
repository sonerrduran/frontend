...
const NumberlinkGame: React.FC<NumberlinkGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            case Difficulty.VERY_EASY:
            case Difficulty.EASY: return 4;
            default: return 5; // Simplified to 5x5 for now, logic works the same
        }
    };
    ...
    const checkWin = (currentPaths: PathNode[][]) => {
        ...
        for (const p of currentPaths) {
            ...
            if (pairStart && pairEnd && pairStart.color === pairEnd.color && (start.r !== end.r || start.c !== end.c)) {
        ...
        if (pair) {
            ...
            return (
                <div className="relative w-full h-full flex items-center justify-center">
                    {pathLine}
                    <div className={`relative z-10 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-black text-xl md:text-2xl shadow-lg border-2 border-slate-900 ${bgClass} text-white`}>
                        {pair.val}
                    </div>
                </div>
            )
    ...
    const getPathSVG = (node: PathNode, prev?: PathNode, next?: PathNode, colorClasses?: string) => {
        ...
        if (prev) {
            ...
            lines.push(
                <line key="prev" x1="50%" y1="50%" x2={`${50 + dx * 50}%`} y2={`${50 + dy * 50}%`} stroke={strokeColor} strokeWidth={strokeWidth} strokeLinecap="round" />
            );
        }
        if (next) {
            ...
            lines.push(
                <line key="next" x1="50%" y1="50%" x2={`${50 + dx * 50}%`} y2={`${50 + dy * 50}%`} stroke={strokeColor} strokeWidth={strokeWidth} strokeLinecap="round" />
            );
        }
        ...
        if (!prev && !next) {
            lines.push(
                <circle key="dot" cx="50%" cy="50%" r={strokeWidth / 2} fill={strokeColor} />
            )
        }
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <span>Aynı renk ve numaraya sahip daireleri birbirine <b>kesintisiz çizgilerle</b> bağla.</span>
                            </li>
                            <li className="flex gap-3 text-red-400 font-bold">
                                <span>🚫</span>
                                <span>Farklı çizgiler birbiriyle <b>kesişemez</b> veya üst üste gelemez.</span>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-slate-500">👆</span>
                                <span>Bir daireden basılı tutarak sürükle ve diğer eşine ulaştır. Düzeltmek için çizgiye tekrar dokun.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-rose-500 hover:bg-rose-400 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-lg px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-rose-400">Numberlink</h2>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <p className="text-slate-300 mb-8 text-lg">
                            {timeLeft <= 0 ? 'Zamanı iyi yönetemedin.' : `Tüm akışları mükemmel bağladın! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-rose-600 hover:bg-rose-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
