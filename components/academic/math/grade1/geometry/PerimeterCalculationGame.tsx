...
const PerimeterCalculationGame: React.FC<PerimeterCalculationGameProps> = ({ onComplete, onExit }) => {
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          <div className="bg-gradient-to-br from-lime-500 via-green-500 to-lime-600 rounded-2xl p-8 md:p-12 mb-8">
            ...
            <div className="bg-lime-700/40 rounded-2xl p-8 mb-8 border-2 border-lime-300">
              <p className="text-white text-2xl font-bold text-center mb-6">Bu dikdörtgenin çevresi kaç birimdir?</p>
                ...
                <div className="relative">
                  ...
                  >
                    {Array.from({ length: width * height }).map((_, idx) => (
                      <div key={idx} className="w-12 h-12 bg-lime-300/80 border-2 border-lime-500 rounded-lg" />
                    ))}
                  </div>
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 text-white font-bold text-lg">{width} birim</div>
                  <div className="absolute top-1/2 -translate-y-1/2 -left-12 text-white font-bold text-lg">{height} birim</div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-lime-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-lime-400 font-bold">1.</span> Ekranda bir dikdörtgen göreceksin</li>
              <li className="flex gap-2"><span className="text-lime-400 font-bold">2.</span> Kenarları say ve çevreyi hesapla</li>
              <li className="flex gap-2"><span className="text-lime-400 font-bold">3.</span> Çevre = 2 × (Genişlik + Yükseklik)</li>
              <li className="flex gap-2"><span className="text-lime-400 font-bold">4.</span> Doğru cevabı seç ve puan kazan!</li>
            </ul>
            <button
