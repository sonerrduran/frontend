...
const coins = [
  ...
  { value: 0.10, name: '10 Kuruş', image: '🪙' },
  { value: 0.25, name: '25 Kuruş', image: '🪙' },
  { value: 0.50, name: '50 Kuruş', image: '🪙' },
...
export default function CoinMatchGame({ onExit }: CoinMatchGameProps) {
  ...
  const generateQuestion = () => {
    ...
    const shuffled = [correct, ...wrongOptions.sort(() => Math.random() - 0.5).slice(0, 2)]
          ...
          {currentMoney && (
            <>
              <div className="bg-gradient-to-br from-yellow-500 to-amber-600 rounded-2xl p-12 mb-8 text-center">
                ...
                <p className="text-2xl text-white/90 mt-2">{currentMoney.value >= 1 ? `${currentMoney.value}₺` : `${currentMoney.value * 100} Kuruş`}</p>
              </div>
              ...
              {feedback && (
                <div className={`text-center text-2xl font-black p-4 rounded-xl ${
                  feedback.includes('✅') ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'
                }`}>
                  {feedback}
                </div>
              )}
