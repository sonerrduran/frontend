...
const ClassTimer: React.FC<Props> = ({ onExit }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 relative">
            ...
            <div className={`bg-slate-900/80 backdrop-blur-xl p-8 md:p-12 rounded-[50px] shadow-2xl border transition-all duration-500 w-full max-w-4xl flex flex-col items-center ${isTimeUp ? 'border-rose-500 shadow-[0_0_100px_rgba(244,63,94,0.4)]' : 'border-white/10'}`}>

                {/* Top Controls */}
                <div className="flex bg-slate-950 p-1 rounded-full mb-8">
                    <button
                        onClick={() => { setMode('TIMER'); setIsRunning(false); setTime(parseInt(inputMins) * 60 || 300); }}
                    ...
                    <button
                        onClick={() => { setMode('STOPWATCH'); setIsRunning(false); setTime(0); }}
                ...
                {/* Display */}
                <div className={`font-black text-[100px] md:text-[180px] leading-none tracking-tighter transition-colors duration-300 drop-shadow-2xl 
        ...
        `}>
                ...
                {mode === 'TIMER' && !isRunning && (
                    <div className="flex gap-2 mt-4 mb-4">
                        {[1, 3, 5, 10, 15, 30].map(m => (
                            <button key={m} onClick={() => setTimerMins(m)} className="w-12 h-12 rounded-full bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white font-bold transition-colors">
                                {m}'
                            </button>
                        ))}
                    </div>
                ...
                <div className="h-8 mb-6 mt-2">
                    {isTimeUp && <span className="text-rose-400 font-black text-2xl tracking-widest uppercase">SÜRE DOLDU!</span>}
                </div>
