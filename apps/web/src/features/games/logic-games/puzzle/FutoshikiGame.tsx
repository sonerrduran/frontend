...
const generateFutoshikiBoard = (size: number, difficulty: Difficulty): Cell[][] => {
    ...
    const fillSquare = (r: number, c: number): boolean => {
        ...
        let available = Array.from({ length: size }, (_, i) => i + 1)
            .filter(n => !usedInRow.has(n) && !usedInCol.has(n));
    ...
    while (placed < numInequalities) {
        ...
        if (dir === 'right' && c < size - 1 && cells[r][c].rightInequality === null) {
            cells[r][c].rightInequality = cells[r][c].targetValue > cells[r][c + 1].targetValue ? '>' : '<';
            placed++;
        } else if (dir === 'bottom' && r < size - 1 && cells[r][c].bottomInequality === null) {
            cells[r][c].bottomInequality = cells[r][c].targetValue > cells[r + 1][c].targetValue ? 'v' : '^';
            placed++;
        }
    }
...
const FutoshikiGame: React.FC<FutoshikiGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const handleInput = (num: number) => {
        ...
        if (newBoard[r][c].rightInequality === '>' && newBoard[r][c + 1].value && !(num > newBoard[r][c + 1].value!)) isObviouslyWrong = true;
        if (newBoard[r][c].rightInequality === '<' && newBoard[r][c + 1].value && !(num < newBoard[r][c + 1].value!)) isObviouslyWrong = true;
        if (c > 0 && newBoard[r][c - 1].rightInequality === '>' && newBoard[r][c - 1].value && !(newBoard[r][c - 1].value! > num)) isObviouslyWrong = true;
        if (c > 0 && newBoard[r][c - 1].rightInequality === '<' && newBoard[r][c - 1].value && !(newBoard[r][c - 1].value! < num)) isObviouslyWrong = true;

        if (newBoard[r][c].bottomInequality === 'v' && newBoard[r + 1]?.[c].value && !(num > newBoard[r + 1][c].value!)) isObviouslyWrong = true;
        if (newBoard[r][c].bottomInequality === '^' && newBoard[r + 1]?.[c].value && !(num < newBoard[r + 1][c].value!)) isObviouslyWrong = true;
        if (r > 0 && newBoard[r - 1][c].bottomInequality === 'v' && newBoard[r - 1][c].value && !(newBoard[r - 1][c].value! > num)) isObviouslyWrong = true;
        if (r > 0 && newBoard[r - 1][c].bottomInequality === '^' && newBoard[r - 1][c].value && !(newBoard[r - 1][c].value! < num)) isObviouslyWrong = true;
            ...
            <LogicGameWrapper
                ...
                infoCard={
                    <div className="flex gap-4">
                        ...
                        <div className={`text-center font-bold text-lg ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : 'text-blue-400'}`}>
