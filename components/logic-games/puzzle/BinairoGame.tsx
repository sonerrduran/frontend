...
const BOARDS: Record<number, BoardData> = {
    ...
    6: {
        size: 6,
        clues: [
            { r: 0, c: 0, val: 'one' }, { r: 0, c: 1, val: 'one' },
            { r: 1, c: 3, val: 'zero' }, { r: 1, c: 5, val: 'zero' },
            { r: 3, c: 0, val: 'zero' },
            { r: 5, c: 2, val: 'one' }, { r: 5, c: 4, val: 'one' },
        ]
    }
};
...
const BinairoGame: React.FC<BinairoGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const {
        ...
        resetGame,
        formatTime
    } = useLogicGame({ difficulty, onComplete, maxMistakes: difficulty === Difficulty.VERY_HARD ? 1 : difficulty === Difficulty.HARD ? 2 : 3 });
    ...
    const isValidPlacement = (currentBoard: CellState[][], r: number, c: number) => {
        ...
        if (c >= 1 && c < size - 1 && currentBoard[r][c - 1] === val && currentBoard[r][c + 1] === val) return false;
        if (c < size - 2 && currentBoard[r][c + 1] === val && currentBoard[r][c + 2] === val) return false;

        // Check Col
        if (r >= 2 && currentBoard[r - 1][c] === val && currentBoard[r - 2][c] === val) return false;
        if (r >= 1 && r < size - 1 && currentBoard[r - 1][c] === val && currentBoard[r + 1][c] === val) return false;
        if (r < size - 2 && currentBoard[r + 1][c] === val && currentBoard[r + 2][c] === val) return false;
            ...
            <LogicGameWrapper
                ...
                infoCard={
                    <div className="flex gap-4">
                        ...
                        <div className={`text-center font-bold text-lg ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : 'text-fuchsia-400'}`}>
            ...
            >
                    ...
                    >
                        {board.map((row, r) =>
                            row.map((cellState, c) => {
                                ...
                                if (cellState === 'zero') cellStyle = "bg-blue-500 text-white shadow-lg shadow-blue-500/50";
                                else if (cellState === 'one') cellStyle = "bg-rose-500 text-white shadow-lg shadow-rose-500/50";

                                if (isClue) {
                                    if (cellState === 'zero') cellStyle = "bg-blue-800 text-blue-200 cursor-not-allowed border-2 border-blue-500/50";
                                    else if (cellState === 'one') cellStyle = "bg-rose-800 text-rose-200 cursor-not-allowed border-2 border-rose-500/50";
                                }
                ...
                <div className="text-center bg-slate-900/50 p-4 rounded-xl">
                    <p className="text-white text-sm font-bold">
                        1 Tık: <span className="text-blue-200 bg-blue-600 px-2 rounded mx-1">0</span> | 
                        2 Tık: <span className="text-rose-200 bg-rose-600 px-2 rounded mx-1">1</span> | 
                        3 Tık: Boş
                    </p>
                </div>
            </LogicGameWrapper>
