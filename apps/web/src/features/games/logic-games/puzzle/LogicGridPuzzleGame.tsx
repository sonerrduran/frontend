...
const LogicGridPuzzleGame: React.FC<LogicGridPuzzleGameProps> = ({ onBack }) => {
  ...
  const initializePuzzle = () => {
    ...
    const newGrid: ('yes' | 'no' | '')[][] = Array(6).fill(0).map(() => Array(3).fill(''));
                ...
                <thead>
                  <tr>
                    ...
                    {categories.colors.map(color => (
                      <th key={color} className="border-2 border-white/30 p-2 bg-cyan-600/50 text-white font-bold text-sm">
                        {color}
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={name}>
                      ...
                      {[0, 1, 2].map(j => (
                        <td
                          ...
                          className={`border-2 border-white/30 p-3 cursor-pointer transition-all text-center text-2xl ${
                            getCellColor(grid[i][j], i, j)
                          }`}
                        >
                ...
                <thead>
                  <tr>
                    ...
                    {categories.fruits.map(fruit => (
                      <th key={fruit} className="border-2 border-white/30 p-2 bg-cyan-600/50 text-white font-bold text-sm">
                        {fruit}
                      </th>
                    ))}
                  </tr>
