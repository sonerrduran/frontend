...
const CAGE_COLORS = [
    'bg-red-500/30', 'bg-blue-500/30', 'bg-green-500/30',
    'bg-yellow-500/30', 'bg-purple-500/30', 'bg-pink-500/30', 'bg-cyan-500/30'
];
...
const generateCages = (board: number[][], size: number) => {
    const cages: Cage[] = [];
    const cageIdBoard = Array(size).fill(0).map(() => Array(size).fill(0));
    let cageIdCounter = 1;
            ...
            if (cageIdBoard[r][c] === 0) {
                ...
                for (let i = 0; i < cageSize; i++) {
                    if (cageIdBoard[currentR][currentC] === 0) {
                        ...
                        const neighbors = [
                            [currentR - 1, currentC], [currentR + 1, currentC],
                            [currentR, currentC - 1], [currentR, currentC + 1]
                        ].filter(([nr, nc]) => nr >= 0 && nr < size && nc >= 0 && nc < size && cageIdBoard[nr][nc] === 0);
                ...
                if (!valid || cells.length === 0) {
                    ...
                    cells.forEach(([cr, cc]) => { cageIdBoard[cr][cc] = 0; });
                    cells.length = 0;
                    cells.push([r, c]);
...
const KillerSudokuGame: React.FC<KillerSudokuGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
            ...
            <div className="w-full max-w-4xl bg-slate-800/80 backdrop-blur-xl rounded-[40px] p-1 border border-slate-700 shadow-2xl">
                ...
                <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[36px] p-8 relative overflow-hidden">
                    ...
                    <div className="absolute top-6 right-6 bg-slate-800/80 backdrop-blur-md rounded-2xl px-4 py-2 border border-white/20 flex gap-3">
                        ...
                        <div className={`text-sm font-bold ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : 'text-white/90'}`}>
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[700px] text-white pt-16">
                        ...
                        <div className="text-center mb-8">
                            ...
                            <p className="text-white/80 text-lg">{size}x{size} Izgara</p>
                        </div>
                        ...
                        >
                            {board.map((row, r) =>
                                row.map((cell, c) => {
                                    ...
                                    const boxRows = size === 4 ? 2 : 2;
                                    const boxCols = size === 4 ? 2 : 3;
                                    const borderRight = (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-2 border-r-white/40' : '';
                                    const borderBottom = (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-2 border-b-white/40' : '';

                                    const isClue = cage?.clueCell[0] === r && cage?.clueCell[1] === c;
                                    const colorClass = cage && cage.colorIndex !== undefined ? CAGE_COLORS[cage.colorIndex] : 'bg-transparent';
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className="z-10 text-white drop-shadow-md">{cell !== 0 ? cell : ''}</span>
                                        </button>
                                    );
                                })
                            )}
            ...
            {showRules && (
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6">
                    <div className="bg-slate-800/90 backdrop-blur-xl p-8 rounded-3xl border border-indigo-500/50 max-w-2xl w-full">
                        ...
                        <h3 className="text-3xl md:text-4xl font-black text-white mb-6 text-center">Nasıl Oynanır?</h3>
                        <ul className="text-white/90 text-left space-y-4 mb-8 text-base md:text-lg">
                            <li className="flex gap-3">
                                ...
                                <span>Normal Sudoku kuralları geçerlidir: Her satır ve her sütunda sayılar benzersiz olmalıdır.</span>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-indigo-400 font-black text-xl">2.</span>
                                <span>Izgara, renkli <span className="text-white font-bold">kafeslere (bölgelere)</span> ayrılmıştır.</span>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-indigo-400 font-black text-xl">3.</span>
                                <span>Bir kafesin köşesinde yazan küçük sayı, o kafesin içindeki hücrelerin <span className="text-white font-bold">TOPLAMINA</span> eşit olmalıdır.</span>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-indigo-400 font-black text-xl">4.</span>
                                <span>Bir kafes içerisinde aynı sayı birden fazla kez <span className="text-red-400 font-bold">KULLANILAMAZ.</span></span>
                            </li>
                        </ul>
                        <button
            ...
            {isGameOver && (
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6">
                    <div className="bg-slate-800/90 backdrop-blur-xl p-8 rounded-3xl border border-indigo-500/50 max-w-md w-full text-center">
                        <h3 className={`text-5xl md:text-6xl font-black mb-6 ${mistakes >= maxMistakes || timeLeft === 0 ? 'text-red-500' : 'text-yellow-400'}`}>
