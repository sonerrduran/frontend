...
const Tachistoscope: React.FC<{ onExit: () => void }> = ({ onExit }) => {
    ...
    const WORDS = ["Güneş", "Kitap", "Bilgi", "Hız", "Zeka", "Odak", "Başarı", "Öğrenme", "Akıl", "Mantık", "Bilişim", "Sistem", "Gelişim", "Teknik", "Pratik"];
    const SYMBOLS = ["◆", "▲", "●", "★", "■", "♣", "♠", "♥", "♦"];
    ...
    const generateContent = () => {
        ...
        return Array(3).fill(0).map(() => SYMBOLS[Math.floor(Math.random() * SYMBOLS.length)]).join(' ');
    };
        ...
        <div className="w-full h-screen bg-slate-900 flex flex-col font-sans overflow-hidden text-white">
            ...
            <div className="bg-slate-800 border-b border-white/10 px-8 py-4 flex items-center justify-between z-30 shadow-2xl">
                <div className="flex items-center gap-6">
                    <button onClick={onExit} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 transition-all font-black text-white/50 hover:text-white">⬅</button>
                    <div>
                        <h1 className="text-xl font-black uppercase tracking-tighter leading-none mb-1">Takistoskop</h1>
                        <p className="text-[10px] font-bold text-sky-400 uppercase tracking-widest">ANLIK ALGILAMA EGZERSİZİ</p>
                    </div>
                </div>

                <div className="flex items-center gap-6">
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-500 uppercase mb-1">Görünme Süresi (ms)</span>
                        <div className="flex items-center gap-3">
                            <input type="range" min="10" max="1000" step="10" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-32 accent-sky-500" />
                            <span className="text-xs font-black w-10">{duration}</span>
                        </div>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-500 uppercase mb-1">Tekrar Aralığı (sn)</span>
                        <select value={interval} onChange={e => setIntervalTime(Number(e.target.value))} className="bg-slate-700 border border-white/10 rounded-lg px-3 py-1.5 text-xs font-black outline-none">
            ...
            <div className="flex-1 relative flex items-center justify-center bg-black overflow-hidden">
                ...
                <div className={`text-7xl md:text-9xl font-black tracking-tighter transition-opacity duration-75 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                    {content}
                </div>
                ...
                {!isPlaying && (
                    <div className="absolute inset-0 flex items-center justify-center bg-slate-900/80 backdrop-blur-md z-40">
                        <div className="text-center">
                            ...
                            <h2 className="text-4xl font-black mb-4 uppercase tracking-tighter">Algılama Hızını Test Et</h2>
                            <p className="text-slate-400 max-w-md mx-auto mb-10 text-sm font-medium">Kelimeler anlık olarak merkezde belirecektir. <br />Gözlerinizi çizgilerin ortasına odaklayın.</p>
                            <button
