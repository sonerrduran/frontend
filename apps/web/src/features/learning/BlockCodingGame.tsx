        ...
        <div className="flex flex-col items-center justify-center min-h-[500px] p-4 text-white">
            <div className="w-full max-w-4xl bg-slate-900/80 backdrop-blur-3xl rounded-[40px] p-8 md:p-12 border border-white/10 shadow-2xl">
                <div className="text-center mb-8">
                    <h3 className="font-black tracking-widest text-[10px] uppercase mb-2 opacity-40">ALGORİTMA VE MANTIK</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div className="bg-black/20 p-4 rounded-3xl border border-white/5 relative aspect-square max-w-[400px] mx-auto w-full">
                        <div className="grid grid-cols-5 grid-rows-5 h-full w-full gap-1">
                            {Array.from({ length: 25 }).map((_, i) => {
                                ...
                                return (
                                    <div key={i} className={`rounded-lg flex items-center justify-center text-2xl transition-all border ${isRobot ? 'bg-indigo-500 scale-90 border-white shadow-[0_0_15px_rgba(255,255,255,0.5)]' : isGoal ? 'bg-emerald-500/20 border-emerald-500 animate-pulse' : 'bg-white/5 border-white/5'}`}>
                                        {isRobot ? '🤖' : isGoal ? '🏁' : ''}
                                    </div>
                                );
                            })}
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-white/5 p-6 rounded-2xl min-h-[120px] relative border border-white/5">
                            <label className="absolute -top-2 left-4 bg-slate-900 px-2 text-[10px] font-black tracking-widest uppercase opacity-40">KOMUT DİZİSİ</label>
                            <div className="flex flex-wrap gap-2">
                                {commands.map((cmd, i) => (
                                    <div key={i} className="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center shadow-lg animate-in slide-in-from-left-2">{cmd}</div>
                                ))}
                                {commands.length === 0 && <p className="text-white/20 italic text-sm">Henüz komut eklenmedi...</p>}
                            </div>
                        </div>
