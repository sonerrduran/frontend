...
const GraphCompletionGame: React.FC<Props> = ({ onComplete, onExit }) => {
  ...
  const [items, setItems] = useState<{ name: string; emoji: string; count: number; hidden: boolean }[]>([]);
  ...
  const generateLevel = () => {
    ...
    const selected = [...itemData].sort(() => Math.random() - 0.5).slice(0, numItems)
      .map(item => ({ ...item, count: Math.floor(Math.random() * (6 + level * 2)) + 2, hidden: false }));
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-5xl md:text-7xl font-black text-white mb-4">📈 Grafik Tamamlama 📈</h1>
        </div>

        <div className="bg-white/10 rounded-3xl p-8 mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
            {items.map((item, idx) => (
              <div key={idx} className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl p-6 text-center">
                ...
                <div className={`rounded-xl px-4 py-2 ${item.hidden ? 'bg-yellow-500/50' : 'bg-white/20'}`}>
                  <span className="text-white text-2xl font-black">{item.hidden ? '?' : item.count}</span>
                </div>
              </div>
            ))}
          </div>
