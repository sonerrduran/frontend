...
const TripleSudokuGame: React.FC<TripleSudokuGameProps> = ({ onBack }) => {
  ...
  const [selectedCell, setSelectedCell] = useState<{ grid: number; row: number; col: number } | null>(null);
  ...
  const generatePuzzle = () => {
    ...
    // Create puzzles
    const newGrids: number[][][] = newSolutions.map(sol => 
      sol.map(row => [...row])
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          {grids.map((grid, gridIdx) => (
            <div key={gridIdx} className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                    ...
                    {row.map((cell, j) => (
                      <button
                        ...
                        className={`w-8 h-8 md:w-9 md:h-9 border font-bold text-sm md:text-base transition-all ${
                          ...
                          selectedCell?.grid === gridIdx && selectedCell?.row === i && selectedCell?.col === j
                            ? 'ring-2 ring-yellow-400'
                            : ''
                        } ${
                          j % 3 === 2 && j < 8 ? 'border-r-2 border-r-gray-800' : 'border-r-gray-400'
                        } ${
                          i % 3 === 2 && i < 8 ? 'border-b-2 border-b-gray-800' : 'border-b-gray-400'
                        }`}
                      >
                        {cell !== 0 ? cell : ''}
                      </button>
