        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 relative">
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-[40px] shadow-2xl border border-white/10 w-full max-w-4xl flex flex-col md:flex-row items-center gap-12">
                ...
                <div className="flex flex-col gap-6 min-w-[250px] w-full md:w-auto">
                    <div>
                        <h2 className="text-4xl font-black text-rose-400 italic drop-shadow-lg mb-2">Nim Oyunu</h2>
                        <p className="text-white/60 text-sm leading-tight border-b border-white/10 pb-4">
                            ...
                            <span className="block mt-1 font-bold text-rose-300">Tahtadaki en son çubuğu alan kaybeder!</span>
                        </p>
                    </div>

                    <div className="flex gap-4">
                        {/* P1 */}
                        <div className={`flex-1 p-3 text-center rounded-2xl border-2 transition-all ${currentPlayer === 1 && !gameOver ? 'bg-indigo-600/30 border-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)] scale-105' : 'bg-slate-800/20 border-white/10 opacity-70'}`}>
                        ...
                        {/* P2 */}
                        <div className={`flex-1 p-3 text-center rounded-2xl border-2 transition-all ${currentPlayer === 2 && !gameOver ? 'bg-amber-500/30 border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.3)] scale-105' : 'bg-slate-800/20 border-white/10 opacity-70'}`}>
                    ...
                    {gameOver && (
                        <div className="bg-emerald-500/20 border border-emerald-500/50 p-6 rounded-3xl text-center mt-4">
                            ...
                            <button onClick={resetGame} className="w-full py-2 bg-emerald-500 text-white font-black rounded-xl hover:bg-emerald-600 transition-colors">YENİ OYUN</button>
                        </div>
                    )}
                    {!gameOver && (
                        <button onClick={resetGame} className="w-full mt-auto py-2 bg-slate-800 text-white/50 font-bold rounded-xl hover:bg-slate-700 hover:text-white transition-colors">
                            Yeniden Başlat
                        </button>
                    )}
                </div>
                ...
                <div className="flex-1 w-full bg-slate-950/50 p-8 rounded-3xl border border-slate-800 min-h-[400px] flex flex-col justify-center gap-8">
                    ...
                    `}
                     ...
                     `}>
                                            {/* Match Tip */}
                                            <div className={`w-full h-8 rounded-full ${isSelected ? 'bg-red-800' : 'bg-[#c15c5c]'} shadow-inner`}></div>
                            ...
                            {Array.from({ length: 7 - stickCount }).map((_, idx) => (
                                <div key={`empty-${idx}`} className="w-6 md:w-8 h-28 md:h-36 mx-1.5 opacity-0"></div>
                            ))}
                        </div>
                    ))}
                </div>
