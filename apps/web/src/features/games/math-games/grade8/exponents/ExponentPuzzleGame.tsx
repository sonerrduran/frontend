    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-600 via-orange-500 to-red-600 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
            ...
            {options.map((option, index) => (
              <button
                ...
                className={`p-6 rounded-2xl font-black text-4xl transition-all ${
                  showFeedback
                    ? option === getCorrectAnswer() ? 'bg-green-500 text-white scale-105' : option === selectedAnswer ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-400'
          ...
          {showFeedback && (
            <div className={`mt-6 p-4 rounded-2xl text-center font-bold text-xl ${selectedAnswer === getCorrectAnswer() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {selectedAnswer === getCorrectAnswer() ? '🎉 Mükemmel!' : `❌ Yanlış! Doğru: ${getCorrectAnswer()}`}
            </div>
          )}
        </div>
      </div>
