...
const ColorDetectiveGame: React.FC<Props> = ({ onComplete, onExit }) => {
  ...
  const colorData = [
    { name: 'Kırmızı', emojis: ['🍎', '🍓', '🌹', '❤️', '🔴'], bgClass: 'from-red-500 to-rose-600' },
    { name: 'Mavi', emojis: ['💙', '🔵', '🦋', '🐟', '🌊'], bgClass: 'from-blue-500 to-cyan-600' },
    { name: 'Yeşil', emojis: ['🍏', '🌿', '🌳', '💚', '🟢'], bgClass: 'from-green-500 to-emerald-600' },
    { name: 'Sarı', emojis: ['🌟', '🍋', '🌻', '💛', '🟡'], bgClass: 'from-yellow-500 to-amber-600' },
    { name: 'Turuncu', emojis: ['🍊', '🧡', '🟠', '🥕', '🦊'], bgClass: 'from-orange-500 to-amber-600' },
    { name: 'Mor', emojis: ['💜', '🟣', '🍇', '🦄', '🔮'], bgClass: 'from-purple-500 to-violet-600' }
  ];
  ...
  const generateLevel = () => {
    ...
    while (newObjects.length < numObjects) {
      const otherColor = colorData.filter(c => c.name !== target.name)[Math.floor(Math.random() * (colorData.length - 1))];
      newObjects.push({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-900 via-pink-900 to-purple-900 p-4 md:p-8 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <div key={i} className="absolute text-5xl opacity-10 animate-float"
            style={{
        ...
        <div className="text-center mb-12">
          ...
          <div className="bg-gradient-to-r from-pink-500 to-purple-500 rounded-3xl px-8 py-4 inline-block shadow-2xl">
            <p className="text-white text-3xl font-black">
              {targetColor} renkte olanları bul!
            </p>
          </div>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl mb-8">
          ...
          {feedback && (
            <div className={`mt-8 text-center text-3xl font-black ${
              feedback.includes('Harika') || feedback.includes('Doğru') ? 'text-green-400' : 'text-red-400'
            }`}>
              {feedback}
            </div>
          )}
