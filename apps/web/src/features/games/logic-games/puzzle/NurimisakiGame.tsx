...
const NurimisakiGame: React.FC<NurimisakiGameProps> = ({ onBack }) => {
  ...
  const generatePuzzle = () => {
    const size = 8;
    const newSolution: ('black' | 'white')[][] = Array(size).fill(0).map(() => Array(size).fill('white'));
    ...
    const newClues: (number | null)[][] = Array(size).fill(0).map(() => Array(size).fill(null));
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        if (newSolution[i][j] === 'black' && Math.random() > 0.5) {
          ...
          for (const [di, dj] of dirs) {
            let ni = i + di;
            let nj = j + dj;
            while (ni >= 0 && ni < size && nj >= 0 && nj < size && newSolution[ni][nj] === 'white') {
              count++;
              ni += di;
              nj += dj;
            }
    ...
    const newGrid: ('black' | 'white' | '')[][] = Array(size).fill(0).map(() => Array(size).fill(''));
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-900 via-blue-900 to-indigo-900 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <button onClick={onBack} className="px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-white font-bold transition-all backdrop-blur-sm border border-white/20">
            ⬅ GERİ DÖN
          </button>
        ...
        {showInstructions && (
          <div className="bg-blue-500/20 backdrop-blur-sm border border-blue-300/30 rounded-xl p-4 mb-6">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setShowInstructions(false)} className="text-white/70 hover:text-white ml-4">✕</button>
            </div>
          </div>
        )}
                  ...
                  <div key={j} className="relative">
                    <button
                      ...
                      className={`w-12 h-12 md:w-14 md:h-14 border-2 transition-all ${
                        cell === '' ? 'bg-white/20 hover:bg-white/30 border-gray-500' :
                        cell === 'black' ? (cell === solution[i][j] ? 'bg-gray-900 border-gray-700' : 'bg-red-900/50 border-red-700') :
                        (cell === solution[i][j] ? 'bg-white/90 border-gray-400' : 'bg-red-400/50 border-red-600')
                      }`}
                    />
                    {clues[i][j] !== null && (
        ...
        <button onClick={generatePuzzle} className="w-full py-4 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 rounded-xl text-white font-bold text-lg transition-all shadow-lg">
          🎲 Yeni Oyun
        </button>
        ...
        {isComplete && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
              ...
              <p className="text-white/90 mb-6">Nurimisaki'yi {formatTime(time)} sürede, {mistakes} hata ile tamamladınız!</p>
              <div className="flex gap-3">
                <button onClick={generatePuzzle} className="flex-1 py-3 bg-white text-orange-600 rounded-xl font-bold hover:bg-orange-50 transition-all">Yeni Oyun</button>
                <button onClick={onBack} className="flex-1 py-3 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-all">Menüye Dön</button>
              </div>
            </div>
          </div>
        )}
