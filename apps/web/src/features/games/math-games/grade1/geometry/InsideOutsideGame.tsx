...
const InsideOutsideGame: React.FC<InsideOutsideGameProps> = ({ onBack }) => {
  ...
  const generateLevel = () => {
    ...
    setItems(shuffled.map((item, idx) => ({
      id: idx,
      emoji: item.emoji,
      name: item.name,
  ...
  const handleZoneClick = (zone: 'inside' | 'outside') => {
    ...
    if (isCorrect) {
      const newItems = items.map(i =>
        i.id === selectedItem ? { ...i, currentZone: zone } : i
      );
      setItems(newItems);
      setScore(score + 10);
      setFeedback('✅ Doğru!');
          ...
          <div className="bg-gradient-to-br from-pink-500 via-rose-500 to-pink-600 rounded-2xl p-8 md:p-12 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              >
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {items.filter(i => i.currentZone === 'inside').map(item => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {items.filter(i => i.currentZone === 'outside').map(item => (
                    <div key={item.id} className="text-5xl">{item.emoji}</div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-pink-700/40 rounded-2xl p-6 border-2 border-pink-300">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                {items.filter(i => i.currentZone === 'none').map(item => (
                  <button
                    key={item.id}
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-pink-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-pink-400 font-bold">1.</span> Nesneleri doğru konuma yerleştir</li>
              <li className="flex gap-2"><span className="text-pink-400 font-bold">2.</span> Önce nesneyi seç, sonra içeride veya dışarıda alanına tıkla</li>
              <li className="flex gap-2"><span className="text-pink-400 font-bold">3.</span> Ev içi nesneler içeride, doğa nesneleri dışarıda olmalı</li>
              <li className="flex gap-2"><span className="text-pink-400 font-bold">4.</span> Tüm nesneleri doğru yerleştir ve seviyeyi tamamla!</li>
            </ul>
            <button
