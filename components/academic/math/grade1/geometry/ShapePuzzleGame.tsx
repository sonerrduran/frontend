...
const ShapePuzzleGame: React.FC<ShapePuzzleGameProps> = ({ onBack }) => {
  ...
  const renderShape = (shape: ShapeType, size: number = 60) => {
    ...
    switch (shape) {
      case 'circle':
        return <div className={`w-${size} h-${size} ${colors.circle} rounded-full`} style={{ width: size, height: size }} />;
      case 'square':
        return <div className={`w-${size} h-${size} ${colors.square} rounded-lg`} style={{ width: size, height: size }} />;
      case 'triangle':
        return (
          <div style={{
            width: 0,
            ...
            <div className="bg-purple-700/40 rounded-2xl p-6 mb-6 border-2 border-purple-300">
              <div className="grid grid-cols-3 gap-4">
                {slots.map(slot => (
                  ...
                  >
                    {slot.filled ? renderShape(slot.shape, 60) : (
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-purple-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2"><span className="text-purple-400 font-bold">1.</span> Boş yerlere uygun şekilleri yerleştir</li>
              <li className="flex gap-2"><span className="text-purple-400 font-bold">2.</span> Önce bir şekil seç, sonra boş yere tıkla</li>
              <li className="flex gap-2"><span className="text-purple-400 font-bold">3.</span> Doğru şekli seçersen puan kazanırsın</li>
              <li className="flex gap-2"><span className="text-purple-400 font-bold">4.</span> Tüm boşlukları doldur ve yapbozu tamamla!</li>
            </ul>
            <button
