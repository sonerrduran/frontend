...
const CatchWordGame: React.FC<CatchWordGameProps> = ({ stats, setMode }) => {
    ...
    const [fallingWords, setFallingWords] = useState<{ id: number; word: string; x: number; y: number }[]>([]);

    const wordBank = ['KİTAP', 'UZAY', 'BİLİM', 'ROKET', 'YILDIZ', 'DÜNYA', 'AKIL', 'BİLGİ', 'BAŞARI', 'ODAK', 'MANTIK', 'ZEKA'];
                    ...
                    {!isPlaying && (
                        <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center z-10">
                            <p className="text-white text-xl font-bold mb-6 text-center px-4 max-w-lg">
                                Yukarıdan düşen kelimeler arasından <strong>HEDEF</strong> kelimeyi bularak tıkla. Yanlış tıklamalar puan kaybettirir.
                            </p>
                            <button
                                onClick={startGame}
