...
const CrossSumsGame: React.FC<CrossSumsGameProps> = ({ onBack }) => {
  ...
  const [clues, setClues] = useState<{ row: number; col: number; sum: number; direction: 'h' | 'v' }[]>([]);
  ...
  const checkCompletion = (currentGrid: (number | null)[][]) => {
    const isFilled = currentGrid.every(row => 
      row.every(cell => cell !== null && cell !== -1)
    );
    if (isFilled) {
      setIsComplete(true);
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 mb-8">
            ...
            <p className="text-white/80 text-center mb-6">
              Toplamları kullanarak sayıları bul!
            </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-900 via-red-900 to-pink-900 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <button onClick={handleBackToLevels} className="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl font-bold transition-all">
            ⬅ GERİ DÖN
          </button>
          <div className="text-white text-center">
            <h2 className="text-3xl font-black">Cross Sums</h2>
            <p className="text-sm opacity-80">Seviye {currentLevel} - {levels[currentLevel - 1].name}</p>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 mb-4">
          <div className="grid gap-1 mb-4" style={{ gridTemplateColumns: `repeat(${grid.length}, 1fr)` }}>
                  ...
                  >
                    {isClue && clue ? (
                      <div className="absolute inset-0 flex flex-col items-end justify-start p-1">
                        ...
                        <span className="text-xs text-yellow-300">{clue.direction === 'h' ? '→' : '↓'}</span>
                      </div>
                    ) : (
                      cell || ''
                    )}
