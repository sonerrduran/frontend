...
const FlashMemoryGame: React.FC<FlashMemoryGameProps> = ({ stats, setMode }) => {
    ...
    const wordBank = [
        'ELMA', 'ARMUT', 'ÇİLEK', 'KİRAZ', 'KAVUN', 'KARPUZ', 'MUZ', 'ÜZÜM', 'LİMON', 'PORTAKAL',
        'KİTAP', 'MASA', 'KALEM', 'DEFTER', 'SİLGİ', 'OKUL', 'SINIF', 'ÖĞRETMEN', 'TAHTA', 'ÇANTA'
    ];
    ...
    return (
        <div className="w-full max-w-5xl mx-auto px-4 py-8 animate-in fade-in">
            <div className="flex items-center gap-4 mb-4">
                <button onClick={() => setMode(GameMode.FAST_READING_BRAIN_GAMES)} className="p-3 bg-white/10 rounded-xl hover:bg-white/20 transition-colors">
                    ⬅
                </button>
            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-white/10 rounded-[40px] p-6 md:p-12 mb-8 shadow-2xl">
                <div className="flex justify-between items-center mb-12">
                    ...
                    <div className="text-2xl font-black text-white">SKOR: <span className="text-emerald-400">{score}</span></div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center min-h-[40vh]">
                    {phase === 'WAITING' && (
                        <div className="text-center">
                            <p className="text-slate-400 text-xl font-medium mb-8 max-w-lg">
                                Ekranda beliren kelimeleri aklında tut. Süre bittiğinde, gösterilen kelimeleri seçenekler arasından işaretle.
                            </p>
                            <button
                                onClick={startRound}
                        ...
                        <div className="text-center animate-in zoom-in">
                            ...
                            <div className="flex gap-4 flex-wrap justify-center">
                                {targetWords.map((word, idx) => (
                                    <div key={idx} className="bg-white/10 px-6 py-4 rounded-2xl border-2 border-purple-500/50 text-white font-black text-3xl shadow-[0_0_20px_rgba(168,85,247,0.3)]">
                                        {word}
                                    </div>
                                ))}
                            </div>
                            {/* Tiny progress bar for time could go here */}
                            <div className="mt-8 text-xl animate-pulse text-rose-400 font-bold">Zaman Daralıyor...</div>
                        </div>
                    )}
                    ...
                    {phase === 'RECALL' && (
                        <div className="w-full text-center animate-in slide-in-from-bottom">
                            ...
                            <p className="text-slate-400 mb-8">Tam {targetWords.length} kelime seçmelisin. ({userSelection.length}/{targetWords.length})</p>
                                ...
                                {options.map((opt, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-4 rounded-2xl font-bold text-xl transition-all border-2 ${isSelected
                    ...
                    {phase === 'RESULT' && (
                        <div className="text-center animate-in bounce-in">
                            <div className={`text-4xl md:text-5xl font-black mb-4 ${resultMsg.includes('Mükemmel') ? 'text-emerald-400' : 'text-rose-400'}`}>
                                {resultMsg.split('!')[0]}!
                            </div>
