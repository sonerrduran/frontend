    ...
    return (
        <div className="flex flex-col min-h-[85vh] w-full p-4 relative">
            <div className="absolute top-4 left-4 z-20 flex gap-4">
                <button onClick={onExit} className="px-6 py-2 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors flex items-center gap-2 border border-slate-700 font-bold shadow-lg">
                    <span>⬅</span> Çıkış
                </button>
                <button onClick={addNote} className="px-6 py-2 bg-yellow-400 text-yellow-900 rounded-xl hover:bg-yellow-300 transition-colors flex items-center gap-2 font-black shadow-lg hover:scale-105">
                    + YENİ NOT EKLE
                </button>
            </div>
            ...
            >
                {notes.length === 0 && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-50">
                        <span className="text-4xl font-black text-white mix-blend-overlay">📌 Sol üstten yeni not ekleyin</span>
                    </div>
                )}
                    ...
                    >
                        ...
                        <div className="absolute top-2 right-2 flex gap-1 opacity-0 hover:opacity-100 transition-opacity z-20">
                            <div className="group relative">
                                <button className="w-6 h-6 bg-black/10 hover:bg-black/20 rounded-full flex items-center justify-center text-xs">🎨</button>
                                <div className="absolute right-0 top-full mt-1 bg-white p-2 rounded-xl shadow-xl flex gap-1 hidden group-hover:flex">
                                    {COLORS.map(c => (
                                        <button key={c} onClick={() => changeColor(note.id, c)} className={`w-6 h-6 rounded-full border-2 ${note.color === c ? 'border-black' : 'border-transparent'} ${c.split(' ')[0]}`}></button>
                                    ))}
                                </div>
                            </div>
                            <button onClick={() => deleteNote(note.id)} className="w-6 h-6 bg-red-500/20 hover:bg-red-500/40 rounded-full flex items-center justify-center text-red-900 font-bold text-xs">×</button>
                        </div>
                        ...
                        {note.isEditing ? (
                            ...
                            <div
                                ...
                                onClick={(e) => { e.stopPropagation(); toggleEditNote(note.id, true); }}
                                style={{ fontFamily: "'Indie Flower', 'Comic Sans MS', cursive" }}
                            >
                                {note.text || <span className="opacity-50 italic">Boş not (düzenlemek için tıkla)</span>}
                            </div>
                        )}

                        {/* Folded corner effect */}
                        <div className={`absolute bottom-0 right-0 w-8 h-8 rounded-tl-xl shadow-[-2px_-2px_4px_rgba(0,0,0,0.1)] bg-black/5`}></div>
                    </div>
                ))}
