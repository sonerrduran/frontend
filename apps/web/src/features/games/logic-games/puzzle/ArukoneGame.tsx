...
const ArukoneGame: React.FC<ArukoneGameProps> = ({ onBack }) => {
  ...
  const [endpoints, setEndpoints] = useState<Map<number, [[number, number], [number, number]]>>(new Map());
  ...
  const colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8', '#F7DC6F', '#E74C3C', '#9B59B6', '#3498DB'];
  ...
  const isEndpoint = (row: number, col: number) => {
    for (const [color, points] of endpoints.entries()) {
      if ((points[0][0] === row && points[0][1] === col) || 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
              {levels.map((lvl, idx) => (
                ...
                >
                  ...
                  <div className="text-xs opacity-60 mt-2">{lvl.size}x{lvl.size} • {lvl.pairs} çift</div>
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="grid gap-1 mb-4" style={{ gridTemplateColumns: `repeat(${size}, 1fr)` }}>
                {grid.map((row, rowIndex) => (
                  row.map((cell, colIndex) => {
                    ...
                    const isInCurrentPath = currentPath.some(([r, c]) => r === rowIndex && c === colIndex);
