...
const GreaterThanKillerSudokuGame: React.FC<GreaterThanKillerSudokuGameProps> = ({ onBack }) => {
  ...
  const [greaterThan, setGreaterThan] = useState<{ row: number; col: number; direction: 'right' | 'down' }[]>([]);
  ...
  const generatePuzzle = () => {
    const size = 6;
    const newSolution: number[][] = Array(size).fill(0).map(() => Array(size).fill(0));
    ...
    // Generate cages
    const colors = ['bg-red-200/40', 'bg-blue-200/40', 'bg-green-200/40', 'bg-yellow-200/40', 'bg-purple-200/40', 'bg-pink-200/40'];
    const newCages: { cells: [number, number][]; sum: number; color: string }[] = [];
    
    // Simple 2-cell cages
    for (let i = 0; i < 6; i++) {
      const cells: [number, number][] = [[i, 0], [i, 1]];
              ...
              <div key={i} className="flex">
                {row.map((cell, j) => (
                  <div key={j} className="relative">
                    ...
                    {greaterThan.find(gt => gt.row === i && gt.col === j && gt.direction === 'right') && (
                      <div className="absolute -right-2 top-1/2 -translate-y-1/2 text-red-600 font-bold text-xl z-10">
                        &gt;
                      </div>
                    )}
                    {greaterThan.find(gt => gt.row === i && gt.col === j && gt.direction === 'down') && (
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
              ...
              <p className="text-white/90 mb-6">
                Büyüktür Killer Sudoku'yu {formatTime(time)} sürede, {mistakes} hata ile tamamladınız!
              </p>
