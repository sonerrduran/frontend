...
const NonogramGame: React.FC<NonogramGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    const getSizeForDifficulty = () => {
        switch (difficulty) {
            ...
            case Difficulty.VERY_HARD: return 8;
            default: return 5;
        }
    };
                ...
                <div className="flex flex-col items-center justify-center text-white">
                    ...
                    <div className="flex flex-col items-center select-none bg-slate-900/40 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                        <div className="flex">
                            ...
                            <div className="w-16 md:w-20 bg-transparent flex items-end justify-end p-2 border-r-2 border-b-2 border-white/30">
                            </div>

                            {/* Col Clues */}
                            <div className="flex bg-slate-800/60 rounded-tr-xl border-b-2 border-white/30">
                                {colClues.map((clueArr, c) => (
                                    <div key={`col-${c}`} className="w-10 h-auto md:w-12 min-h-[80px] flex flex-col items-center justify-end p-1 gap-1 border-r border-white/20 text-xs md:text-sm font-black text-white">
                        ...
                        <div className="flex">
                            ...
                            <div className="flex flex-col bg-slate-800/60 rounded-bl-xl border-r-2 border-white/30">
                                {rowClues.map((clueArr, r) => (
                                    <div key={`row-${r}`} className="h-10 md:h-12 w-16 md:w-20 flex items-center justify-end p-2 gap-2 border-b border-white/20 text-xs md:text-sm font-black text-white">
                                        {clueArr.map((clue, idx) => (
                                            <span key={idx}>{clue}</span>
                                        ))}
                                    </div>
                                ...
                                {board.map((row, r) => (
                                    <div key={`gr-${r}`} className="flex">
                                        {row.map((cell, c) => {
                                            ...
                                            const borderR = c === 4 && size > 5 ? 'border-r-2 border-white/40' : 'border-r border-white/20';
                                            const borderB = r === 4 && size > 5 ? 'border-b-2 border-white/40' : 'border-b border-white/20';
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    `}
                                                >
                                                    {isCrossed && <span className="text-white/60 font-black text-xl select-none">✕</span>}
                                                </div>
                                            );
                                        })}
                                    </div>
