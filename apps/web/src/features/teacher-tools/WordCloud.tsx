...
const COLORS = [
    ...
    'text-rose-400', 'text-amber-400', 'text-emerald-400', 'text-indigo-400',
    'text-fuchsia-400', 'text-teal-400', 'text-orange-400', 'text-lime-400',
    'text-yellow-400', 'text-violet-400', 'text-sky-400', 'text-red-400',
];
...
const WordCloud: React.FC<Props> = ({ onExit }) => {
    ...
    const addWord = () => {
        ...
        setWords(prev => {
            ...
            if (existing) {
                return prev.map(w => w.text.toLowerCase() === trimmed.toLowerCase() ? { ...w, count: w.count + 1 } : w);
            }
