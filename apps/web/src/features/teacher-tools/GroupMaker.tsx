...
const GroupMaker: React.FC<Props> = ({ onExit }) => {
    const [namesText, setNamesText] = useState("Ahmet\nAyşe\nMehmet\nFatma\nAli\nVeli\nZeynep\nDeniz\nCan\nCanan");
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 relative">
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-[40px] shadow-2xl border border-white/10 w-full max-w-5xl flex flex-col md:flex-row gap-8 items-stretch">
                ...
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <p className="text-white/60 text-sm">Öğrenci listesini girin, otomatik takımlar oluşturun.</p>
                ...
                <div className="flex-[1.5] bg-slate-800/30 rounded-3xl border border-white/5 p-6 flex flex-col">
                    <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                        ...
                        <span className="text-sm bg-white/10 px-2 py-0.5 rounded-full text-white/50">{groups.length > 0 ? 'Oluşturuldu' : 'Bekleniyor...'}</span>
                    </h3>
                    ...
                    {groups.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto pr-2 custom-scrollbar">
                            {groups.map((team, idx) => (
                                <div key={idx} className="bg-slate-800 border border-slate-700 p-4 rounded-2xl shadow-lg relative overflow-hidden group">
                                    <div className="absolute -right-4 -top-4 text-7xl opacity-5 group-hover:opacity-10 transition-opacity">🏆</div>
                                    <h4 className="font-black text-emerald-300 mb-3 border-b border-white/10 pb-2">Takım {idx + 1}</h4>
