    ...
    const initGame = useCallback(() => {
        ...
        for (let r = 0; r < size; r++) {
            ...
            for (let c = 0; c < size; c++) {
                row.push({
                    r, c, type: 'white', clue: null, hasBulb: false, isIlluminated: false, isError: false
                });
            }
            newBoard.push(row);
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-white/70 text-lg">{size}x{size} Bölge</p>
                </div>
                            ...
                            >
                                {board.map((row, r) =>
                                    row.map((cell, c) => {
                                        if (cell.type === 'black') {
                                            return (
                                                <div key={`${r}-${c}`} className={`aspect-square rounded-sm shadow-inner flex items-center justify-center text-xl md:text-3xl font-black transition-all ${cell.isError ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-800 text-white/90'}`}>
                                        ...
                                        ${cell.hasBulb ? (cell.isError ? 'bg-red-500 ring-2 ring-red-700 animate-pulse' : 'bg-yellow-300 ring-4 ring-yellow-400 z-10 scale-105') :
                                            cell.isIlluminated ? 'bg-yellow-100 shadow-[inset_0_0_15px_rgba(250,204,21,0.5)]' : 'bg-white hover:bg-slate-100'}
                                    `}
                                >
                                    {cell.hasBulb ? '💡' : ''}
                                </button>
                ...
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
                    <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-yellow-500/30 max-w-md w-full">
                        ...
                        <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base font-medium">
                            <li className="flex gap-2"><span className="text-yellow-400">1.</span> Amacın, ızgaradaki tüm beyaz kareleri ampuller koyarak aydınlatmaktır.</li>
                            <li className="flex gap-2"><span className="text-yellow-400">2.</span> Ampuller yatay ve dikey yönde (siyah duvarlara çarpana kadar) ışık saçar. O karenin ışıklandığını göreceksin.</li>
                            <li className="flex gap-2"><span className="text-yellow-400">3.</span> Siyah hücrelerdeki sayılar, o siyah hücrenin sağına, soluna, altına ve üstüne toplam kaç ampul konulması gerektiğini gösterir.</li>
                            <li className="flex gap-2"><span className="text-yellow-400">4.</span> Bir ampul, başka bir ampulü aydınlatamaz! (İki ampul aynı satır/sütunda birbirini göremez, aksi halde kırılırlar).</li>
                        </ul>
                        <button
                ...
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
                    <div className="bg-slate-800/90 p-8 rounded-3xl border border-yellow-500/30 max-w-md w-full">
                        <h3 className={`text-5xl md:text-6xl font-black mb-4 ${mistakes >= maxMistakes || timeLeft === 0 ? 'text-red-500' : 'text-yellow-400'}`}>
                            {mistakes >= maxMistakes ? '❌ ÇOK HATA!' : timeLeft === 0 ? '⏰ SÜRE BİTTİ' : '💡 IŞIL IŞIL!'}
                        </h3>
