...
const READING_TEXTS = [
    {
        id: 1,
        title: 'Örnek Metin',
        content: 'Bu hızlı okuma egzersizi için örnek bir metindir. Kelimeler tek tek veya gruplar halinde gösterilecektir. Okuma hızınızı ayarlayabilir ve pratik yapabilirsiniz.',
        difficulty: 'easy'
    }
];
        ...
        <div className="w-full max-w-4xl mx-auto px-4 py-8 animate-in fade-in">
            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-white/10 rounded-[40px] p-8 md:p-12 mb-8 shadow-2xl">
                ...
                <div className="mt-8 text-center space-x-4">
                    <button onClick={() => { setIsPlaying(false); setCurrentIndex(0); }} className="px-6 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-bold transition-colors">
                        BAŞA SAR
                    </button>
                    <button onClick={() => setCurrentIndex(Math.max(0, currentIndex - 10))} className="px-6 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-bold transition-colors">
                        -10 Kelime
                    </button>
                    <button onClick={() => setCurrentIndex(Math.min(words.length - 1, currentIndex + 10))} className="px-6 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-bold transition-colors">
                        +10 Kelime
                    </button>
                </div>
            </div>
