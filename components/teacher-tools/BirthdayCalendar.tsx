                ...
                <div className="md:col-span-2 bg-slate-900/80 backdrop-blur-xl p-6 rounded-[32px] shadow-2xl border border-white/10">
                    <div className="flex items-center justify-between mb-4">
                        <button onClick={() => { const d = new Date(viewYear, viewMonth - 1); setViewMonth(d.getMonth()); setViewYear(d.getFullYear()); }}
                            className="w-9 h-9 rounded-xl bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-all">‹</button>
                        <h2 className="text-xl font-black text-white">{MONTHS_TR[viewMonth]} {viewYear}</h2>
                        <button onClick={() => { const d = new Date(viewYear, viewMonth + 1); setViewMonth(d.getMonth()); setViewYear(d.getFullYear()); }}
                            className="w-9 h-9 rounded-xl bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-all">›</button>
                    </div>
                    ...
                    <div className="grid grid-cols-7 mb-2">
                        {DAYS_TR.map(d => (
                            <div key={d} className="text-center text-white/30 text-xs font-bold py-1">{d}</div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        {cells.map((day, i) => {
                            ...
                            return (
                                <div key={i} title={bdays.map(s => s.name).join(', ')}
                                    ...
                                    <span className={`text-xs font-bold ${isToday ? 'text-purple-300' : 'text-white/70'}`}>{day}</span>
                                    {bdays.length > 0 && (
                                        <div className="flex flex-col items-center gap-0.5 mt-0.5">
                                            <span className="text-[10px]">🎂</span>
                                            {bdays.length > 1 && <span className="text-[8px] text-pink-300 font-bold">+{bdays.length}</span>}
                                        </div>
                                    )}
                                </div>
                            );
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {todayBirthdays.length > 0 && (
                        <div className="bg-gradient-to-br from-pink-500/20 to-rose-600/20 border border-pink-500/30 rounded-2xl p-4">
                            <h3 className="text-pink-300 font-black text-sm mb-2 flex items-center gap-1">🎉 Bugün Doğum Günü!</h3>
                            {todayBirthdays.map((s, i) => (
                                <div key={i} className="text-white font-bold text-sm bg-white/10 rounded-lg px-3 py-1.5 mb-1">{s.name} 🎂</div>
                            ))}
                        </div>
                    )}
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {upcoming.map(({ daysLeft, student, date }, i) => (
                                    <div key={i} className="flex items-center gap-2 p-2 bg-white/5 rounded-lg">
                                        <span className={`text-lg ${daysLeft === 0 ? 'animate-bounce' : ''}`}>{daysLeft === 0 ? '🎉' : '🎂'}</span>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-white text-xs font-bold truncate">{student.name}</div>
                                            <div className="text-white/40 text-[10px]">{date.getDate()} {MONTHS_TR[date.getMonth()]}</div>
                                        </div>
                                        <span className={`text-xs font-black px-2 py-0.5 rounded-full ${daysLeft === 0 ? 'bg-pink-500 text-white' : 'bg-white/10 text-white/50'}`}>
                        ...
                        <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4">
                            <div className="flex flex-col gap-2 mb-3">
                                <input value={newName} onChange={e => setNewName(e.target.value)}
                                    placeholder="Öğrenci adı" className="bg-slate-950 text-white p-2.5 rounded-lg border border-white/10 focus:border-pink-500 outline-none text-sm" />
                                ...
                                <button onClick={addStudent} className="py-2 bg-pink-500 hover:bg-pink-400 text-white rounded-lg font-bold text-sm transition-all">
                                    ➕ Ekle
                                </button>
                            </div>
                            <div className="space-y-1 max-h-40 overflow-y-auto">
                                {students.map((s, i) => {
                                    const d = parseDate(s.birthdate);
                                    return (
                                        <div key={i} className="flex items-center gap-2 p-1.5 bg-white/5 rounded-lg group">
                                            <span className="text-[10px] text-white/50 w-4">{i + 1}</span>
                                            <div className="flex-1 min-w-0">
                                                <div className="text-white text-xs truncate">{s.name}</div>
                                                <div className="text-white/30 text-[10px]">{d ? `${d.getDate()} ${MONTHS_TR[d.getMonth()]}` : ''}</div>
                                            </div>
                                            <button onClick={() => removeStudent(i)}
                                                className="opacity-0 group-hover:opacity-100 text-red-400 text-xs transition-all">✕</button>
                                        </div>
                                    );
                                })}
                            </div>
