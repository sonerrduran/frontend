...
const FastReadingDashboard: React.FC<FastReadingDashboardProps> = ({ stats, setMode }) => {
    ...
    if (wpm >= 500) { badge = "🔥 500 WPM Kulübü (Hız Ustası)"; badgeColor = "text-rose-500"; }
    else if (wpm >= 300) { badge = "⚡ Süper Okuyucu"; badgeColor = "text-orange-500"; }
    else if (wpm >= 200) { badge = "🚀 Gelişen Okuyucu"; badgeColor = "text-emerald-500"; }
    else if (wpm > 0) { badge = "🌱 Başlangıç"; badgeColor = "text-cyan-500"; }
            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 to-slate-900 border border-indigo-500/30 p-8 rounded-3xl text-center shadow-lg">
                        <h3 className="text-indigo-300 font-bold uppercase tracking-widest text-sm mb-4">En Yüksek WPM</h3>
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-white/10 p-8 rounded-3xl flex flex-col items-center justify-center shadow-lg">
                        <h3 className="text-slate-400 font-bold uppercase tracking-widest text-sm mb-4">Mevcut Seviye & Rozet</h3>
                        <div className={`text-2xl md:text-3xl font-black text-center ${badgeColor}`}>
                            {badge}
                        </div>
                        {wpm < 500 && wpm > 0 && (
                            <p className="text-slate-500 mt-4 text-sm font-medium">Sonraki rozet için {wpm < 200 ? 200 : wpm < 300 ? 300 : 500} WPM'e ulaş!</p>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5 space-y-4">
                    ...
                    <div className="h-40 flex items-end justify-between gap-2 px-2">
                        {[120, 150, 180, 175, 210, 240, wpm || 100].map((val, i, arr) => {
                            ...
                            return (
                                <div key={i} className="w-full bg-slate-700 rounded-t-lg relative group transition-all hover:bg-indigo-500" style={{ height: `${height}%` }}>
