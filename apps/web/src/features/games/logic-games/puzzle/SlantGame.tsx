...
const BOARDS: Record<number, BoardData> = {
    4: {
        size: 4,
        hints: [
            { r: 0, c: 0, val: 1 }, { r: 0, c: 3, val: 1 },
            { r: 1, c: 1, val: 1 }, { r: 1, c: 3, val: 2 }, { r: 1, c: 4, val: 1 },
            { r: 2, c: 0, val: 2 }, { r: 2, c: 2, val: 2 },
            { r: 3, c: 1, val: 3 }, { r: 3, c: 3, val: 2 },
            { r: 4, c: 1, val: 1 }, { r: 4, c: 4, val: 1 }
        ]
    },
    5: {
        size: 5,
        hints: [
            { r: 0, c: 1, val: 1 }, { r: 0, c: 4, val: 1 },
            { r: 1, c: 0, val: 1 }, { r: 1, c: 3, val: 2 }, { r: 1, c: 5, val: 1 },
            { r: 2, c: 2, val: 2 }, { r: 2, c: 4, val: 3 },
            { r: 3, c: 1, val: 3 }, { r: 3, c: 3, val: 2 },
            { r: 4, c: 0, val: 1 }, { r: 4, c: 2, val: 1 }, { r: 4, c: 5, val: 1 },
            { r: 5, c: 3, val: 1 }, { r: 5, c: 4, val: 1 }
        ]
    },
    6: {
        size: 6,
        hints: [
            { r: 0, c: 1, val: 1 }, { r: 0, c: 3, val: 2 }, { r: 0, c: 4, val: 1 },
            { r: 1, c: 0, val: 1 }, { r: 1, c: 2, val: 3 }, { r: 1, c: 5, val: 1 },
            { r: 2, c: 1, val: 2 }, { r: 2, c: 4, val: 2 }, { r: 2, c: 6, val: 1 },
            { r: 3, c: 2, val: 1 }, { r: 3, c: 5, val: 3 },
            { r: 4, c: 0, val: 1 }, { r: 4, c: 3, val: 4 }, { r: 4, c: 6, val: 0 },
            { r: 5, c: 1, val: 2 }, { r: 5, c: 4, val: 2 },
            { r: 6, c: 2, val: 1 }, { r: 6, c: 5, val: 1 }
        ]
    }
};
...
const SlantGame: React.FC<SlantGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const hasLoop = (currentBoard: number[][]) => {
        const parent = Array((size + 1) * (size + 1)).fill(0).map((_, i) => i);
    ...
    const getDegrees = (currentBoard: number[][]) => {
        const degrees = Array(size + 1).fill(0).map(() => Array(size + 1).fill(0));
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
                if (currentBoard[r][c] === 1) {
                    degrees[r][c]++;
    ...
    const getCurrentDegrees = useCallback(() => {
        if (!board.length) return Array(size + 1).fill(0).map(() => Array(size + 1).fill(0));
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Her boş karenin içine köşeden köşeye bir çapraz çizgi ( <span className="text-sky-400 font-bold">\</span> veya <span className="text-pink-400 font-bold">/</span> ) çizmelisin.</div>
                            </li>
                            <li className="flex gap-3">
                                <span className="text-amber-400">🎯</span>
                                <span>Köşelerdeki sayılar, o noktadan <b>kaç tane çizginin geçeceğini</b> (kaç çizginin ucunun o noktaya değeceğini) gösterir.</span>
                            </li>
                            <li className="flex gap-3 text-red-400 font-bold">
                                <span>🚫</span>
                                <span>Çizgiler kendi içinde asla <b>kapalı bir döngü (loop)</b> oluşturamaz! Zaten başlarken bile nerede döngü olacağını görüp önlemelisin.</span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-sky-600 hover:bg-sky-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-2xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-indigo-500">Slant</h2>
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-sky-300'}`}>
                                    ...
                                    >
                                        {cell === 1 && (
                                            <svg className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-md" viewBox="0 0 100 100" preserveAspectRatio="none">
                                                <line x1="0" y1="0" x2="100" y2="100" stroke="#0ea5e9" strokeWidth="8" strokeLinecap="round" />
                                            </svg>
                                        )}
                                        {cell === 2 && (
                                            <svg className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-md" viewBox="0 0 100 100" preserveAspectRatio="none">
                                                <line x1="100" y1="0" x2="0" y2="100" stroke="#ec4899" strokeWidth="8" strokeLinecap="round" />
                                            </svg>
                                        )}
                                    </button>
                                ))
                                ...
                                return (
                                    <div
                                        ...
                                        className={`absolute flex items-center justify-center font-black rounded-full shadow-lg transform -translate-x-1/2 -translate-y-1/2
                                            ${isExceeded ? 'bg-rose-500 text-white animate-pulse' :
                                                isSatisfied ? 'bg-emerald-500 text-white border-2 border-emerald-300' :
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-md mx-auto">
                    <p className="text-slate-400 text-sm font-bold">1 Tık: <span className="text-sky-400 font-black">\</span> | 2 Tık: <span className="text-pink-400 font-black">/</span> | 3 Tık: Boş</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Tahtada Hata Var!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Ya kapalı bir döngü yarattın ya da sayılarla çizgiler uyuşmadı.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Slant bulmacasını döngüye girmeden çözdün! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-sky-600 hover:bg-sky-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
