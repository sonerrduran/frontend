...
const SpinWheel: React.FC<Props> = ({ onExit }) => {
    const [items, setItems] = useState(['Ahmet', 'Ayşe', 'Mehmet', 'Fatma', 'Ali', 'Zeynep', 'Can', 'Selin']);
            ...
            <div className="w-full max-w-4xl mt-14 md:mt-0 flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex flex-col items-center gap-4">
                    <div className="relative">
                        <canvas ref={canvasRef} width={340} height={340} className="rounded-full shadow-2xl shadow-indigo-500/30" />
                    </div>
                    <button onClick={spin} disabled={isSpinning || items.length < 2}
                    ...
                    <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-5">
                        ...
                        {showEdit ? (
                            <>
                                ...
                                <button onClick={applyEdit} className="mt-2 w-full py-2 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl font-bold text-sm transition-all">
                                    ✅ Listeyi Güncelle
                                </button>
                            </>
                        ) : (
                            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
                                {items.map((item, i) => (
                                    <span key={i} className="px-3 py-1 rounded-full text-white text-xs font-bold"
                                        style={{ background: SEGMENT_COLORS[i % SEGMENT_COLORS.length] + '33', border: `1px solid ${SEGMENT_COLORS[i % SEGMENT_COLORS.length]}55` }}>
                                        {item}
                                    </span>
                                ))}
                            </div>
