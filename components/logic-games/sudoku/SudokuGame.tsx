            ...
            <div className="max-w-4xl mx-auto">
                            ...
                            >
                                {board.map((row, r) =>
                                    row.map((cell, c) => {
                                        ...
                                        const boxRows = size === 4 ? 2 : 2;
                                        const boxCols = size === 4 ? 2 : 3;
                                        const borderRight = (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-4 border-emerald-400' : '';
                                        const borderBottom = (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-4 border-emerald-400' : '';
                ...
                {isGameOver && (
                    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-3xl">
                        <h3 className={`text-5xl md:text-7xl font-black mb-4 ${mistakes >= maxMistakes || timeLeft === 0 ? 'text-red-500' : 'text-yellow-400'}`}>
