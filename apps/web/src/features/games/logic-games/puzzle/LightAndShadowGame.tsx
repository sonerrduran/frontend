...
const LightAndShadowGame: React.FC<LightAndShadowGameProps> = ({ onBack }) => {
  ...
  const generatePuzzle = () => {
    const size = 8;
    const newSolution: ('light' | 'shadow')[][] = Array(size).fill(0).map(() => Array(size).fill('light'));
    ...
    const newGrid: ('light' | 'shadow' | '')[][] = Array(size).fill(0).map(() => Array(size).fill(''));
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 overflow-x-auto">
          <div className="inline-block">
            ...
            <div className="flex ml-16">
              {colClues.map((clue, i) => (
                <div key={i} className="w-10 h-12 md:w-12 md:h-14 flex flex-col items-center justify-center text-xs">
