            ...
            <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between z-30 shadow-sm">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">AKTİF GÖRME ALANI EGZERSİZİ</p>
                    </div>
                </div>

                <div className="flex items-center gap-6">
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-400 uppercase mb-1">Genişleme Hızı</span>
                        <input type="range" min="1" max="8" value={speed} onChange={e => setSpeed(Number(e.target.value))} className="w-32 accent-orange-500" />
                    </div>
                    <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                        {(['CIRCLE', 'SQUARE', 'HEXAGON'] as const).map(s => (
                            <button key={s} onClick={() => setShape(s)} className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase transition-all ${shape === s ? 'bg-orange-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{s === 'CIRCLE' ? 'Daire' : s === 'SQUARE' ? 'Kare' : 'Altıgen'}</button>
                        ))}
                    </div>
                </div>

                <button onClick={() => setIsPlaying(!isPlaying)} className={`px-10 py-3 rounded-2xl font-black uppercase text-sm shadow-xl transition-all ${isPlaying ? 'bg-rose-500 text-white' : 'bg-orange-600 text-white'}`}>
                ...
                {!isPlaying && (
                    <div className="absolute inset-0 bg-white/40 backdrop-blur-sm z-40 flex items-center justify-center">
                        <div className="text-center bg-white p-12 rounded-[48px] shadow-2xl border border-slate-200 max-w-lg">
                            <h2 className="text-3xl font-black text-slate-700 mb-4 uppercase tracking-tighter">Görüş Alanını Genişlet</h2>
                            <p className="text-slate-500 text-sm mb-10 leading-relaxed font-medium">Kırmızı noktaya odaklanın ve başınızı oynatmadan kenarlara doğru genişleyen şekilleri takip edin. Periferik vizyonunuzu geliştirin.</p>
                            <button onClick={() => setIsPlaying(true)} className="bg-orange-600 text-white px-12 py-4 rounded-2xl font-black uppercase text-sm shadow-lg hover:scale-105 active:scale-95 transition-all">Egzersizi Başlat</button>
                        </div>
                    </div>
                )}
            </div>
