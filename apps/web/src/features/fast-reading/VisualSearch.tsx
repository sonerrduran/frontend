...
const VisualSearch: React.FC<{ onExit: () => void }> = ({ onExit }) => {
    ...
    const WORDS = ["GÜNEŞ", "DENİZ", "KİTAP", "BİLGİ", "ODAK", "RADAR", "KEMER", "METAL", "KALAN", "DÜNYA"];

    const generateGrid = () => {
        const size = 14;
        const newGrid = Array(size).fill(0).map(() =>
            Array(size).fill(0).map(() => CHARS[Math.floor(Math.random() * CHARS.length)])
        );

        const newTarget = difficulty === 'EASY' ? WORDS[Math.floor(Math.random() * WORDS.length)] :
        ...
        <div className="w-full h-screen bg-slate-50 flex flex-col font-sans overflow-hidden">
            <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between z-30 shadow-sm">
                <div className="flex items-center gap-6">
                    <button onClick={onExit} className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-black text-slate-500 hover:bg-slate-200">⬅</button>
                    <div>
                        <h1 className="text-xl font-black uppercase text-indigo-600">Kelime Arama</h1>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">GÖRSEL TARAMA EGZERSİZİ</p>
                    </div>
                </div>

                <div className="flex items-center gap-6">
                    <div className="bg-indigo-50 border border-indigo-200 px-6 py-2 rounded-2xl">
                        <span className="text-[10px] font-black text-indigo-400 uppercase block mb-1">Hedef Terim</span>
                        <span className="text-2xl font-black text-indigo-700 tracking-widest">{target || '---'}</span>
                    </div>
                    <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                        {(['EASY', 'HARD'] as const).map(d => (
                            <button key={d} onClick={() => { setDifficulty(d); setIsPlaying(false); }} className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase transition-all ${difficulty === d ? 'bg-indigo-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{d === 'EASY' ? 'Kelime' : 'Harf Grubu'}</button>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right">
                        <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">Bulunan</span>
                ...
                {!isPlaying && (
                    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md z-40 flex items-center justify-center">
                        <div className="text-center bg-white p-12 rounded-[48px] shadow-2xl border border-slate-200 max-w-lg">
                            <div className="w-20 h-20 bg-indigo-100 rounded-3xl flex items-center justify-center mx-auto mb-6 text-4xl">🔍</div>
                            <h2 className="text-3xl font-black text-slate-700 mb-4 uppercase tracking-tighter">Hızlıca Taramaya Başla</h2>
                            <p className="text-slate-500 text-sm mb-10 leading-relaxed font-medium">Karmaşık harf tablosu içinde yukarıda gösterilen hedef terimi bulun ve üzerine tıklayın. Gözlerinizin tarama hızını geliştirin.</p>
                            <button
                                onClick={() => { setIsPlaying(true); generateGrid(); setTimer(60); }}
