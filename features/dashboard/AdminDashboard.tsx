...
import { schoolAPI, classroomAPI, userAPI, timetableAPI, dutyAPI, curriculumAPI, questionAPI, analyticsAPI } from '../../services/api';
...
const DUTY_LOCATIONS = ['Bahçe', 'Koridor 1. Kat', 'Koridor 2. Kat', 'Koridor 3. Kat', 'Yemekhane', 'Otopark Girişi', 'Ana Giriş'];
            ...
            <header className="bg-[#1a1a2e]/80 backdrop-blur-lg border-b border-white/10 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={logout} className="px-4 py-2 rounded-lg bg-white/5 text-white/60 hover:bg-white/10 transition-all">
                            Çıkış 🚪
                        </button>
                    </div>
                </div>
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {[
                    { icon: '📚', label: 'Sınıflar', value: stats.classrooms, color: 'from-blue-500/20 to-blue-600/10' },
                    { icon: '👩‍🏫', label: 'Öğretmenler', value: stats.teachers, color: 'from-green-500/20 to-green-600/10' },
                    { icon: '👨‍🎓', label: 'Öğrenciler', value: stats.students, color: 'from-purple-500/20 to-purple-600/10' },
                    { icon: '📋', label: 'Müfredat', value: '—', color: 'from-orange-500/20 to-orange-600/10' },
                ].map((stat, i) => (
                    <div key={i} className={`bg-gradient-to-br ${stat.color} border border-white/10 rounded-xl p-5 text-center`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                    { icon: '📅', label: 'Ders Programı', desc: 'Haftalık program oluştur', tab: 'timetable' as Tab, color: 'hover:border-blue-500/50 hover:bg-blue-500/10' },
                    { icon: '🛎️', label: 'Nöbet Listesi', desc: 'Öğretmen nöbet planı', tab: 'duty' as Tab, color: 'hover:border-orange-500/50 hover:bg-orange-500/10' },
                    { icon: '📚', label: 'Müfredat', desc: 'Ders ve konu yönetimi', tab: 'curriculum' as Tab, color: 'hover:border-green-500/50 hover:bg-green-500/10' },
                    { icon: '❓', label: 'Soru Havuzu', desc: 'Soru bankası yönetimi', tab: 'questions' as Tab, color: 'hover:border-yellow-500/50 hover:bg-yellow-500/10' },
                    { icon: '👥', label: 'Kullanıcılar', desc: 'Öğretmen ve öğrenci yönetimi', tab: 'users' as Tab, color: 'hover:border-purple-500/50 hover:bg-purple-500/10' },
                    { icon: '📈', label: 'Analitik', desc: 'Okul bazlı raporlar', tab: 'analytics' as Tab, color: 'hover:border-cyan-500/50 hover:bg-cyan-500/10' },
                ].map((a, i) => (
        ...
        <>
            ...
            {showCreate && (
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 flex gap-3">
                    <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Program adı (örn: 2025-2026 Güz)"
                    ...
                    <button onClick={createTimetable} className="px-4 py-2 rounded-lg bg-cyan-500 text-white hover:bg-cyan-600 transition-all">Oluştur</button>
                    <button onClick={() => setShowCreate(false)} className="px-4 py-2 rounded-lg bg-white/10 text-white/60 hover:bg-white/20 transition-all">İptal</button>
                </div>
            )}
            ...
            {timetables.length > 0 && (
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                    {timetables.map(t => (
                        <button key={t.id} onClick={() => loadTimetable(t.id)}
                            ...
                            <span onClick={(e) => { e.stopPropagation(); deleteTimetable(t.id); }}
                                className="ml-1 text-red-400/60 hover:text-red-400 cursor-pointer">✕</span>
                        </button>
                    ))}
                </div>
            )}
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-[#1a1a2e]/80">
                                    <th className="px-3 py-3 text-left text-white/60 text-xs font-medium w-24">Saat</th>
                                    {DAYS.map((d, i) => (
                                        <th key={i} className="px-3 py-3 text-center text-white/80 text-sm font-semibold">{d}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                                ...
                                                <td key={di} className="px-2 py-1 text-center border-l border-white/5">
                                                    {slot ? (
                                                        <div className="bg-gradient-to-br from-cyan-500/15 to-purple-500/15 rounded-lg p-2 relative group">
                                                            ...
                                                            <div className="text-white text-xs font-medium mt-0.5">{slot.subject?.name}</div>
                                                            <div className="text-white/40 text-[10px]">{slot.classroom?.name}</div>
                                                            <button onClick={() => deleteSlot(slot.id)}
                                                                className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500/80 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">✕</button>
                                                        </div>
                                                    ) : (
                                                        <button onClick={() => setAddSlot({ day: di + 1, period: p.period })}
            ...
            {/* Add Slot Modal */}
            {addSlot && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setAddSlot(null)}>
                    <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <select value={slotForm.classroomId} onChange={e => setSlotForm({ ...slotForm, classroomId: e.target.value })}
                                    ...
                                    {classrooms.map(c => <option key={c.id} value={c.id}>{c.name} ({c.gradeLevel}. Sınıf)</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="text-white/60 text-sm">Ders</label>
                                <select value={slotForm.subjectId} onChange={e => setSlotForm({ ...slotForm, subjectId: e.target.value })}
                                    className="w-full mt-1 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-cyan-500/50">
...
function DutyTab() {
    ...
    const loadAll = async () => {
        try {
            const [sch, tch]: any[] = await Promise.all([
                dutyAPI.list(),
                dutyAPI.getTeachers(),
            ]);
        ...
        <>
            ...
            {showCreate && (
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 space-y-3">
                    <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Nöbet listesi adı"
                    ...
                    <div className="flex gap-3">
                        <button onClick={createSchedule} className="px-4 py-2 rounded-lg bg-cyan-500 text-white hover:bg-cyan-600 transition-all">Oluştur</button>
                        <button onClick={() => setShowCreate(false)} className="px-4 py-2 rounded-lg bg-white/10 text-white/60 hover:bg-white/20 transition-all">İptal</button>
                    </div>
                </div>
            )}
            ...
            {schedules.length > 0 && (
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                    {schedules.map(s => (
                        <button key={s.id} onClick={() => loadSchedule(s.id)}
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm whitespace-nowrap transition-all
                            ...
                            {s.name}
                            <span onClick={(e) => { e.stopPropagation(); deleteSchedule(s.id); }}
                                className="ml-1 text-red-400/60 hover:text-red-400 cursor-pointer">✕</span>
                        </button>
                    ))}
                </div>
            ...
            {selected ? (
                <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                    <div className="p-4 border-b border-white/5 flex items-center justify-between">
                        <div>
                            <span className="text-white/40 text-sm">📆 {selected.weekStart} — {selected.weekEnd}</span>
                        </div>
                    </div>
                    <div className="grid grid-cols-5 divide-x divide-white/5">
                        {DAYS.map((day, di) => (
                            <div key={di} className="min-h-[200px]">
                                <div className="bg-[#1a1a2e]/60 px-3 py-2 text-center text-white/80 text-sm font-semibold border-b border-white/5">{day}</div>
                                <div className="p-2 space-y-2">
                                    {getAssignments(di + 1).map((a: any) => (
                                        <div key={a.id} className="bg-gradient-to-r from-orange-500/15 to-red-500/10 rounded-lg p-3 relative group">
                                            <div className="text-lg">{a.teacher?.avatar || '👩‍🏫'}</div>
                                            <div className="text-white text-sm font-medium mt-1">{a.teacher?.name}</div>
                                            {a.location && <div className="text-white/40 text-xs mt-0.5">📍 {a.location}</div>}
                                            <button onClick={() => deleteAssignment(a.id)}
                                                className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500/80 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">✕</button>
                                        </div>
                                    ))}
                                    <button onClick={() => setAddAssign(di + 1)}
            ...
            {addAssign !== null && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setAddAssign(null)}>
                    <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-white mb-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <select value={assignForm.location} onChange={e => setAssignForm({ ...assignForm, location: e.target.value })}
                                    className="w-full mt-1 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-cyan-500/50">
                                    <option value="">Konum seçin (opsiyonel)</option>
                                    {DUTY_LOCATIONS.map(loc => <option key={loc} value={loc}>{loc}</option>)}
                                </select>
                            </div>
                        </div>
                        <div className="flex gap-3 mt-6">
                            ...
                            <button onClick={() => setAddAssign(null)}
                                className="px-6 py-2.5 rounded-lg bg-white/10 text-white/60 hover:bg-white/20 transition-all">
                                İptal
                            </button>
                        </div>
...
function UsersTab() {
    ...
    const roleLabels: Record<string, string> = {
        SUPER_ADMIN: 'Süper Admin', SCHOOL_ADMIN: 'Okul Müdürü', TEACHER: 'Öğretmen', STUDENT: 'Öğrenci', PARENT: 'Veli',
    };

    const filtered = users.filter(u => {
        const matchText = u.name?.toLowerCase().includes(filter.toLowerCase()) ||
            u.email?.toLowerCase().includes(filter.toLowerCase());
        const matchRole = !roleFilter || u.role === roleFilter;
        return matchText && matchRole;
    });

    const counts = { total: users.length, teachers: users.filter(u => u.role === 'TEACHER').length, students: users.filter(u => u.role === 'STUDENT').length, parents: users.filter(u => u.role === 'PARENT').length };
        ...
        <>
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-3 text-sm">
                    {[['Tümü', '', counts.total], ['Öğretmen', 'TEACHER', counts.teachers], ['Öğrenci', 'STUDENT', counts.students], ['Veli', 'PARENT', counts.parents]].map(([label, key, count]) => (
                        <button key={String(key)} onClick={() => setRoleFilter(String(key))}
            ...
            <input value={filter} onChange={e => setFilter(e.target.value)} placeholder="🔍 İsim veya email ile ara..."
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tbody>
                            {filtered.map(u => (
                                <tr key={u.id} className="border-t border-white/5 hover:bg-white/5 transition-colors group">
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            <span className="text-xl">{u.avatar || '👤'}</span>
                                            <div>
                                                <div className="text-white font-medium text-sm">{u.name}</div>
                                                {u.gradeLevel && <div className="text-white/30 text-xs">{u.gradeLevel}. Sınıf</div>}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-white/50 text-sm">{u.email}</td>
                                    <td className="px-4 py-3 text-center">
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium border ${roleColors[u.role] || 'bg-white/10 text-white/50 border-white/10'}`}>
                                    ...
                                    <td className="px-4 py-3 text-center text-cyan-400 font-medium text-sm">{(u.xp || 0).toLocaleString()}</td>
                                    <td className="px-4 py-3 text-center">
...
function CurriculumTab() {
    ...
    useEffect(() => {
        curriculumAPI.tree().then((res: any) => { setSubjects(res.data || []); setLoading(false); }).catch(() => setLoading(false));
    }, []);
                        ...
                        <div key={s.id} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                            <div className="px-5 py-4 flex items-center gap-3 border-b border-white/5">
                                ...
                                <div>
                                    ...
                                    <div className="text-white/40 text-xs">{s.code} • Sınıf {s.gradeMin}-{s.gradeMax}</div>
                                </div>
                                <span className="ml-auto bg-white/10 text-white/50 px-2 py-1 rounded-full text-xs">{s.topics?.length || 0} konu</span>
                            </div>
                            {s.topics?.length > 0 && (
                                    ...
                                    {s.topics.map((t: any) => (
                                        <div key={t.id} className="flex items-center gap-2 text-sm text-white/60 py-1">
                                            ...
                                            {t.subTopics?.length > 0 && (
                                                <span className="text-white/30 text-xs ml-1">({t.subTopics.length} alt konu)</span>
                                            )}
                                        </div>
                                    ))}
                                </div>
    ...
    return (
        <>
            ...
            <div className="flex flex-wrap gap-3 mb-4">
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Soru metni ara..."
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
                {['EASY', 'MEDIUM', 'HARD'].map(d => (
                    <div key={d} className={`rounded-xl p-4 border border-white/10 ${DIFF_COLORS[d]}`}>
                        <div className="text-2xl font-black">{questions.filter(q => q.difficulty === d).length}</div>
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
                {filtered.map(q => (
                    <div key={q.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-4 group hover:bg-white/8 transition-all">
                        <span className={`px-2 py-0.5 rounded-md text-xs font-bold border shrink-0 ${DIFF_COLORS[q.difficulty] || 'bg-white/10 text-white/50 border-white/10'}`}>
...
function AnalyticsTab() {
    ...
    useEffect(() => {
        analyticsAPI.school().then((res: any) => { setData(res.data); }).catch(() => { }).finally(() => setLoading(false));
    }, []);
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {[
                    { label: 'Toplam Oturum', value: overview.totalSessions ?? '—', icon: '🎮', color: 'from-blue-500/20 to-blue-600/10' },
                    { label: 'Doğru Cevap', value: `${Math.round((overview.correctRate || 0) * 100)}%`, icon: '✅', color: 'from-green-500/20 to-green-600/10' },
                    { label: 'Aktif Öğrenci', value: overview.activeStudents ?? '—', icon: '👨‍🎓', color: 'from-purple-500/20 to-purple-600/10' },
                    { label: 'Top. Soru Çözüldü', value: (overview.totalAnswers || 0).toLocaleString(), icon: '❓', color: 'from-orange-500/20 to-orange-600/10' },
                ].map((s, i) => (
                    <div key={i} className={`bg-gradient-to-br ${s.color} border border-white/10 rounded-xl p-5`}>
                                ...
                                <div key={s.id} className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between mb-1">
                                            <span className="text-white text-xs font-medium truncate max-w-[120px]">{s.name}</span>
                                            <span className="text-cyan-400 text-xs font-bold">{(s.xp || 0).toLocaleString()} XP</span>
                                        </div>
                                        <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 transition-all duration-700" style={{ width: `${((s.xp || 0) / maxXP) * 100}%` }} />
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="space-y-3">
                            {gameStats.map((g: any, i: number) => {
                                ...
                                return (
                                    <div key={i}>
                                        <div className="flex justify-between mb-1">
                                            <span className="text-white text-xs font-medium">{g.gameMode || 'Bilinmeyen'}</span>
                                            <span className="text-white/50 text-xs">{count} oturum · %{correct} doğru</span>
                                        </div>
                                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full rounded-full bg-gradient-to-r from-purple-500 to-pink-500" style={{ width: `${(count / maxPlays) * 100}%` }} />
                                        </div>
                                    </div>
                                );
                            })}
