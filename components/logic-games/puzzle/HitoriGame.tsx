...
const HitoriGame: React.FC<HitoriGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const initGame = useCallback(() => {
        const initialBoard = Array(size).fill(null).map(() => Array(size).fill('white' as CellState));
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
                    ...
                    <div className="bg-gradient-to-br from-slate-500 via-gray-500 to-slate-600 rounded-2xl p-8 md:p-12 mb-8">
                        <div className="flex flex-col items-center">
                            ...
                            >
                                {board.length > 0 && boardData.grid.map((row, r) =>
                                    row.map((val, c) => {
                                        ...
                                        if (state === 'black') {
                                            cellStyle = isErrorAdj ? 'bg-red-600 text-transparent animate-pulse ring-2 ring-inset ring-red-900' : 'bg-slate-900 text-transparent hover:bg-slate-800';
                                        } else if (state === 'circled') {
                                            cellStyle = 'bg-white text-slate-900';
                                            contentStyle = isErrorDupe ? 'border-4 border-red-500 rounded-full animate-bounce' : 'border-4 border-emerald-500 rounded-full text-emerald-700';
                                        } else {
                                            if (isErrorDupe) {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`w-10 h-10 md:w-14 md:h-14 flex items-center justify-center ${contentStyle}`}>
                                                    {val}
                                                </div>
                                            </button>
                                        )
                        ...
                        <button onClick={() => setShowRules(true)} className="w-full bg-slate-500 hover:bg-slate-400 text-white text-xl font-black py-4 rounded-2xl transition-all">
                            📖 NASIL OYNANIR?
                        </button>
                    </div>
                </div>
                ...
                {showRules && (
                    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center rounded-3xl animate-in fade-in zoom-in">
                        <div className="bg-slate-800/80 p-6 md:p-8 rounded-3xl border border-slate-500/30 max-w-md w-full">
                            ...
                            <h3 className="text-2xl md:text-3xl font-black text-slate-300 mb-4 uppercase">Nasıl Oynanır?</h3>
                            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base font-medium">
                                <li className="flex gap-2"><span className="text-slate-400">1.</span> Her satır ve sütunda bir sayı en fazla bir kez bulunabilir. Aynı olanları elemek için üzerlerine tıklayıp siyaha boyamalısın.</li>
                                <li className="flex gap-2"><span className="text-slate-400">2.</span> Siyah kareler yatay veya dikeyde birbirine değemez! (Çapraz değebilirler).</li>
                                <li className="flex gap-2"><span className="text-slate-400">3.</span> Karalanmamış (beyaz) tüm hücreler birbiriyle dikey/yatay olarak bağlantılı kalmalı, ızgara siyahlardan dolayı bölünmemeli.</li>
                                <li className="flex gap-2"><span className="text-slate-400">4.</span> Bir hücreye tıklamak onu siyah yapar, tekrar tıklamak çember (kesin beyaz) içine alır, bir daha tıklamak normale çevirir.</li>
                            </ul>
                            <button
                ...
                {isGameOver && (
                    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in p-6 text-center rounded-3xl">
                        <h3 className={`text-5xl md:text-7xl font-black mb-4 ${timeLeft === 0 ? 'text-red-500' : 'text-slate-300'}`}>
                            {timeLeft === 0 ? 'SÜRE BİTTİ' : 'BÜYÜK ELEME BAŞARILI!'}
                        </h3>
                        <p className="text-xl md:text-2xl text-white font-bold mb-8">Kazanılan Yıldız: <span className="text-slate-400">⭐ {score}</span></p>
