...
const BOARDS: Record<number, BoardData> = {
    4: {
        regions: [
            { id: 1, cells: [{ r: 0, c: 0 }, { r: 0, c: 1 }, { r: 1, c: 0 }], maxSize: 3 },
            { id: 2, cells: [{ r: 0, c: 2 }, { r: 0, c: 3 }, { r: 1, c: 3 }], maxSize: 3 },
            { id: 3, cells: [{ r: 1, c: 1 }, { r: 1, c: 2 }, { r: 2, c: 1 }, { r: 2, c: 2 }], maxSize: 4 },
            { id: 4, cells: [{ r: 2, c: 0 }, { r: 3, c: 0 }, { r: 3, c: 1 }], maxSize: 3 },
            { id: 5, cells: [{ r: 2, c: 3 }, { r: 3, c: 2 }, { r: 3, c: 3 }], maxSize: 3 }
        ],
        initial: [
            { r: 0, c: 0, val: 2 },
            { r: 2, c: 2, val: 4 }
    ...
    5: {
        regions: [
            { id: 1, cells: [{ r: 0, c: 0 }, { r: 0, c: 1 }, { r: 1, c: 0 }, { r: 2, c: 0 }], maxSize: 4 },
            { id: 2, cells: [{ r: 0, c: 2 }, { r: 0, c: 3 }, { r: 1, c: 2 }, { r: 1, c: 3 }], maxSize: 4 },
            { id: 3, cells: [{ r: 0, c: 4 }, { r: 1, c: 4 }, { r: 2, c: 4 }], maxSize: 3 },
            { id: 4, cells: [{ r: 1, c: 1 }, { r: 2, c: 1 }, { r: 2, c: 2 }, { r: 3, c: 1 }, { r: 3, c: 2 }], maxSize: 5 },
            { id: 5, cells: [{ r: 2, c: 3 }, { r: 3, c: 3 }, { r: 4, c: 3 }, { r: 4, c: 4 }], maxSize: 4 },
            { id: 6, cells: [{ r: 3, c: 4 }], maxSize: 1 },
            { id: 7, cells: [{ r: 3, c: 0 }, { r: 4, c: 0 }, { r: 4, c: 1 }, { r: 4, c: 2 }], maxSize: 4 }
        ],
        initial: [
            { r: 1, c: 2, val: 3 },
            { r: 3, c: 1, val: 4 },
...
const SuguruGame: React.FC<SuguruGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const isValidBoard = (currentBoard: CellState[][]) => {
        ...
        for (const region of boardData.regions) {
            const vals = region.cells.map(cell => currentBoard[cell.r][cell.c]).filter(v => v !== null) as number[];
    ...
    const handleNumberInput = (num: number) => {
        ...
        // Check region limits
        const region = boardData.regions.find(reg => reg.cells.some(cell => cell.r === r && cell.c === c));
        if (region) {
            if (num > region.maxSize) isInvalid = true;
            const regionHasNum = region.cells.some(cell => (cell.r !== r || cell.c !== c) && board[cell.r][cell.c] === num);
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-8">
                    ...
                    <div className="flex gap-4">
                        <div className="px-6 py-3 bg-slate-800/80 rounded-xl">
                            <span className="text-white font-black">Hata: {mistakes}/{maxMistakes}</span>
                        </div>
                        <div className="px-6 py-3 bg-slate-800/80 rounded-xl">
                            <span className="text-white font-black">⏱️ {Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 text-lg mt-2">{size}x{size} Izgara</p>
                </div>
                                    ...
                                    row.map((val, c) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center text-xl md:text-3xl font-black transition-all ${borders} ${isSelected ? 'bg-indigo-500/50 shadow-inner' :
                                                    val !== null ? (isInitial ? 'bg-indigo-950/80 text-indigo-200' : 'bg-indigo-800/60 text-white') :
                ...
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
                    <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-indigo-500/30 max-w-md w-full">
                        ...
                        <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
                            <li className="flex gap-2"><span className="text-indigo-400 font-bold">1.</span> Her bölgeye 1'den başlayarak bölge boyutu kadar sayı yerleştir.</li>
                            <li className="flex gap-2"><span className="text-indigo-400 font-bold">2.</span> Aynı sayılar yatay, dikey veya çapraz olarak birbirine değemez!</li>
                            <li className="flex gap-2"><span className="text-indigo-400 font-bold">3.</span> Her bölgede her sayı sadece bir kez kullanılabilir.</li>
                        </ul>
                        <button
            ...
            {isGameOver && (
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 p-6 text-center">
                    <div className="bg-slate-800/90 p-8 rounded-3xl border border-indigo-500/30 max-w-md w-full">
                        <h3 className={`text-5xl md:text-6xl font-black mb-4 ${mistakes >= maxMistakes || timeLeft === 0 ? 'text-red-500' : 'text-yellow-400'}`}>
