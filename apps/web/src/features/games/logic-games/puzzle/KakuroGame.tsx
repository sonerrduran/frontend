...
const generateKakuroBoard = (size: number): Cell[][] => {
    ...
    const board: Cell[][] = Array(size).fill(null).map((_, r) =>
        Array(size).fill(null).map((_, c) => ({
            r, c, type: (r === 0 || c === 0) ? 'black' : 'white', value: 0
        }))
    );
        ...
        for (let r = 1; r < size; r++) {
            for (let c = 1; c < size; c++) {
                if (tempBoard[r][c].type === 'white') {
                    ...
                    const available = [1, 2, 3, 4, 5, 6, 7, 8, 9].filter(n => !usedH.has(n) && !usedV.has(n));
                ...
                infoCard={
                    <div className="flex gap-4">
                        <div className="text-center">
                            <div className="text-white font-bold text-lg">❌ {mistakes}/{maxMistakes}</div>
                        </div>
                        <div className={`text-center font-bold text-lg ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : 'text-orange-300'}`}>
                    ...
                    >
                        {board.map((row, r) =>
                            row.map((cell, c) => {
                                ...
                                if (cell.type === 'black') {
                                    return (
                                        <div key={`${r}-${c}`} className="aspect-square bg-slate-800 rounded-sm overflow-hidden"></div>
                                    );
                                }

                                if (cell.type === 'clue') {
                                    return (
                                        <div key={`${r}-${c}`} className="aspect-square bg-slate-800 rounded-sm relative overflow-hidden flex">
