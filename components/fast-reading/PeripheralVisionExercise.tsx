...
const PeripheralVisionExercise: React.FC<PeripheralVisionExerciseProps> = ({ stats, setMode }) => {
    ...
    const words = ['KİTAP', 'UZAY', 'BİLİM', 'ROKET', 'YILDIZ', 'DÜNYA', 'AKIL', 'BİLGİ', 'BAŞARI', 'ODAK', 'MANTIK', 'ZEKA'];
    ...
    useEffect(() => {
        ...
        if (isPlaying) {
            ...
            }, Math.max(500, 2000 - (level * 300))); // Gets faster
        }
        ...
        <div className="w-full max-w-5xl mx-auto px-4 py-8 animate-in fade-in">
            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-white/10 rounded-[40px] p-6 md:p-12 mb-8 shadow-2xl text-center">
                ...
                <div className="relative w-full h-64 md:h-96 bg-black/40 border-2 border-cyan-500/30 rounded-3xl flex items-center justify-center overflow-hidden mb-8">
                    ...
                    {isPlaying && (
                        <>
                            <div className="absolute left-4 md:left-12 text-2xl md:text-5xl font-black text-white/80 transition-all duration-200" style={{ letterSpacing: '0.2em' }}>
                                {leftWord}
                            </div>
                            <div className="absolute right-4 md:right-12 text-2xl md:text-5xl font-black text-white/80 transition-all duration-200" style={{ letterSpacing: '0.2em' }}>
                                {rightWord}
                            </div>
                        </>
                    )}
                ...
                <p className="text-slate-400 font-medium max-w-2xl mx-auto">
                    💡 İpucu: Sadece ortadaki <strong>mavi noktaya</strong> bak. Gözlerini sağa veya sola çevirmeden, her iki taraftaki kelimeleri aynı anda görmeye (algılamaya) çalış.
                </p>
