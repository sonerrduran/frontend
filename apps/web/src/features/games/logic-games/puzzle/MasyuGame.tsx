    ...
    const checkWinCondition = (currentH: boolean[][], currentV: boolean[][]) => {
        ...
        for (const pearl of boardData.pearls) {
            ...
            if (pearl.type === 1) {
                ...
                if (!isCorner(r, c)) { errors = true; break; }
                ...
                if (up) {
                    if (!isStraight(r - 1, c)) { errors = true; break; }
                }
                if (down) {
                    if (!isStraight(r + 1, c)) { errors = true; break; }
                }
                if (left) {
                    if (!isStraight(r, c - 1)) { errors = true; break; }
                }
                if (right) {
                    if (!isStraight(r, c + 1)) { errors = true; break; }
                }
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white font-sans selection:bg-slate-500/30">
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                <span>🔗</span>
                                <div>Tüm noktalardan (hücrelerden) geçmek zorunda olmayan, ancak <b>tek parça, kapalı bir çizgi döngüsü (loop)</b> oluşturmalısın.</div>
                            </li>
                            <li className="flex gap-3">
                                <span>⚪</span>
                                <div><b>Beyaz İnciler:</b> Çizgi incinin üzerinden <b>dümdüz</b> geçmelidir. İnci hücresinden çıkar çıkmaz (hemen öncesi veya sonrası) <b>en az bir tarafta dönüş (köşe) yapmalıdır.</b></div>
                            </li>
                            <li className="flex gap-3">
                                <span>⚫</span>
                                <div><b>Siyah İnciler:</b> Çizgi tam incinin üzerinde <b>kesinlikle 90 derece dönmelidir</b>. Ancak döndükten sonra her iki yöne doğru çıkarken <b>en az iki hücre boyunca dümdüz</b> ilerlemelidir. (Yani dönüşten hemen sonra tekrar dönüş yapılamaz).</div>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-slate-600 hover:bg-slate-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
            ...
            <div className="w-full max-w-3xl px-4 py-6 md:py-12">
                ...
                <div className="flex justify-between items-center mb-12 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                        <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-slate-300'}`}>
                    ...
                    >
                        ...
                        <div className="absolute inset-8 pointer-events-none">
                            {Array(size).fill(0).map((_, i) => (
                                <div key={`bg-v-${i}`} className="absolute top-0 bottom-0 border-l border-slate-300 pointer-events-none" style={{ left: `${i * cellSize}px` }} />
                            ))}
                            {Array(size).fill(0).map((_, i) => (
                                <div key={`bg-h-${i}`} className="absolute left-0 right-0 border-t border-slate-300 pointer-events-none" style={{ top: `${i * cellSize}px` }} />
                            ))}
                        </div>
                            ...
                            {hEdges.map((row, r) =>
                                row.map((edge, c) => (
                                    <button
                                        ...
                                        style={{ top: `${r * cellSize - 15}px`, left: `${c * cellSize + 10}px`, width: `${cellSize - 20}px`, height: `30px` }}
                                    >
                                        <div className={`w-full h-2 rounded-full mx-auto transition-colors ${edge ? 'bg-slate-900 shadow-md' : 'bg-transparent group-hover:bg-slate-300'}`}></div>
                                    </button>
                                ))
                            )}
                        </div>
                            ...
                            {vEdges.map((row, r) =>
                                row.map((edge, c) => (
                                    <button
                                        ...
                                        className="absolute group z-20 outline-none"
                                        style={{ top: `${r * cellSize + 10}px`, left: `${c * cellSize - 15}px`, width: `30px`, height: `${cellSize - 20}px` }}
                                    >
                                        <div className={`h-full w-2 rounded-full my-auto mx-auto transition-colors ${edge ? 'bg-slate-900 shadow-md' : 'bg-transparent group-hover:bg-slate-300'}`}></div>
                                    </button>
                                ))
                            )}
                        </div>
                ...
                <div className="mt-8 text-center bg-slate-800/50 p-4 rounded-2xl border border-white/5 max-w-sm mx-auto">
                    <p className="text-slate-400 text-sm font-medium">Büyük bir döngü oluşturmak için iki hücre arasına tıkla.</p>
                </div>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'İnci İhlali!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Tebrikler Detektif!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'İncilerin üzerinden geçerken dönüş/düz kurallarına dikkat etmedin.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Tüm incileri kurallarına uyarak tek bir kolyede birleştirdin! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-slate-600 hover:bg-slate-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
