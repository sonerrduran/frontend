...
const CheckersGame: React.FC<Props> = ({ onExit }) => {
    ...
    const [lastCapture, setLastCapture] = useState<{ player: Player, position: { r: number, c: number } } | null>(null);
    ...
    const resetGame = () => {
        const initialBoard: (Piece | null)[][] = Array(ROWS).fill(null).map(() => Array(COLS).fill(null));
    ...
    const getValidMoves = (r: number, c: number, currentBoard: (Piece | null)[][], assumePlayer?: Player): Move[] => {
            ...
            <div className="relative max-w-7xl mx-auto">
                ...
                <div className="grid lg:grid-cols-[1fr_600px_1fr] gap-6 items-start">
                    ...
                    <div className="space-y-4">
                        <div className={`bg-gradient-to-br from-red-800 to-red-900 backdrop-blur-xl p-6 rounded-3xl border-2 transition-all ${
                            currentPlayer === 1 && !gameOver ? 'border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)] scale-105' : 'border-slate-700/50'
                        }`}>
                        ...
                        <div className="bg-slate-800/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-700/50 shadow-lg">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="bg-slate-900/50 rounded-xl p-3">
                                    ...
                                    <p className="text-white font-bold text-xl">{captureCount[1] + captureCount[2]}</p>
                                </div>
                                <div className="bg-slate-900/50 rounded-xl p-3">
                                ...
                                <div className="grid grid-cols-8 grid-rows-8 w-full aspect-square border-4 border-amber-950 shadow-inner bg-amber-100 rounded-lg overflow-hidden">
                                    {board.map((row, rIdx) =>
                                        row.map((cell, cIdx) => {
                                            ...
                                            const isLastCapture = lastCapture?.position.r === rIdx && lastCapture?.position.c === cIdx;
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {cell && (
                                                        ...
                                                        `}>
                                                            <div className={`w-[70%] h-[70%] rounded-full border-2 border-white/30 shadow-inner flex items-center justify-center`}>
                                                                {cell.isKing && (
                    ...
                    <div className="space-y-4">
                        <div className={`bg-gradient-to-br from-slate-700 to-slate-900 backdrop-blur-xl p-6 rounded-3xl border-2 transition-all ${
                            currentPlayer === 2 && !gameOver ? 'border-slate-400 shadow-[0_0_30px_rgba(148,163,184,0.4)] scale-105' : 'border-slate-700/50'
                        }`}>
                            <div className="flex items-center gap-3 mb-4">
