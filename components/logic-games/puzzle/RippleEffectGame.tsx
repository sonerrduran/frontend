...
const RippleEffectGame: React.FC<RippleEffectGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
    ...
    const getBorderClasses = (r: number, c: number) => {
        ...
        if (r === size - 1 || boardData.regions[r + 1][c] !== reg) classes += "border-b-[3px] border-b-white ";
        else classes += "border-b ";

        if (c === 0 || boardData.regions[r][c - 1] !== reg) classes += "border-l-[3px] border-l-white ";
        else classes += "border-l ";

        if (c === size - 1 || boardData.regions[r][c + 1] !== reg) classes += "border-r-[3px] border-r-white ";
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/10 bounce-in">
                        ...
                        <ul className="space-y-4 mb-8 text-slate-300 text-lg">
                            <li className="flex gap-3">
                                ...
                                <div>Her bölgeyi <b>1'den o bölgenin hücre sayısına</b> kadar rakamlarla doldur. (Suguru gibi)</div>
                            </li>
                            <li className="flex gap-3 text-emerald-400 font-bold">
                                <span>🌊</span>
                                <span><b>Dalga Etkisi Kuralı:</b> Aynı satır veya sütunda iki tane aynı rakam varsa, aralarında <b>en az o rakamın değeri kadar</b> hücre bulunmalıdır! <br /><span className="text-sm font-normal text-slate-400">(Örn: İki tane 3 arasında en az 3 hücre mesafe olmalı).</span></span>
                            </li>
                        </ul>
                        <button onClick={() => setShowRules(false)} className="w-full py-4 rounded-xl font-black text-xl transition-all bg-teal-600 hover:bg-teal-500 text-white shadow-lg active:scale-95">ANLADIM, BAŞLA!</button>
                    </div>
                </div>
            )}
                ...
                <div className="flex-1 w-full max-w-md">
                    ...
                    <div className="flex justify-between items-center mb-8 bg-slate-800/50 p-4 md:p-6 rounded-3xl border border-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-emerald-500">Ripple Effect</h2>
                        ...
                        <div className="text-right flex flex-col items-end gap-2">
                            <div className="bg-slate-900/80 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                                ...
                                <span className={`text-xl md:text-2xl font-black font-mono ${timeLeft < 60 ? 'text-rose-400 animate-pulse' : 'text-teal-300'}`}>
                        ...
                        >
                            {board.map((row, r) =>
                                row.map((cell, c) => {
                                    ...
                                    const regColors = ['bg-blue-100', 'bg-green-100', 'bg-yellow-100', 'bg-pink-100', 'bg-purple-100', 'bg-teal-100', 'bg-orange-100', 'bg-indigo-100', 'bg-rose-100', 'bg-lime-100', 'bg-fuchsia-100'];
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative h-[50px] text-xl md:text-2xl font-black transition-all duration-200 flex items-center justify-center ${getBorderClasses(r, c)}
                                                ${fixed ? `text-slate-900 font-black ${baseColor} brightness-90` :
                                                    selected ? 'bg-teal-400 text-white shadow-[inset_0_0_15px_rgba(20,184,166,0.8)] z-10' :
                                                        cell !== 0 ? `text-teal-700 ${baseColor} brightness-105 hover:brightness-110` :
                                                            `${baseColor} hover:brightness-95`}
                                            `}
                                        >
                                            {isError && <div className="absolute inset-0 bg-red-500/30 animate-pulse pointer-events-none"></div>}
                ...
                <div className="w-full max-w-xs bg-slate-800 p-6 rounded-3xl shadow-2xl border border-white/10 mt-4 lg:mt-0">
                    ...
                    {selectedCell ? (
                        <p className="text-center text-sm text-slate-400 mb-4 bg-slate-900/50 p-2 rounded-lg border border-slate-700">
                            Seçili bölge boyutu: <b className="text-teal-400">{rSizes[boardData.regions[selectedCell.r][selectedCell.c]]}</b>
                        </p>
                    ) : (
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {Array.from({ length: maxNumber }, (_, i) => i + 1).map(num => (
                            <button
                                ...
                                disabled={!selectedCell || num > rSizes[boardData.regions[selectedCell.r][selectedCell.c]]}
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-slate-800 p-8 md:p-12 rounded-[2rem] max-w-md w-full text-center shadow-2xl border border-white/10 bounce-in">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
                            {mistakes >= maxMistakes ? 'Dalga Çarpıştı!' : timeLeft <= 0 ? 'Süre Doldu!' : 'Harika!'}
                        </h2>
                        <p className="text-slate-300 mb-8 text-lg">
                            {mistakes >= maxMistakes ? 'Aynı rakamlar arasındaki mesafe kuralını ihlal ettin.' : timeLeft <= 0 ? 'Süreyi daha iyi kullanmayı dene.' : `Ripple Effect bulmacasını mükemmel çözdün! Kazanılan Yıldız: ${score}`}
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={initGame} className="w-full py-4 rounded-2xl font-black text-xl transition-all bg-teal-600 hover:bg-teal-500 text-white shadow-lg active:scale-95">Tekrar Dene</button>
