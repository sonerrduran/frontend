...
const SeatingChart: React.FC<Props> = ({ onExit }) => {
    ...
    const clearSeat = (r: number, c: number) => {
        setSeats(prev => prev.map((row, ri) => row.map((s, ci) => ri === r && ci === c ? null : s)));
    };
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 relative">
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-6 md:p-8 rounded-[40px] shadow-2xl border border-white/10 w-full max-w-5xl">
                ...
                <div className="flex flex-wrap items-center justify-center gap-3 mb-6">
                    {LAYOUTS.map((l, i) => (
                        <button key={i} onClick={() => changeLayout(i)}
                ...
                <div className="flex flex-col items-center gap-2 mb-6">
                    {seats.map((row, r) => (
                        <div key={r} className="flex gap-2">
                            {row.map((name, c) => (
                                <button key={c} onClick={() => name ? clearSeat(r, c) : null}
                                    className={`w-20 h-16 md:w-24 md:h-20 rounded-xl text-center flex flex-col items-center justify-center transition-all text-[10px] md:text-xs font-bold border ${name
